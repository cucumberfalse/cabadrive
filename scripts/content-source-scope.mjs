...
export function validatePracticeQuestionSourceScope({ question, source, policy }) {
  ...
  const allowedTopicPolicies = new Set(asArray(policy?.allowedPracticeQuestionTopicMentionsPolicies));
  const requiredExclusions = asArray(policy?.requiredExcludedCategorySpecificSources);

  if (!scope || typeof scope !== "object" || Array.isArray(scope)) {
    errors.push(`${question.id}: source ${sourceId} must define practiceQuestionScope for practice questions.`);
    return errors;
  }
  ...
  if (!Array.isArray(scope.eligibleCategories) || scope.eligibleCategories.length === 0) {
    errors.push(`${question.id}: source ${sourceId} must define non-empty practiceQuestionScope.eligibleCategories.`);
  } else {
    for (const category of scope.eligibleCategories) {
      if (!allowedCategories.has(category)) {
        errors.push(`${question.id}: source ${sourceId} includes unsupported practice category ${category}.`);
      }
    }
    if (!scope.eligibleCategories.includes(question.category)) {
      errors.push(`${question.id}: source ${sourceId} does not allow question category ${question.category}.`);
    }
  }
  ...
  if (!Array.isArray(scope.excludedCategorySpecificSources)) {
    errors.push(`${question.id}: source ${sourceId} must define practiceQuestionScope.excludedCategorySpecificSources.`);
  } else {
    const missingExclusions = requiredExclusions.filter((exclusion) => !scope.excludedCategorySpecificSources.includes(exclusion));
