...
function hasExplanationQualityResidue(value) {
  return (
    typeof value === "string" &&
    (GENERIC_EXPLANATION_FILLER_PATTERN.test(value) ||
      SPANISH_RESIDUE_PATTERN.test(value) ||
      latinResidueTokens(value).length > 0)
  );
}
...
function requireFullQualityExplanationText(value, label, field, errors) {
  ...
  if (hasExplanationQualityResidue(value)) {
    errors.push(
      `${label}: ${field} contains generic filler, untranslated Spanish, transliteration, or unsupported Latin residue.`,
    );
  }
...
export function buildExplanationAlignmentEvidenceEntry({
  question,
  explanation,
  image,
  usage,
...
function findImageAndUsage({ question, imageMetadataManifest }) {
  ...
  const image = usage
    ? (imageMetadataManifest?.images || []).find((item) => item.imageId === usage.imageId)
    : undefined;
  return { image, usage };
}
...
function allVisualObjects(image) {
  ...
  for (const object of [
    ...
    if (isNonEmptyString(object.id))
      objects.set(object.id, [...(objects.get(object.id) || []), object]);
  }
  for (const annotation of image?.annotations || []) {
    if (isNonEmptyString(annotation.id))
      objects.set(annotation.id, [...(objects.get(annotation.id) || []), annotation]);
  }
  return objects;
}
...
function validateVisualClaims({ explanation, image, label, errors }) {
  ...
  for (const claim of visualClaims) {
    ...
    if (
      ...
      errors.push(
        `${label}: visual claim objectType ${claim.objectType} contradicts metadata types ${matchingObjects.map((object) => object.type).join(", ")}.`,
      );
    }
    if (isNonEmptyString(claim.gestureId)) {
      ...
      if (isNonEmptyString(claim.bodyPart) && gesture.bodyPart !== claim.bodyPart) {
        errors.push(
          `${label}: visual claim bodyPart ${claim.bodyPart} contradicts metadata bodyPart ${gesture.bodyPart}.`,
        );
      }
      if (isNonEmptyString(claim.pose) && gesture.pose !== claim.pose) {
        errors.push(
          `${label}: visual claim pose ${claim.pose} contradicts metadata pose ${gesture.pose}.`,
        );
      }
      if (
        isNonEmptyString(claim.actorPerspectiveDirection) &&
        gesture.actorPerspectiveDirection !== claim.actorPerspectiveDirection
      ) {
        errors.push(
      ...
      if (
        isNonEmptyString(claim.viewerPerspectiveDirection) &&
        gesture.viewerPerspectiveDirection !== claim.viewerPerspectiveDirection
      ) {
        errors.push(
...
function requireEvidenceEntry(entry, label, expectedHashes, requiredChecks, errors) {
  ...
  if (entry.status !== "approved")
    errors.push(`${label}: explanation alignment evidence status must be approved.`);
  if (!isNonEmptyString(entry.reviewer))
    errors.push(`${label}: explanation alignment evidence reviewer must be a non-empty string.`);
  ...
  for (const [field, expected] of Object.entries(expectedHashes)) {
    if (entry[field] !== expected)
      errors.push(`${label}: explanation alignment evidence ${field} mismatch.`);
  }
  for (const [field, expected] of Object.entries(expectedHashes)) {
  ...
  for (const check of requiredChecks) {
    if (entry.checks?.[check] !== true)
      errors.push(`${label}: explanation alignment evidence checks.${check} must be true.`);
  }
}
...
export function validateExplanationAlignment({
  ...
  if (!isPlainObject(evidence)) {
    ...
    if (evidence.locale !== locale)
      errors.push(`explanation alignment evidence locale must be ${locale}.`);
  ...
  if (!Array.isArray(evidence.entries))
    errors.push("explanation alignment evidence entries must be an array.");
  const evidenceByQuestionId = new Map();
  for (const entry of evidenceEntries) {
    const label = isNonEmptyString(entry?.questionId)
      ? entry.questionId
      : "explanation evidence entry";
    if (!isPlainObject(entry)) {
      errors.push("explanation evidence entry must be an object.");
      continue;
    }
    if (!isNonEmptyString(entry.questionId))
      errors.push(`${label}: evidence questionId must be a non-empty string.`);
    if (evidenceByQuestionId.has(entry.questionId))
      errors.push(`${entry.questionId}: duplicate explanation alignment evidence.`);
    evidenceByQuestionId.set(entry.questionId, entry);
  }

  for (const explanation of explanations || []) {
    const label = isNonEmptyString(explanation?.questionId)
    ...
    if (explanationByQuestionId.has(explanation.questionId))
      errors.push(`${explanation.questionId}: duplicate explanation entry.`);
    ...
    if (!isNonEmptyString(explanation.textRu))
      errors.push(`${label}: textRu must be a non-empty string.`);
    if (requireFullQuality)
      requireFullQualityExplanationText(explanation.textRu, label, "textRu", errors);
    if (!isNonEmptyString(explanation.correctAnswerId))
      errors.push(`${label}: correctAnswerId must be a non-empty string.`);
    if (explanation.correctAnswerId !== question.correctAnswerId)
      errors.push(`${label}: correctAnswerId must match the current question.`);
    ...
    if (requireFullQuality) {
      requireFullQualityExplanationText(
        explanation.correctAnswerExplanationRu,
        label,
    ...
    if (
      ...
      ].includes(explanation.claimScope)
    ) {
    ...
    for (const answerId of wrongAnswerIds) {
      if (!providedWrongIds.has(answerId))
        errors.push(`${label}: missing wrong-answer rationale for ${answerId}.`);
      if (providedWrongIds.has(answerId) && !isNonEmptyString(wrongAnswerExplanations[answerId])) {
        ...
        requireFullQualityExplanationText(
          wrongAnswerExplanations[answerId],
          label,
          `wrong-answer rationale for ${answerId}`,
          errors,
        );
      }
    }
    for (const answerId of providedWrongIds) {
      if (!sourceAnswerIds.has(answerId))
        errors.push(`${label}: wrongAnswerExplanations references missing answer ${answerId}.`);
      if (answerId === question.correctAnswerId)
        errors.push(
          `${label}: wrongAnswerExplanations must not include the correct answer ${answerId}.`,
        );
    }
    ...
    const requiredChecks = [
      "correctAnswerRationalePresent",
      "wrongAnswerRationalesPresent",
      "claimScopeReviewed",
    ];
    if (question.image) {
      if (!usage)
        errors.push(`${label}: image-backed question is missing a question image usage mapping.`);
      if (!image) errors.push(`${label}: image-backed question is missing image metadata.`);
      if (
        !Array.isArray(explanation.imageDetailReferences) ||
        explanation.imageDetailReferences.length < 1
      ) {
      ...
      for (const detail of usage?.answerCriticalDetails || []) {
        if (detail.criticality === "required" && !refs.has(detail.detailId)) {
          errors.push(
            `${label}: explanation missing required image detail reference ${detail.detailId}.`,
          );
        }
      }
      if (image && usage) {
    ...
    requireEvidenceEntry(
      evidenceByQuestionId.get(explanation.questionId),
      label,
      expectedHashes,
      requiredChecks,
      errors,
  ...
  if (strictCoverage) {
    for (const question of questions || []) {
      if (!explanationByQuestionId.has(question.id))
        errors.push(`${question.id}: missing explanation entry.`);
    }
  }
  for (const entry of evidenceEntries) {
    if (isNonEmptyString(entry?.questionId) && !explanationByQuestionId.has(entry.questionId)) {
      errors.push(
        `${entry.questionId}: explanation alignment evidence has no matching explanation entry.`,
      );
    }
  }
