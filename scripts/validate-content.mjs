...
} from "./content-manual-vehiculo-4ruedas.mjs";
import {
  assertGeneratedContentIndexesFresh,
  combinedContentFromShards,
} from "./content-shards.mjs";
...
} from "./primary-sources-validation.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const errors = [];
const warnings = [];
const qualityGate =
  process.argv.includes("--quality-gate") || process.argv.includes("--final-content");
...
function sha256(relativePath) {
  return createHash("sha256")
    .update(readFileSync(path(relativePath)))
    .digest("hex");
}
  ...
  for (const [kind, shards] of Object.entries(shardedContent.shards || {})) {
    for (const { relativePath, shard } of shards) {
      if (shard.qualityStatus !== "complete") {
        errors.push(
          `${relativePath}: ${kind} shard qualityStatus must be complete for the full content quality gate.`,
        );
      }
    }
  }
...
const translationAlignmentEvidence = readJson(
...
const questionImageMetadataEvidence = readJson(
  "content/validation/question-image-metadata.evidence.json",
);
const imageOverlayManifest = readJson(
...
const explanationAlignmentEvidence = readJson(
...
const learningImageRuntimeManifest = readJson(
...
const primarySourcesValidationMode = qualityGate
  ? "strict"
  : process.env.PRIMARY_SOURCES_VALIDATION_MODE || "draft";
...
if (mode?.mode !== "unofficial_b_fallback") {
  errors.push(
    "Current content mode must use unofficial_b_fallback until official B question bank is validated.",
  );
}
if (!mode?.label?.includes("not an official question bank"))
  errors.push("Content mode label must block official question-bank claims.");
...
for (const source of sources) {
  ...
  if (!policy.allowedJurisdictions.includes(source.jurisdiction))
    errors.push(`${source.id}: unsupported jurisdiction ${source.jurisdiction}`);
  if (!policy.allowedSourceStatuses.includes(source.status))
    errors.push(`${source.id}: unsupported source status ${source.status}`);
  ...
  if (!source.localPath || !existsSync(path(source.localPath)))
    errors.push(`${source.id}: localPath is missing on disk`);
  if (
    source.localPath &&
    existsSync(path(source.localPath)) &&
    source.hash &&
    sha256(source.localPath) !== source.hash
  ) {
...
if (!exam) {
  ...
  if (examSource && exam.sourceHash !== examSource.hash)
    errors.push("Exam sourceHash must match source registry hash.");
  ...
  if (
    exam.status === "defined" &&
    (exam.questionCount <= 0 || exam.timeLimitMinutes <= 0 || exam.passingScore <= 0)
  ) {
...
for (const question of questions) {
  ...
  if (question.category !== "B")
    errors.push(`${question.id}: only category B questions are allowed.`);
  ...
  if (question.contentStatus === "unofficial_fallback") {
    if (!policy.allowUnofficialFallbackPractice)
      errors.push(`${question.id}: unofficial fallback practice is disabled by policy.`);
    if (!source?.unofficial)
      errors.push(`${question.id}: fallback questions must use an explicitly unofficial source.`);
    if (question.status !== "needs_review")
      errors.push(`${question.id}: fallback question status must stay needs_review.`);
    if (
      policy.requireFallbackPracticeReleaseException &&
      !activeExceptionSourceIds.has(question.sourceId)
    ) {
      errors.push(
        `${question.id}: fallback source must be covered by an active release exception.`,
      );
    }
  ...
  if (!question.answers?.some((answer) => answer.id === question.correctAnswerId))
    errors.push(`${question.id}: correctAnswerId must point to an answer.`);
  if (question.image) {
    imageCount += 1;
    if (/^https?:\/\//.test(question.image.localPath))
      errors.push(`${question.id}: image.localPath must be local, not remote.`);
    if (!existsSync(path(question.image.localPath)))
      errors.push(`${question.id}: image file missing: ${question.image.localPath}`);
    if (
      existsSync(path(question.image.localPath)) &&
      sha256(question.image.localPath) !== question.image.sha256
    )
      errors.push(`${question.id}: image hash mismatch.`);
  }
}
if (questions.length < 100) warnings.push("Fallback B question set has fewer than 100 questions.");
if (imageCount < 1)
  errors.push("Question set must include local images where source questions reference images.");

for (const translation of translations) {
  if (!questionIds.has(translation.questionId))
    errors.push(`Translation references missing question ${translation.questionId}`);
  if (!translation.disclaimer?.includes("Неофициальный"))
    errors.push(`${translation.questionId}: translation disclaimer must mark unofficial status.`);
}
errors.push(
...
for (const explanation of explanations) {
  if (!questionIds.has(explanation.questionId))
    errors.push(`Explanation references missing question ${explanation.questionId}`);
  if (!explanation.disclaimer?.includes("не является официальной"))
    errors.push(
      `${explanation.questionId}: explanation disclaimer is missing official-status language.`,
    );
  for (const sourceId of explanation.relatedSourceIds || []) {
    if (!sourceById.has(sourceId))
      errors.push(`${explanation.questionId}: explanation source missing ${sourceId}`);
  }
}
errors.push(
...
for (const term of vocabulary) {
  ...
  for (const questionId of term.sourceQuestionIds || []) {
    if (!questionIds.has(questionId))
      errors.push(`${term.id}: vocabulary question missing ${questionId}`);
  }
}

for (const item of guide) {
  requireString(item.id, "guide.id");
  if (!item.disclaimer?.includes("не официальный"))
    errors.push(`${item.id}: guide disclaimer must mark unofficial status.`);
  ...
  for (const questionId of item.relatedQuestionIds || []) {
    if (!questionIds.has(questionId))
      errors.push(`${item.id}: guide question missing ${questionId}`);
  }
}
...
for (const exception of exceptions) {
  if (exception.status !== "active") continue;
  for (const approvalId of exception.approvalIds || [])
    exceptionByApproval.set(approvalId, exception);
}
for (const approval of approvals) {
  if (approval.status !== "approved") errors.push(`${approval.id}: approval must be approved.`);
  if (
    approval.soloSelfAudit &&
    approval.releaseReadiness === "needs_external_review" &&
    !exceptionByApproval.has(approval.id)
  ) {
    errors.push(`${approval.id}: solo self-audit requires an active release exception.`);
  }
  if (!approval.evidenceBundlePath || !existsSync(path(approval.evidenceBundlePath)))
    errors.push(`${approval.id}: evidence bundle missing.`);
  for (const sourceId of approval.sourceIds || []) {
    const source = sourceById.get(sourceId);
    if (!source) errors.push(`${approval.id}: approval source missing ${sourceId}`);
    if (source && approval.sourceHashes?.[sourceId] !== source.hash)
      errors.push(`${approval.id}: approval source hash mismatch for ${sourceId}`);
  }
}
...
console.log(
  `Difficulty labels validated: ${difficultyValidation.questionCount} questions, ${difficultyValidation.topicCount} topics.`,
);
