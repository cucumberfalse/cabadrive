...
const evidencePath =
  process.env.MANUAL_GUIDE_VISUAL_COMPLETENESS_EVIDENCE_PATH ?? defaultEvidencePath;
...
function removeAllowedCopy(value) {
  return allowedCopyPatterns.reduce(
    (current, rule) => current.replace(globalCopyPattern(rule.pattern), " "),
    value,
  );
}
...
function mobilitySpaceRecord() {
  ...
  const sourceDimensions = existsSync(spaceSourceAssetPath)
    ? readImageDimensions(spaceSourceAssetPath)
    : null;
  return {
    id: "mobility-space-50-people",
    status: "implemented",
    ...
    sourceRegion: {
      coordinateSystem:
        "content/validation/manual-guide/ch1-sustainable-mobility/page-023-context-infographic-source-crop.jpg pixels",
      x: 20,
      y: 268,
      width: 585,
      height: 125,
...
function headrestCombinedRecord() {
  ...
  const sourceDimensions = existsSync(headrestSourceAssetPath)
    ? readImageDimensions(headrestSourceAssetPath)
    : null;
  return {
    id: "headrest-combined-diagram",
    ...
    sourceRegion: {
      coordinateSystem:
        "content/validation/manual-guide/app2-safety-elements/page-132-safety-elements-source-crop.jpg pixels",
      x: 1040,
      y: 2160,
      width: 820,
      height: 600,
...
function safetyEquipmentRecord() {
  const matafuegosDimensions = existsSync(matafuegosAssetPath)
    ? readImageDimensions(matafuegosAssetPath)
    : null;
  const matafuegosSourceDimensions = existsSync(matafuegosSourceAssetPath)
    ? readImageDimensions(matafuegosSourceAssetPath)
    : null;
  const chalecoDimensions = existsSync(chalecoAssetPath)
    ? readImageDimensions(chalecoAssetPath)
    : null;
  const chalecoSourceDimensions = existsSync(chalecoSourceAssetPath)
    ? readImageDimensions(chalecoSourceAssetPath)
    : null;
  return {
    id: "matafuegos-chaleco-reflectivo",
    status: "implemented-app1-only",
    ...
    cards: [
      {
        ...
        sourceSha256: existsSync(matafuegosSourceAssetPath)
          ? sha256File(matafuegosSourceAssetPath)
          : null,
      },
      {
        ...
        sourceSha256: existsSync(chalecoSourceAssetPath)
          ? sha256File(chalecoSourceAssetPath)
          : null,
      },
    ],
    extractionMethod:
    ...
    runtimeDisplay: {
      ...
      noUpscale: true,
      translationDomSelector: ".manual-source-image-term-translations",
      visualHeightToBodyLineEstimate:
        "Matafuegos 330px natural crop height at 340px max display width; no browser pixel upscaling.",
    },
    remainingScopeNote:
...
function hospitalMapRecord() {
  const dimensions = existsSync(hospitalMapAssetPath)
    ? readImageDimensions(hospitalMapAssetPath)
    : null;
  const sourceDimensions = existsSync(hospitalMapSourceAssetPath)
    ? readImageDimensions(hospitalMapSourceAssetPath)
    : null;
  const cropEvidence = existsSync(hospitalMapCropEvidencePath)
    ? JSON.parse(readFileSync(hospitalMapCropEvidencePath, "utf8")).targets.find(
        (entry) => entry.cardId === "app2-hospital-map-source-card",
      )
    : null;
  return {
    id: "app2-hospital-map-source-card",
    ...
    sourceSha256: existsSync(hospitalMapSourceAssetPath)
      ? sha256File(hospitalMapSourceAssetPath)
      : null,
    ...
    runtimeDisplay: {
      ...
      translationDomSelector:
        ".manual-source-image-card + .manual-guide-section-block, .manual-source-image-term-translations",
    },
  };
}

function noAvanzarRecord() {
  const dimensions = existsSync(noAvanzarAssetPath)
    ? readImageDimensions(noAvanzarAssetPath)
    : null;
  const sourceDimensions = existsSync(noAvanzarSourceAssetPath)
    ? readImageDimensions(noAvanzarSourceAssetPath)
    : null;
  ...
  return {
    ...
    sourceSha256: existsSync(noAvanzarSourceAssetPath)
      ? sha256File(noAvanzarSourceAssetPath)
      : null,
    protectedImagePolicy:
  ...
  return {
    ...
    panels: anexoRegulatoryPanelRecords.map((panel) => {
      ...
      return {
        ...
        dimensions,
        sourceDimensions: existsSync(panel.sourceAssetPath)
          ? readImageDimensions(panel.sourceAssetPath)
          : null,
        ...
        byteIdenticalToSource:
          existsSync(panel.assetPath) && existsSync(panel.sourceAssetPath)
...
function blindSpotRecord() {
  const dimensions = existsSync(blindSpotAssetPath)
    ? readImageDimensions(blindSpotAssetPath)
    : null;
  const sourceDimensions = existsSync(blindSpotSourceAssetPath)
    ? readImageDimensions(blindSpotSourceAssetPath)
    : null;
  const cropEvidence = existsSync(blindSpotCropEvidencePath)
    ? JSON.parse(readFileSync(blindSpotCropEvidencePath, "utf8"))
    : null;
  const target = cropEvidence?.targets?.find(
  ...
  return {
    ...
    sourceSha256: existsSync(blindSpotSourceAssetPath)
      ? sha256File(blindSpotSourceAssetPath)
      : null,
...
function tireManufacturingTreadLifeRecord() {
  ...
  const sourceDimensions = existsSync(tireSourceAssetPath)
    ? readImageDimensions(tireSourceAssetPath)
    : null;
...
function isCompleteUserExample(entry) {
  if (entry.status === "implemented-app1-only" || entry.status === "implemented-app2-only")
    return false;
...
function reportEvidenceMismatch(reason, detail) {
  ...
  console.error(
    "Run `node scripts/manual-guide-visual-completeness-audit.mjs --write` to intentionally regenerate it.",
  );
}
...
if (writeMode) {
  ...
  if (copyAudit.findings.length > 0) {
    console.error(
      `manual guide copy audit failed with ${copyAudit.findings.length} finding(s). Evidence: ${evidencePath}`,
    );
    process.exit(1);
  }
  process.exit(0);
