...
function hasTranslationQualityResidue(value) {
  return (
    typeof value === "string" &&
    (DRAFT_WRAPPER_PATTERN.test(value) ||
      SPANISH_MARKER_PATTERN.test(value) ||
...
export function translationTupleForQuestion(question, translation) {
  const answerTranslations = isPlainObject(translation.answerTranslations)
    ? translation.answerTranslations
    : {};
  return {
...
export function buildTranslationAlignmentEvidenceEntry({
  question,
  translation,
  reviewer,
  reviewedAt,
...
export function validateTranslationAlignment({
  ...
  if (!evidence || typeof evidence !== "object") {
    ...
    if (evidence.locale !== locale)
      errors.push(`translation alignment evidence locale must be ${locale}.`);
  ...
  if (!Array.isArray(evidence?.entries))
    errors.push("translation alignment evidence entries must be an array.");
  const evidenceByQuestionId = new Map();

  for (const entry of evidenceEntries) {
    const label = isNonEmptyString(entry?.questionId)
      ? entry.questionId
      : "translation evidence entry";
    ...
    if (evidenceByQuestionId.has(entry.questionId))
      errors.push(`${entry.questionId}: duplicate translation alignment evidence.`);
    evidenceByQuestionId.set(entry.questionId, entry);
    if (entry.status !== "approved")
      errors.push(`${label}: translation alignment evidence status must be approved.`);
    if (!isNonEmptyString(entry.reviewer))
      errors.push(`${label}: translation alignment evidence reviewer must be a non-empty string.`);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(entry.reviewedAt || "")) {
      errors.push(`${label}: translation alignment evidence reviewedAt must be YYYY-MM-DD.`);
    }
    if (!SHA256_PATTERN.test(entry.sourceTextSha256 || ""))
      errors.push(`${label}: sourceTextSha256 must be a sha256 hex digest.`);
    if (!SHA256_PATTERN.test(entry.translationTextSha256 || ""))
      errors.push(`${label}: translationTextSha256 must be a sha256 hex digest.`);
  ...
  for (const translation of translations || []) {
    ...
    if (translationByQuestionId.has(translation.questionId))
      errors.push(`${translation.questionId}: duplicate translation entry.`);
    ...
    if (!isNonEmptyString(translation.questionTextRu))
      errors.push(`${translation.questionId}: questionTextRu must be a non-empty string.`);
    if (requireFullQuality && hasTranslationQualityResidue(translation.questionTextRu)) {
      errors.push(
        `${translation.questionId}: questionTextRu contains draft-wrapper, untranslated Spanish, transliteration, or unsupported Latin residue.`,
      );
    }
    if (!isPlainObject(translation.answerTranslations))
      errors.push(`${translation.questionId}: answerTranslations must be an object.`);
    ...
    for (const answerId of sourceAnswerIds) {
      if (!translationAnswerIds.has(answerId)) {
        ...
        errors.push(
          `${translation.questionId}: answer translation for ${answerId} must be a non-empty string.`,
        );
    ...
    for (const answerId of translationAnswerIds) {
      if (!sourceAnswerIds.has(answerId))
        errors.push(`${translation.questionId}: extra answer translation for ${answerId}.`);
    }
    ...
    if (entry.translationTextSha256 !== fingerprints.translationTextSha256) {
      errors.push(
        `${translation.questionId}: translation alignment translation fingerprint mismatch.`,
      );
    }
  }
  ...
  if (strictCoverage) {
    for (const question of questions || []) {
      if (!translationByQuestionId.has(question.id))
        errors.push(`${question.id}: missing translation entry.`);
    }
  }
  ...
  for (const entry of evidenceEntries) {
    if (isNonEmptyString(entry?.questionId) && !translationQuestionIds.has(entry.questionId)) {
      errors.push(
        `${entry.questionId}: translation alignment evidence has no matching translation entry.`,
      );
    }
  }
