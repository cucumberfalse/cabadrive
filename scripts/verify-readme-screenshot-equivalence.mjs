...
export function compareBoundedScreenshotPixels(first, second, masks, maximumDifferences = 16) {
  if (first.width !== second.width || first.height !== second.height) {
    throw new Error(`screenshot dimensions differ: ${first.width}x${first.height} versus ${second.width}x${second.height}`);
  }
  const differences = [];
  for (let offset = 0; offset < first.pixels.length; offset += 3) {
    const delta = [0, 1, 2].map((channel) => Math.abs(first.pixels[offset + channel] - second.pixels[offset + channel]));
    ...
    if (delta.some((value) => value > 1)) throw new Error(`pixel (${x},${y}) exceeds per-channel delta 1: ${delta.join(",")}`);
    if (!mask) throw new Error(`pixel (${x},${y}) differs outside declared rounded-corner masks`);
    if (differences.length > maximumDifferences) throw new Error(`more than ${maximumDifferences} pixels differ`);
  }
  return differences;
}
...
async function captureTo(outputDir) {
  ...
  await execFileAsync(process.execPath, ["scripts/capture-readme-screenshots.mjs"], {
    cwd: process.cwd(),
    env: { ...process.env, README_SCREENSHOT_OUTPUT_DIR: outputDir, README_SCREENSHOT_PORT: String(port) },
    timeout: 30_000
  });
}
...
export async function verifyIndependentCaptures() {
  ...
  try {
    ...
    for (const name of screenshotNames) {
      ...
      for (const [label, image] of [["first", first], ["second", second]]) {
        const black = findOpaqueBlackRegion(image);
        if (black.found) throw new Error(`${name} ${label} capture contains an opaque-black region`);
      }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  for (const name of screenshotNames) console.log(`${name}: ${result[name].length} differing pixel(s) ${JSON.stringify(result[name])}`);
