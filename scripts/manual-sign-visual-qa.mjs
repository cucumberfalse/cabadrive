...
function writeContactSheetHtml(inventory, finalRowsById) {
  ...
  const bySection = new Map(
    sections.map((sectionId) => [
      sectionId,
      inventory.entries.filter((entry) => entry.sectionId === sectionId),
    ]),
  );
...
async function collectQa(page, rootSelector) {
  return page.locator(rootSelector).evaluate((root) => {
    ...
    const qaImages = images.filter(
      (image) =>
        image.dataset.renderMode ||
        image.closest("[data-entry-id]") ||
        image.closest(".manual-sign-card"),
    );
    ...
    return {
      ...
      unloadedImages: qaImages.filter(
      ...
      oldRenderModeCount: cards.filter(
        (card) => card.getAttribute("data-render-mode") === "source-image-css-clip",
      ).length,
    };
  });
}
  ...
  for (const viewport of [
    ...
    for (const sectionId of sections) {
      ...
      const section = page.locator(
        `[data-testid="manual-guide-section"][data-manual-section-id="${sectionId}"]`,
      );
      await section.waitFor({ state: "visible" });
      await forceLoadQaImages(section);
      await section.screenshot({
        path: repoPath(`${screenshotDir}/${viewport.name}-${sectionId}.png`),
      });
  ...
  try {
    ...
    const summary = {
      ...
      entryCounts: {
        ...
        categoryHeading: inventory.entries.filter((entry) => entry.entryKind === "category-heading")
          .length,
        bySection: Object.fromEntries(
          sections.map((sectionId) => [
            sectionId,
            inventory.entries.filter((entry) => entry.sectionId === sectionId).length,
          ]),
        ),
      },
      contactSheets,
      runtime,
      pendingAuditCount: inventory.entries.filter(
        (entry) =>
          entry.cropAuditStatus !== "reviewed-final-correct" &&
          entry.cropAuditStatus !== "category-heading-dom",
      ).length,
      renderModeViolations: inventory.entries.filter(
        (entry) => signLike(entry) && entry.renderMode !== "individual-source-crop-3x",
      ).length,
      noUpscaleViolationsInInventory: finalRows.filter(
        (row) => signLike(row) && row.noUpscaleProof?.passes !== true,
      ).length,
    };
