...
const evidencePath =
  process.env.MANUAL_GUIDE_TRANSLATION_COMPLETENESS_EVIDENCE_PATH ?? defaultEvidencePath;
const sectionRoot =
  process.env.MANUAL_GUIDE_TRANSLATION_COMPLETENESS_SECTION_ROOT ?? defaultSectionRoot;
const introductionPathValue = process.env.MANUAL_GUIDE_TRANSLATION_COMPLETENESS_INTRODUCTION_PATH;
const introductionPath =
  introductionPathValue === ""
    ? null
    : (introductionPathValue ??
      (sectionRoot === defaultSectionRoot ? defaultIntroductionPath : null));
const manualSignInventoryPathValue =
  process.env.MANUAL_GUIDE_TRANSLATION_COMPLETENESS_MANUAL_SIGN_INVENTORY_PATH;
const manualSignInventoryPath =
  manualSignInventoryPathValue === ""
    ? null
    : (manualSignInventoryPathValue ??
      (sectionRoot === defaultSectionRoot ? defaultManualSignInventoryPath : null));
...
const terminologyDecisions = [
  ...
  {
    termEs: "velocidad adecuada del tramo",
    translationRu: "подходящая скорость для этого участка",
  },
...
const acceptedExceptionPatterns = [
  ...
  },
  {
    ...
    pattern:
      /^(?:MiBA|Boti|WhatsApp|Ecobici|Metrobus|Metrobús|BA Ecobici by Tembici|Sube y Baja|Metrobus de Buenos Aires|Scoring|Cabadrive|Bitren|Isofix|Latch)$/u,
  ...
  },
  {
    ...
    pattern:
      /^(?:Ley|Decreto|Anexo|Autopista|Avenida|Calle|Paseo|Buenos Aires|Ciudad Autonoma de Buenos Aires|Ciudad Autónoma de Buenos Aires)(?:\b|$)/u,
...
function loadRenderedManualSignCatalogRecords(manualSections) {
  ...
  assertCondition(existsSync(manualSignInventoryPath), "manual sign inventory is missing", {
    manualSignInventoryPath,
  });

  const inventory = JSON.parse(readFileSync(manualSignInventoryPath, "utf8"));
  assertCondition(
    isObject(inventory) && Array.isArray(inventory.entries),
    "manual sign inventory entries must be an array",
    {
      manualSignInventoryPath,
    },
  );
  ...
  for (const sectionMeta of manualSections) {
    for (const [blockIndex, block] of (sectionMeta.section.blocks ?? []).entries()) {
      ...
      const catalogSectionId =
        typeof block.sectionId === "string" ? block.sectionId : sectionMeta.section.sectionId;
  ...
  return inventory.entries.flatMap((entry, entryIndex) => {
    ...
    assertCondition(
      typeof entry.spanishLabel === "string",
      "manual sign entry spanishLabel must be a string",
      { entryIndex },
    );
    assertCondition(
      typeof entry.russianTranslation === "string",
      "manual sign entry russianTranslation must be a string",
      { entryIndex },
    );
...
function synthesizeIntroductionSections(modulePath, exports) {
  ...
  assertCondition(
    ...
    "introductionNavigation must be exported as an array",
    { modulePath },
  );
  assertCondition(isObject(pandemia), "pandemiaVialSection must be exported as an object", {
    modulePath,
  });
  assertCondition(
    Array.isArray(articles),
    "introductionArticleSections must be exported as an array",
    { modulePath },
  );
  ...
  return navigation.map((entry) => {
    assertCondition(
      ...
      "introductionNavigation entry must have an id",
      { modulePath, entry },
    );
    ...
    if (!isPandemiaRoute) {
      assertCondition(
        isObject(article),
        "article introduction route must have matching introductionArticleSections data",
        { modulePath, entry },
      );
    }
...
function loadIntroductionSections() {
  ...
  assertCondition(existsSync(introductionPath), "introduction data source is missing", {
    introductionPath,
  });
...
function shouldInspectPath(path, key, value, parent) {
  ...
  if (
    parent &&
    Array.isArray(parent.termTranslations) &&
    (key === "termEs" || key === "translationRu")
  )
    return true;
...
function isStructuredPair(record) {
  ...
  return (
    record.sectionStrings?.some(
      (candidate) =>
        (candidate.fieldPath === translationPath || candidate.fieldPath === labelPath) &&
        /\p{Script=Cyrillic}/u.test(candidate.text),
    ) === true
  );
}
...
function phraseHasInlineRussianSupport(text, phrase) {
  const pattern = new RegExp(
    `${escapeRegExp(phrase)}[»”"']?\\s*\\([^)]*\\p{Script=Cyrillic}[^)]*\\)`,
    "iu",
  );
  if (pattern.test(text)) return true;
  const immediatePairPattern = new RegExp(
    `${escapeRegExp(phrase)}[»”"']?\\s*(?:[-:–—]|=)\\s*\\p{Script=Cyrillic}`,
    "iu",
  );
...
function detectedSpanishPhrases(text) {
  ...
  return unique.filter((phrase) => {
    ...
    return !unique.some((other) => {
      ...
      return (
        normalizedOther !== normalizedPhrase &&
        normalizedOther.length > normalizedPhrase.length &&
        normalizedOther.includes(normalizedPhrase)
      );
    });
  });
}
...
function latinSegments(text) {
  const matches =
    text.match(/[\p{Script=Latin}\p{Mark}]+(?:[ /'’-]+[\p{Script=Latin}\p{Mark}]+)*/gu) ?? [];
...
function requiredProbeCoverage(candidates, records) {
  return requiredProbeTerms.map((probe) => {
    ...
    const candidate = candidates.find(
      (entry) =>
        entry.sectionId === "ch3-highways" &&
        normalizeText(entry.detectedSpanishPhrase) === normalizedProbe,
    );
    const coveredText = records.find(
      (record) =>
        record.sectionId === "ch3-highways" && normalizeText(record.text).includes(normalizedProbe),
    );
    return {
      probe,
...
function validateEvidenceShape(value) {
  ...
  assertCondition(
    value.generatedBy === "scripts/manual-guide-translation-completeness-audit.mjs",
    "evidence.generatedBy mismatch",
  );
  ...
  assertCondition(
    Array.isArray(value.requiredProbeCoverage),
    "evidence.requiredProbeCoverage must be an array",
  );
  assertCondition(Array.isArray(value.exceptions), "evidence.exceptions must be an array");
  assertCondition(
    Array.isArray(value.reviewedIdentifierPolicies),
    "evidence.reviewedIdentifierPolicies must be an array",
  );
  assertCondition(isObject(value.routeInventory), "evidence.routeInventory must be an object");
  assertCondition(
    isObject(value.routeInventory.counts),
    "evidence.routeInventory.counts must be an object",
  );
  assertCondition(
  ...
  for (const exception of value.exceptions) {
    assertCondition(
      exception.disposition === "allowed-narrow-exception",
      "exceptions must use allowed-narrow-exception disposition",
      exception,
    );
    assertCondition(
      !genericTrafficTerms.has(normalizeText(exception.detectedSpanishPhrase)),
      "generic traffic terms cannot be allowlisted as exceptions",
      exception,
    );
    assertCondition(
      exception.note !== "uppercase acronym or compact official identifier",
      "generic uppercase exceptions are forbidden",
      exception,
    );
  }
}
...
function reportEvidenceMismatch(reason, detail) {
  ...
  console.error(
    "Run `node scripts/manual-guide-translation-completeness-audit.mjs --write` to intentionally regenerate it.",
  );
}
...
function reportValidationFindings(findings) {
  ...
  console.error(
    `manual guide translation completeness audit found ${findings.length} validation finding(s):`,
  );
...
const broadExceptions = exceptions.filter((entry) =>
  genericTrafficTerms.has(normalizeText(entry.detectedSpanishPhrase)),
);
...
const document = {
  ...
  counts: {
    ...
    translatedOrRetainedWithSupport: residues.filter(
      (entry) =>
        entry.disposition !== "unresolved" && entry.disposition !== "allowed-narrow-exception",
    ).length,
...
if (!existsSync(evidencePath)) {
  ...
  try {
    ...
    console.error(
      `manual guide translation completeness audit failed: committed evidence is malformed: ${error.message}`,
    );
    malformed = true;
    failed = true;
  }
