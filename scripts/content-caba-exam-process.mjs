...
const allowedCallouts = new Set([
  "required_step",
  "optional_preparation",
  "adjacent_path",
  "warning",
]);
const allowedVolatility = new Set([
  "stable_procedure",
  "volatile_fee",
  "volatile_location",
  "volatile_screen",
...
function isOfficialUrl(value) {
  return (
    typeof value === "string" &&
    /^https:\/\/(buenosaires\.gob\.ar|documentosboletinoficial\.buenosaires\.gob\.ar|boletadepago\.seguridadvial\.gob\.ar)\//.test(
      value,
    )
  );
}
...
export function validateCabaExamProcessGuide({ guide, fileExists = () => false } = {}) {
  ...
  if (guide.id !== "caba-exam-process")
    errors.push("caba exam process guide id must be caba-exam-process.");
  if (guide.locale !== "ru") errors.push("caba exam process guide locale must be ru.");
  if (!allowedStatuses.has(guide.status))
    errors.push(`caba exam process guide status is unsupported: ${guide.status}`);
  if (!allowedContentStatuses.has(guide.contentStatus))
    errors.push("caba exam process guide must be marked as unofficial_learning_aid.");
  if (!isIsoDate(guide.lastReviewedAt))
    errors.push("caba exam process guide lastReviewedAt must be an ISO date.");
  ...
  if (!guide.disclaimerRu?.includes("неофициальная"))
    errors.push("caba exam process disclaimer must mark the guide as unofficial.");
  if (!guide.officialActionWarningRu?.includes("официальные"))
    errors.push("caba exam process official action warning must direct users to official pages.");

  const scope = guide.primaryScope || {};
  if (scope.jurisdiction !== "CABA")
    errors.push("caba exam process primaryScope.jurisdiction must be CABA.");
  if (scope.procedure !== "otorgamiento")
    errors.push("caba exam process primaryScope.procedure must be otorgamiento.");
  ...
  for (const source of guide.sources || []) {
    ...
    if (sourceById.has(source.id))
      errors.push(`Duplicate caba exam process source id: ${source.id}`);
    sourceById.set(source.id, source);
    requireString(errors, source.title, `${source.id}.title`);
    if (!isOfficialUrl(source.url))
      errors.push(`${source.id}.url must be an approved official URL.`);
    if (!isIsoDate(source.checkedAt)) errors.push(`${source.id}.checkedAt must be an ISO date.`);
    if (!allowedOwners.has(source.officialOwner))
      errors.push(`${source.id}.officialOwner is unsupported.`);
    if (!allowedCurrentness.has(source.currentnessStatus))
      errors.push(`${source.id}.currentnessStatus is unsupported.`);
    requireString(errors, source.resultRu, `${source.id}.resultRu`);
  }
  for (const sourceId of requiredSourceIds) {
    if (!sourceById.has(sourceId))
      errors.push(`caba exam process missing required source ${sourceId}.`);
  }
  ...
  for (const section of guide.sections || []) {
    ...
    if (!allowedCallouts.has(section.calloutType))
      errors.push(`${section.id}.calloutType is unsupported.`);
    if (section.volatility && !allowedVolatility.has(section.volatility))
      errors.push(`${section.id}.volatility is unsupported.`);
    if (section.volatility && !section.volatilityWarningRu)
      errors.push(`${section.id}: volatile sections must include volatilityWarningRu.`);
    if (
      section.id.includes("psychophysical") &&
      /диагноз|вы годны|вам подходит|вы подходите/i.test(section.bodyRu.join(" "))
    ) {
      errors.push(
        `${section.id}: psychophysical section must avoid medical eligibility advice wording.`,
      );
    }
  }
  for (const sectionId of requiredSectionIds) {
    if (!sectionIds.has(sectionId))
      errors.push(`caba exam process missing required section ${sectionId}.`);
  }
  ...
  for (const group of guide.officialLinks || []) {
    ...
    for (const link of group.links || []) {
      ...
      if (!isOfficialUrl(link.url))
        errors.push(`${group.id}.${link.labelRu}: link URL must be official.`);
      if (!sourceById.has(link.sourceId))
        errors.push(`${group.id}.${link.labelRu}: missing source ${link.sourceId}.`);
    }
  }
  ...
  if (!Array.isArray(guide.optionalImages))
    errors.push("cabaExamProcess.optionalImages must be an array.");
  for (const image of guide.optionalImages || []) {
    ...
    if (/^https?:\/\//.test(image.localPath))
      errors.push(`${image.id}: localPath must be local, not remote.`);
    if (!image.localPath.startsWith("content/assets/"))
      errors.push(`${image.id}: localPath must live under content/assets/.`);
