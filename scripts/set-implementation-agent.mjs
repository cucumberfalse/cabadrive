...
const implementation = String(args.implementation || args.impl || "")
  .trim()
  .toLowerCase();
const review = String(args.review || "")
  .trim()
  .toLowerCase();
...
if (!allowedImplementation.has(implementation)) {
  console.error(
    "Usage: node scripts/set-implementation-agent.mjs --implementation <claude|codex> [--review <codex|claude|gemini>]",
  );
  process.exit(1);
}
