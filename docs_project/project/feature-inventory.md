...
- `Источники` / official primary-source reader from feature `019-primary-sources-section`: exposes every official manifest entry as a local reference surface, imports the generated coverage inventory through the app data boundary, defaults approved translated chunks to simple Russian, allows full Russian translation and original Spanish views, omits simplified Spanish, and keeps all Russian source-reader content outside `content/official-documents/`.
- Primary-source Russian coverage is still partial while document batches continue. The reader shows not-yet-translated status honestly for manifest/coverage documents without approved Russian shards and must not be presented as a finished whole-corpus release until strict primary-source validation and exact-text readiness blockers are resolved.
