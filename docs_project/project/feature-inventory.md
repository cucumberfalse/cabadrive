...
- `Источники` / official primary-source reader from feature `019-primary-sources-section`: uses every official manifest entry for corpus counts/status, imports generated coverage metadata through the app data boundary, exposes only approved learner chunks as readable rows, defaults those chunks to simple Russian, allows full Russian translation and learner-shard original Spanish views, omits simplified Spanish, and keeps all Russian source-reader content outside `content/official-documents/`.
- Primary-source Russian coverage is still partial while document batches continue. The reader hides/blocks documents and chunks without approved Russian shards from the reading list and represents them only through aggregate not-yet-available status; it must not be presented as a finished whole-corpus release until strict primary-source validation and exact-text readiness blockers are resolved.
