...
- `scripts/official-documents-validation.mjs` owns no-file-I/O validation for the governed official-documents manifest under `content/official-documents/manifest.json`, including required source metadata, local archive paths, SHA-256-shaped hashes, conversion notes, currentness/effective-status fields, exact-text validation status, raw evidence for lossy formats, and source-trace checks that current guide claims do not cite missing or not-current documents.
