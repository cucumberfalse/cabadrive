...
- `Источники` is implemented as a distinct official primary-source reader separate from `Материалы`. It covers every entry in `content/official-documents/manifest.json` at the list/status level, uses the generated coverage inventory for chunk navigation, defaults translated chunks to simple Russian, lets learners switch to full Russian translation or original Spanish, omits simplified Spanish, and labels Russian layers as unofficial learning support.
- The current primary-source corpus is intentionally partial while translation batches continue: documents without approved Russian learner shards remain visible with `not translated` status and Spanish archive-derived chunk access. The UI must not describe the whole corpus as release-ready until strict/final primary-source gates pass.
