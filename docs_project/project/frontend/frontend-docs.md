...
- Release status: the reader UI is implemented and locally tested, but the feature must not be described as final-release complete until the official archive exact-text gate passes or receives an explicit narrower disposition.
