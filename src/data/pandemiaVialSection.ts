...
export const manualGuideNavigation: ManualGuideNavigationEntry[] = [
  ...
  },
  {
    id: "chapter-1",
    kind: "group",
    labelRu: "Глава 1. К устойчивой мобильности",
    sourceTitleEs: "CAPÍTULO 1: HACIA UNA MOVILIDAD SUSTENTABLE",
    sourcePage: 20,
    status: "pending",
    children: [
      ["chapter-1-cities", "Города для людей", "Ciudades para las personas"],
      ["chapter-1-sustainable-mobility", "Что такое устойчивая мобильность?", "¿Qué es la movilidad sustentable?"],
      ["chapter-1-pedestrian-priority", "Приоритет пешеходов", "Prioridad peatonal"],
      ["chapter-1-bicycle", "Велосипед", "Bicicleta"],
      ["chapter-1-public-transport", "Система общественного транспорта", "Sistema de transporte público"],
      ["chapter-1-shared-trip", "Совместная поездка", "Viaje compartido"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "chapter-2",
    kind: "group",
    labelRu: "Глава 2. Вождение - ответственное действие",
    sourceTitleEs: "CAPÍTULO 2: CONDUCIR ES UN ACTO DE RESPONSABILIDAD",
    sourcePage: 42,
    status: "pending",
    children: [
      ["chapter-2-legal", "Правовые обязанности", "Responsabilidades legales"],
      ["chapter-2-documents", "Обязательные документы", "Documentación obligatoria"],
      ["chapter-2-incident-duties", "Обязанности при дорожных инцидентах", "Obligaciones en caso de incidentes viales"],
      ["chapter-2-scoring", "Scoring", "Scoring"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "chapter-3",
    ...
    status: "pending",
    children: [
      ["chapter-3-norm-priority", "Приоритет норм", "Prioridad normativa"],
      ["chapter-3-way-priority", "Приоритет проезда", "Prioridades de paso"],
      ["chapter-3-lights", "Использование фар", "Uso de luces"],
      ["chapter-3-speed", "Скорость", "Velocidad"],
      ["chapter-3-turns", "Повороты на перекрестках", "Giros en intersecciones"],
      ["chapter-3-overtaking", "Обгон и опережение", "Adelantamiento y sobrepaso"],
      ["chapter-3-highways", "Движение по автомагистралям и быстрым дорогам", "Conducción en autopistas y otras vías rápidas"],
      ["chapter-3-adverse", "Движение в сложных условиях", "Conducción en situaciones adversas"],
      ["chapter-3-parking", "Остановка и стоянка", "Detención y estacionamiento"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "chapter-4",
    ...
    status: "pending",
    children: [
      ["chapter-4-alcohol-drugs", "Алкоголь и наркотики", "Ingesta de alcohol y drogas"],
      ["chapter-4-fatigue", "Сон и усталость", "Sueño y fatiga"],
      ["chapter-4-stress", "Стресс", "Estrés"],
      ["chapter-4-distractions", "Отвлечения", "Distracciones"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "chapter-5",
    ...
    status: "pending",
    children: [
      ["chapter-5-attitudes", "Типы поведения", "Tipos de actitudes"],
      ["chapter-5-equality", "К равному обществу", "Hacia una sociedad igualitaria"],
      ["chapter-5-gender-violence", "Профилактика и помощь при гендерном насилии", "Prevención y asistencia en situaciones de violencia de género"],
      ["chapter-5-preventive-efficient", "Предупредительное и эффективное вождение", "Conducción preventiva y eficiente"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "annex-1",
    ...
    status: "pending",
    children: [
      ["annex-1-safety", "Элементы безопасности", "Elementos de seguridad"],
      ["annex-1-required", "Другие обязательные элементы безопасности", "Otros elementos de seguridad obligatorios"],
      ["annex-1-recommended", "Рекомендуемые элементы безопасности", "Elementos de seguridad recomendables"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "annex-2",
    ...
    status: "pending",
    children: [
      ["annex-2-social", "Социальная ответственность", "Una responsabilidad social"],
      ["annex-2-safety", "Элементы безопасности", "Elementos de seguridad"],
      ["annex-2-factors", "Факторы вождения", "Factores involucrados en la conducción"],
      ["annex-2-safe-driving", "Безопасное вождение", "Conducción segura"],
      ["annex-2-highways-hospitals", "Автомагистрали и больницы", "Autopistas y Hospitales"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "annex-3",
    ...
    status: "pending",
    children: [
      ["annex-3-profile", "Профиль грузоперевозчика", "Perfil del transportista de cargas"],
      ["annex-3-social", "Социальная ответственность", "Una responsabilidad social"],
      ["annex-3-factors", "Факторы вождения", "Factores involucrados en la conducción"],
      ["annex-3-safe-driving", "Безопасное вождение", "Conducción segura"],
      ["annex-3-safety", "Элементы безопасности", "Elementos de seguridad"],
      ["annex-3-highways", "Автомагистрали", "Autopistas"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  },
  {
    id: "annex-4",
    ...
    status: "pending",
    children: [
      ["annex-4-regulatory", "Предписывающие", "Reglamentarias"],
      ["annex-4-warning", "Предупреждающие", "Preventivas"],
      ["annex-4-informational", "Информационные", "Informativas"],
      ["annex-4-temporary", "Временные", "Transitorias"],
      ["annex-4-horizontal", "Горизонтальная разметка", "Horizontales"],
      ["annex-4-light", "Световые сигналы", "Señalamiento luminoso"]
    ].map(([id, labelRu, sourceTitleEs]) => ({ id, labelRu, sourceTitleEs, status: "pending" as const }))
  }
];
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        textRu:
          "Идея Vision Zero простая и этическая: никто не должен погибать или получать постоянные травмы в дорожных инцидентах. Подход появился в Швеции в 1997 году; эта страна уже больше трех десятилетий считается ориентиром в дорожной безопасности. Поэтому транспортную систему нужно проектировать так, чтобы она сдерживала и уменьшала последствия человеческих ошибок и сама помогала делать движение безопасным."
      },
      {
        id: "principles",
        kind: "list",
