  ...
  blocks: [
    ...
    },
    {
      ...
      itemsRu: [
        ...
        "Исключение 1: знак Pare (стоп) или Ceda el Paso (уступите дорогу) отменяет приоритет на перекрестке без светофора. Pare (стоп) требует снизить скорость и полностью остановиться до нуля перед пешеходным переходом; Ceda el Paso (уступите дорогу) требует уступить, но полная остановка обязательна только если она нужна для движения имеющих преимущество.",
        "Исключение 2: у железнодорожного переезда приоритет у транспортного средства, которое выезжает с paso a nivel (переезда на одном уровне).",
    ...
    },
    {
      ...
      textRu:
        "Если пересекаются дороги разных категорий, преимущество у транспорта на более важной артерии. Иерархия такая: avenida (проспект) выше calle (улицы), calle (улица) выше pasaje (пассажа/узкого проезда)."
    },
    {
      ...
      itemsRu: [
        ...
        "Нужно облегчать возвращение colectivos (автобусов) в поток после остановки на соответствующей parada (остановке).",
        "Обгоняя остановившийся colectivo (автобус), нужно ожидать, что пешеходы могли быть скрыты за ним и могут начать переходить перед ним или за ним."
      ]
    }
  ]
};
