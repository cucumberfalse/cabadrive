  ...
  blocks: [
    ...
    },
    {
      ...
      itemsRu: [
        ...
        "Если срок истек не более года назад, возможна renovación (продление). Если прошло больше года, оформление идет как новое otorgamiento (первичная выдача).",
    ...
    },
    {
      ...
      kind: "source-image-cards",
      titleRu: "Начинающий водитель и cédula (регистрационная карточка)",
      sourceTextEs:
      ...
      cards: [
        {
          ...
          bodyRu:
            "Статус principiante (начинающий водитель) действует для человека, впервые получившего лицензию для мототранспортных средств или автомобилей. Знак нужно возить и показывать по правилам раздела."
        },
        {
          id: "cedulas-source-card",
          titleRu: "Cédula de identificación (идентификационная карточка)",
          displayMode: "full-width",
          maxDisplayWidthPx: 1110,
          sourcePage: 47,
          sourceRegion: { x: 1085, y: 2490, width: 1110, height: 260 },
          assetPath: `${assetRoot}/cedulas-source-as-is.jpg`,
          altRu: "Примеры зеленой, синей, коричневой и розовой cédula (регистрационной карточки).",
          visibleSpanish: true,
          sourceImageException,
          termTranslations: [
            { termEs: "Cédula de identificación del vehículo", translationRu: "Свидетельство идентификации транспортного средства" },
            { termEs: "Cédula verde", translationRu: "Зеленая регистрационная карточка" },
            { termEs: "Cédula azul", translationRu: "Синяя регистрационная карточка" },
            { termEs: "Cédula marrón", translationRu: "Коричневая регистрационная карточка" },
            { termEs: "Cédula rosa", translationRu: "Розовая регистрационная карточка" }
          ],
          bodyRu:
            "Cédula (регистрационная карточка), выданная DNRPA, идентифицирует транспортное средство и владельца или уполномоченное лицо. На изображении показаны прежние цветовые варианты; текущая логика раздела объясняется рядом текстом."
        }
      ],
      visualNotes: [
    ...
    },
    {
      id: "cedula-rules",
      kind: "list",
      titleRu: "Cédula de identificación del vehículo (идентификационная карточка транспортного средства)",
      sourceTextEs:
        "Función... Tipos... Vigencia... Uso... Trámite...",
      itemsRu: [
        "Если cédula (регистрационную карточку) не иметь при себе, невозможно подтвердить, что водитель является владельцем или имеет разрешение пользоваться транспортным средством.",
        "Ранее DNRPA выдавала разные cédulas (регистрационные карточки) по типу транспорта и статусу владельца/уполномоченного. Для унификации создана новая cédula (карточка), заменяющая прежние и действующая непрерывно.",
        "Срок действия: 1 год, но без срока истечения для владельца и для должным образом уполномоченных такси, remises (частных наемных автомобилей), грузовых и пассажирских транспортных средств, когда cédula (карточка) указывает услугу.",
        "Если cédula (карточка) актуальна, любой человек с лицензией нужной категории может управлять этим транспортным средством.",
        "Если cédula (карточка) просрочена, управлять может только владелец. Нельзя управлять с cédula (карточкой), оформленной на другое лицо как уполномоченного.",
        "Документ личный, непередаваемый и не имеет срока действия для владельца.",
        "При продаже транспортного средства cédula (карточка) владельца и cédulas (карточки) уполномоченных сдаются в Registro Nacional de la Propiedad Automotor и автоматически снимаются с учета.",
        "Владелец может оформить столько cédulas (карточек) для уполномоченных, сколько пожелает."
      ]
    },
    {
      ...
      itemsRu: [
        ...
        "Результаты VTV: apto (годен) - можно ездить, выдается certificado (сертификат), informe (отчет) и oblea (наклейка/стикер); condicional (условно годен) - есть легкие дефекты, нужно исправить и пройти повторную проверку в течение 60 рабочих дней; rechazado (отклонен) - есть серьезные дефекты, ездить нельзя, повторная проверка бесплатна в течение 60 рабочих дней по предварительной записи.",
        ...
        "RVA по Закону 3708: все автомобили, зарегистрированные в CABA, включая 0 km, должны выгравировать автозапчасти в течение 30 дней после постановки на учет. После гравировки размещается защитная oblea (наклейка/стикер) на лобовом стекле и выдается certificado (сертификат)."
      ]
    },
    {
      id: "vtv-rva-visuals",
      ...
      cards: [
        {
          ...
          bodyRu:
            "Схема показывает первую VTV, renovación (продление) и связь последней цифры номера с месяцем процедуры. Правила рядом изложены русским текстом."
        },
        {
          id: "rva-source-card",
