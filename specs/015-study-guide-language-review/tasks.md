...
- Difficulty metadata refresh evidence: all three Slice K topics kept their `difficulty`, `difficultyMeta.dimensions`, `difficultyMeta.rationaleRu`, `difficultyMeta.provenance`, and `difficultyMeta.basis` unchanged. Only `difficultyMeta.sourceFingerprint` changed, and each new value matched `difficultyTopicFingerprint(topic)` after the learner-facing text rewrite:
  - `emergency-response-and-crash-scene`: `95d5a374f8444c5ebc9397bea3435d4148b63354f78b2c524a573222f3282c4b` -> `850628eab48abcd9d6b47c1ad4e44ab0a142658645af595266c0d37c8c2978b4`
  - `crash-liability-and-legal-duties`: `dd05ef41afaeb9bfe42c103490ba38dfcabf68a6128bbe895c34a45aabb40d48` -> `6ad83e9d3063f666a16cd81e86b81a3d28188a798dddb5d8b5d8c42618d4bce1`
...
- PR #92 Slice K advisory follow-up addressed: replaced the rendered misspelled English phrase `inmovilized vehicle` in `emergency-response-and-crash-scene` ticket `b-fallback-388-a1` with the Russian/Spanish learner anchor `обездвиженного автомобиля (vehículo inmovilizado)`, and replaced the mixed `solo vías interurbanas` phrase in `crash-liability-and-legal-duties` ticket `b-fallback-432-a1` with `только для vías interurbanas (междугородних дорог)`.
- PR #92 follow-up preservation guard: exact guide diff versus the prior PR #92 head contained only four expected JSON leaves: the two advisory learner strings and `difficultyMeta.sourceFingerprint` for `emergency-response-and-crash-scene` and `crash-liability-and-legal-duties`. IDs, slugs/status, placements, `questionId`, `imageLocalPath`, `answerId`, `verdict`, Spanish `termEs`, term/ticket/trap source references, claims/source traces, `difficulty`, and non-fingerprint `difficultyMeta` stayed unchanged.
- PR #92 advisory fingerprint refresh evidence: `emergency-response-and-crash-scene` changed from `90a8e9cff91ac0eedcafa70dffa773e5c144355317ce1b66bd562d505a8bd2a4` to `850628eab48abcd9d6b47c1ad4e44ab0a142658645af595266c0d37c8c2978b4`; `crash-liability-and-legal-duties` changed from `3a85fb7862ae1da8eca61e40b5ee016de339373cbd78900c55dc3102fc28eb65` to `6ad83e9d3063f666a16cd81e86b81a3d28188a798dddb5d8b5d8c42618d4bce1`; `authorities-controls-and-sanctions` stayed at `036d54a52f810edb42829da849d49323073754613060c173acb9dfe5d4a22266`.
- PR #92 advisory validation evidence:
  - JSON parse, structural count, affected-topic fingerprint, and non-fingerprint difficulty metadata guard: pass; totals remained 38 topics, 639 rendered placements, 460 unique question IDs, 1,831 answer explanations, 225 trap notes, 4 source-conflict notes, 170 claims, 38 `difficulty`, and 38 `difficultyMeta`. All 38 topic fingerprints matched `difficultyTopicFingerprint(topic)`, and only the two affected topic fingerprints changed versus the prior PR #92 head.
  - Targeted Slice K rendered-field cleanup scan: pass; scanned 217 rendered fields with 0 hits for `inmovilized`, bare `solo vías`, curated English/process leftovers, or exact duplicated `spanishTerms[].translationRu` values.
  - Guide text residue scan: pass; `rg -n "inmovilized|inmobilized|solo vías|solo vias" content/guide/topic-study-guide.ru.json` produced no output.
  - `node --test tests/content-topic-guide.test.mjs`: pass, 21/21 tests.
  - `pnpm run validate:content`: pass, `Difficulty labels validated: 460 questions, 38 topics.` and `Content validation passed: 460 category B fallback questions, 276 local image references.`
  - `git diff --check`: pass after this evidence update.
  - Changed-path guard: pass after this evidence update; `git diff --name-only` listed only `content/guide/topic-study-guide.ru.json` and `specs/015-study-guide-language-review/tasks.md`, and the forbidden/non-scope path diff produced no output.
