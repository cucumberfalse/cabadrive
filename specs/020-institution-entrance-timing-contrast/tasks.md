...
- Latest-main sync evidence after Orchestrator merge on 2026-05-10: `origin/main` is `4cc2bb735e1a92151ca3aa9001c82ae0f65c730f`, local `HEAD` is merge commit `1873d5f651f9af65692ebd3553e9e1542a5b128b`, and `git status --short --branch` returned `## codex/020-institution-entrance-timing-contrast...origin/codex/020-institution-entrance-timing-contrast [ahead 3]` with no dirty files before this evidence update.
- Latest-main diff scope after the merge: `git diff --name-only origin/main...HEAD` returned only `content/guide/topic-study-guide.ru.json`, `specs/020-institution-entrance-timing-contrast/feature-request.md`, `specs/020-institution-entrance-timing-contrast/plan.md`, `specs/020-institution-entrance-timing-contrast/spec.md`, `specs/020-institution-entrance-timing-contrast/tasks.md`, `tests/content-topic-guide.test.mjs`, and `tests/e2e/app.spec.ts`.
- Feature `021-docker-smoke-isolation` from latest main changed the local Docker contract implementation: `docker-compose.yml` no longer has fixed `container_name` or shared `image`; `Makefile up` reports `http://localhost:${CABADRIVE_HOST_PORT:-5173}`.
- Latest-main sync evidence after Orchestrator merged `origin/main` again on 2026-05-10: fetched `origin/main` is `f5fcb1602885022bb60b412f90bf290d51674c76` (`Close final 010 and 019 process gates (#97)`), local pre-merge branch head was `18e42b2ff2aaebf2cbbda96ae328e6933ef40064`, and `git merge --no-ff origin/main` completed without conflicts as merge commit `81b53de62fc11dcae51169f77b20ec018cf70e8f`.
- Latest-main post-merge status evidence before this process-memory update: `git status --short --branch` returned `## codex/020-institution-entrance-timing-contrast...origin/codex/020-institution-entrance-timing-contrast [ahead 2]` with no dirty files, and `git diff --name-only origin/main...HEAD` returned only `content/guide/topic-study-guide.ru.json`, `specs/020-institution-entrance-timing-contrast/feature-request.md`, `specs/020-institution-entrance-timing-contrast/plan.md`, `specs/020-institution-entrance-timing-contrast/spec.md`, `specs/020-institution-entrance-timing-contrast/tasks.md`, `tests/content-topic-guide.test.mjs`, and `tests/e2e/app.spec.ts`.
- Latest-main sync evidence after Orchestrator requested another merge on 2026-05-10: fetched `origin/main` is `55d196a8762fc156520f045a48c0cb25ee83d569` (`Close feature 021 review gates (#99)`), local pre-merge branch head was `80bea7ba1ac0d8f22ee301b63fb6e5c7771f1b04`, and `git merge --no-ff origin/main` completed without conflicts as merge commit `946538126d745cb042f522d97b4ed229b0f1bc5f`.
- Latest-main post-merge status evidence before this process-memory update: `git status --short --branch` returned `## codex/020-institution-entrance-timing-contrast...origin/codex/020-institution-entrance-timing-contrast [ahead 2]` with no dirty files, and `git diff --name-only origin/main...HEAD` returned only `content/guide/topic-study-guide.ru.json`, `specs/020-institution-entrance-timing-contrast/feature-request.md`, `specs/020-institution-entrance-timing-contrast/plan.md`, `specs/020-institution-entrance-timing-contrast/spec.md`, `specs/020-institution-entrance-timing-contrast/tasks.md`, `tests/content-topic-guide.test.mjs`, and `tests/e2e/app.spec.ts`.
...
- Earlier official `make up` Docker smoke was blocked by the fixed Compose `container_name: cabadrive`: Docker reported that `/cabadrive` was already in use by container `0d6fc7e5f782eec79a308c7acb95c2f14bf79a03e8b8c53fe50cb38dfc049c3b`. Read-only `docker inspect` showed that container belongs to sibling project `cabadrive-main-final-validation` with working dir `/Users/chap/devel/cabadrive-main-final-validation`, so this implementation did not stop, remove, or rename it. This dead end is now historical after merging latest main with feature `021-docker-smoke-isolation`; see latest Docker evidence below.
...
- The prior `make up` fixed-container-name blocker is resolved on the merged branch by feature `021-docker-smoke-isolation`; official Docker contract smoke now passes at the Makefile-reported `http://localhost:5173`.
...
- Latest-main targeted content check after Orchestrator merged `origin/main` `4cc2bb735e1a92151ca3aa9001c82ae0f65c730f`: `node --test tests/content-topic-guide.test.mjs` passed 22 tests, including the rendered-field institution timing contrast assertion.
- Latest-main content validation after the merge: `pnpm run validate:content` passed; difficulty labels validated for 460 questions and 38 topics, and content validation passed for 460 category B fallback questions and 276 local image references.
- Latest-main full preflight after the merge: `pnpm run preflight` passed on `HEAD` `1873d5f651f9af65692ebd3553e9e1542a5b128b`; it ran the feature-memory gate (`No configured product paths changed; feature-memory gate passes.`), repository baseline check, content validation, 120 Node tests, build, and 34 Playwright tests. The merged Node suite now includes feature `021` Docker isolation tests, including project-scoped containers, configurable host port, no shared local image tag requirement, and Docker runtime docs coverage.
- Latest-main Docker contract evidence after feature `021` merge: `make build` passed and built the project-scoped image `cabadrive-020-institution-entrance-timing-contrast-cabadrive`, including content validation and production build inside Docker.
- Latest-main official Docker smoke after feature `021` merge: `make up` passed using the project-scoped container `cabadrive-020-institution-entrance-timing-contrast-cabadrive-1` and reported `Cabadrive is available at http://localhost:5173`; `curl -fsS http://localhost:5173/` returned the Cabadrive HTML shell with `<title>Cabadrive</title>`; `make down` stopped and removed the project-scoped container and network. No alternate temporary-container workaround was used.
- Latest-main targeted content check after merging `origin/main` `f5fcb1602885022bb60b412f90bf290d51674c76`: `node --test tests/content-topic-guide.test.mjs` passed 22 tests, including the rendered-field institution timing contrast assertion.
- Latest-main feature-memory guard after the `f5fcb1602885022bb60b412f90bf290d51674c76` merge: `node scripts/check-feature-memory.mjs --worktree` passed with `No configured product paths changed; feature-memory gate passes.`
- Latest-main content validation after the `f5fcb1602885022bb60b412f90bf290d51674c76` merge: `pnpm run validate:content` passed; difficulty labels validated for 460 questions and 38 topics, and content validation passed for 460 category B fallback questions and 276 local image references.
- Latest-main full preflight after the `f5fcb1602885022bb60b412f90bf290d51674c76` merge: `pnpm run preflight` passed on `HEAD` `81b53de62fc11dcae51169f77b20ec018cf70e8f`; it ran the feature-memory gate, repository baseline check, content validation, 120 Node tests, build, and 34 Playwright tests.
- Latest-main Docker contract after the `f5fcb1602885022bb60b412f90bf290d51674c76` merge: `make build` passed and built project-scoped image `cabadrive-020-institution-entrance-timing-contrast-cabadrive`; `make up` passed and reported `Cabadrive is available at http://localhost:5173`; `curl -fsS http://localhost:5173/` returned the Cabadrive HTML shell with `<title>Cabadrive</title>`; `make down` stopped and removed the project-scoped container and network. No blocker and no alternate temp-container workaround.
- Latest-main diff check after the merge and before this process-memory update: `git diff --check` passed.
- Latest-main targeted content check after merging `origin/main` `55d196a8762fc156520f045a48c0cb25ee83d569`: `node --test tests/content-topic-guide.test.mjs` passed 22 tests, including the rendered-field institution timing contrast assertion.
- Latest-main feature-memory guard after the `55d196a8762fc156520f045a48c0cb25ee83d569` merge: `node scripts/check-feature-memory.mjs --worktree` passed with `No configured product paths changed; feature-memory gate passes.`
- Latest-main content validation after the `55d196a8762fc156520f045a48c0cb25ee83d569` merge: `pnpm run validate:content` passed; difficulty labels validated for 460 questions and 38 topics, and content validation passed for 460 category B fallback questions and 276 local image references.
- Latest-main full preflight after the `55d196a8762fc156520f045a48c0cb25ee83d569` merge: `pnpm run preflight` passed on `HEAD` `946538126d745cb042f522d97b4ed229b0f1bc5f`; it ran the feature-memory gate, repository baseline check, content validation, 120 Node tests, build, and 34 Playwright tests.
- Latest-main Docker contract after the `55d196a8762fc156520f045a48c0cb25ee83d569` merge: `make build` passed and built project-scoped image `cabadrive-020-institution-entrance-timing-contrast-cabadrive`; `make up` passed and reported `Cabadrive is available at http://localhost:5173`; `curl -fsS http://localhost:5173/` returned the Cabadrive HTML shell with `<title>Cabadrive</title>`; `make down` stopped and removed the project-scoped container and network. No blocker and no alternate temp-container workaround.
- Latest-main diff check after the `55d196a8762fc156520f045a48c0cb25ee83d569` merge and before this process-memory update: `git diff --check` passed.
...
- Latest-main chronology guard after the Orchestrator merge: the review/check evidence above applies to older head `2bbc7b35f9b4cb2982ea3804ebedd0fc1bde52b0`, not to merged local head `1873d5f651f9af65692ebd3553e9e1542a5b128b` or any future head that includes this evidence update. After Orchestrator commits and pushes this final task-memory evidence, Orchestrator must obtain fresh latest-head review/check/mergeability evidence, including latest-head diff scope, required checks, review status, and mergeability, before merge.
- Latest-main chronology guard after the `f5fcb1602885022bb60b412f90bf290d51674c76` merge: prior review/check evidence applied to older head `18e42b2ff2aaebf2cbbda96ae328e6933ef40064`, not to merged local head `81b53de62fc11dcae51169f77b20ec018cf70e8f` or any future head containing this process-memory update. After this evidence update is committed and pushed, Orchestrator must obtain fresh latest-head review/check/mergeability evidence, including latest-head diff scope, required checks, Review Agent status, GitHub bot status, and mergeability, before merge.
- Latest-main chronology guard after the `55d196a8762fc156520f045a48c0cb25ee83d569` merge: prior review/check evidence applied to older head `80bea7ba1ac0d8f22ee301b63fb6e5c7771f1b04`, not to merged local head `946538126d745cb042f522d97b4ed229b0f1bc5f` or any future head containing this process-memory update. After this evidence update is committed and pushed, Orchestrator must obtain fresh latest-head review/check/mergeability evidence, including latest-head diff scope, required checks, Review Agent status, GitHub bot status, and mergeability, before merge.
...
- Local workflow feedback before latest-main merge: the repository's fixed Docker `container_name: cabadrive` made parallel worktree `make up` smoke fragile because one sibling container blocked all others from using the official port/name. This implementation preserved sibling work and used an alternate temporary smoke container only after recording the official `make up` blocker. After the feature `021-docker-smoke-isolation` merge from `origin/main`, this blocker no longer reproduces in this worktree's official Docker flow.
...
- Docker workflow disposition before latest-main merge: the fixed Compose `container_name: cabadrive` was a real parallel-worktree smoke fragility and was outside feature 020 product scope. After latest main brought in feature `021-docker-smoke-isolation`, the official feature-020 Docker contract passed without fallback, so no feature-020 Docker follow-up remains.
