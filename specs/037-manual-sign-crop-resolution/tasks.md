...
- [x] Run `node --test tests/manual-sign-inventory.test.mjs` and record the outcome. Passed after final crop generation: `9/9` focused manual sign inventory tests.
- [x] Run `pnpm run test` and record the outcome. Passed: `443/443` tests.
...
- [x] Run Playwright/visual QA for all six sign sections and record the command, server URL, and evidence paths. `node scripts/manual-sign-visual-qa.mjs` wrote all contact sheets; `node scripts/manual-sign-visual-qa.mjs --runtime-url http://localhost:5174/` wrote all desktop/mobile runtime screenshots and `visual-qa-summary.json`. The Vite dev command used for runtime QA was `pnpm run dev -- --port 5178`; because the app script did not forward that argument, Vite served at `http://localhost:5174/`, which was used for QA.
...
- [x] Run `pnpm run preflight` unless a concrete blocker is recorded with fallback evidence. Passed: feature-memory gate, repo baseline check, content validation, `443/443` tests, build, and `82/82` Playwright e2e tests.
...
- Final command outcomes:
  ...
  - `swiftc -parse scripts/manual-sign-crop-resolution.swift`: passed.
  ...
  - `node --test tests/manual-sign-inventory.test.mjs`: passed, `9/9`.
  - `node --check scripts/manual-sign-visual-qa.mjs`: passed.
  - `node scripts/manual-sign-visual-qa.mjs`: passed, wrote contact sheets.
  - `pnpm run dev -- --port 5178`: started Vite; Vite served at `http://localhost:5174/` because the script did not forward the requested port.
  - `node scripts/manual-sign-visual-qa.mjs --runtime-url http://localhost:5174/`: passed, wrote desktop/mobile screenshots and visual QA summary.
  ...
  - `pnpm run validate:manual-sign-inventory`: passed.
  - `pnpm run test`: passed, `443/443`.
  - `pnpm run build`: passed; existing large-chunk warnings only; generated service worker with `2156` cached assets.
  - `pnpm run preflight`: passed, including feature-memory gate, repo baseline check, content validation, `443/443` tests, build, and `82/82` Playwright e2e tests.
- Final dead ends/implementation notes: initial direct `sourceRegion + cropRegion` PDF scaling produced blank/sliver/wrong crops and was rejected; whole-sheet mapping without row/column constraints admitted neighbors on some rows and was corrected; initial generic tail trimming could remove attached plates, so final tooling uses row-specific tail trim modes where needed; a Swift `UInt8` overflow in edge/content scanning was fixed before final crop output; the Vite dev server ignored the forwarded `--port 5178` argument and served on `5174`, which is recorded as the runtime QA URL.
