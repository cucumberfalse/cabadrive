...
| `ch1-sustainable-mobility` | Implement website section page `¿Qué es la movilidad sustentable?` / `Что такое устойчивая мобильность?` with source page `23` infographic and vulnerability-order evidence | `codex/030-ch1-sustainable-mobility` | PR pending | local head pending commit/push | implemented locally; focused source-fidelity/content/tsc/build/Playwright/diff, full test, full preflight, and Docker smoke passed; PR pending | yes |
