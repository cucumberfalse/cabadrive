...
- PR #177 current-head source-order conflict at `b1117ddee0e71a72ce4d8416c39b406161cf3eb2`: Architect disposition: addressed; current placement of the principles visual after connectivity follows PDF text extraction rather than the source crop, so Implementation Agent must restore source-crop order and final Architect/Analyst validation must rerun after the non-evidence fix.
- PR #177 current-head source-order conflict `PRRT_kwDOSX65IM6F2v9j` / `discussion_r3328424446`: fixed locally by restoring source-crop order; final Architect/Analyst validation remains stale and must rerun on the post-fix head after review/check gates settle.
