...
- Implementation decision: feature `030-manual-chapters-1-2` remains a legacy visual-evidence baseline and is not silently rewritten in this prerequisite slice. The stricter checker path applies to future non-legacy manual registries and to any implemented section that opts into `visualEvidenceSchemaVersion: 3` / `visualRulePolicyId: 031-strict-source-fidelity`.
