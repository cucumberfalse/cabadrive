...
- Protected visual confirmation: no Appendix I photo, traffic sign, road-marking visual, source crop, validation crop, or source page image was translated, relabeled, recolored, retouched, masked, inpainted, reconstructed, or redrawn. The only Appendix I Spanish cleanup is the headrest/SRI transferred-infographic glyph cleanup from source crops; the mirror photo remains source-as-is.
- Final handoff evidence: `git diff --cached --check` is run before each commit, and the final pushed PR head is reported by Implementation Agent handoff.
