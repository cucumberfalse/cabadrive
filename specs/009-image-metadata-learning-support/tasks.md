...
- [ ] T098 If runtime-affecting changes exist, run `make down`, `make build`, `make up`, HTTP/browser smoke test against `http://localhost:5173`, and `make down`. Follow-up branch `codex/019-feature-009-docker-smoke-closure` attempted the exact local Docker smoke on 2026-05-10 from head `90a11d943880606586d4bc02aa7774a8d7a73f3d`: `make down` passed, `make build` passed and built `cabadrive:local` image `sha256:7a0ca7ec04fddd97a74c03c25ad03e1bfb680a90953a01df336479bba9fec7d4`, but `make up` failed before starting this branch's container because Docker already had `/cabadrive` running from unrelated worktree `/Users/chap/devel/cabadrive-main-final-validation` under Compose project `cabadrive-main-final-validation`. HTTP `curl -I --max-time 10 http://localhost:5173` returned `HTTP/1.1 200 OK`, but that response came from the pre-existing container image `sha256:32c0b98f11b73980db69c58bb08d0f465051982662699aeb4b8146b246ea9ccf`, not from this branch's `make up`; final `make down` removed only this worktree's failed Compose network. T098 therefore remains open and is not honestly closed by this PR.
...
- Feature 019 T098 docker-smoke follow-up on 2026-05-10 in `/Users/chap/devel/cabadrive-019-feature-009-docker-smoke-closure` on `codex/019-feature-009-docker-smoke-closure`: `git fetch origin --prune` left local head and `origin/main` both at `90a11d943880606586d4bc02aa7774a8d7a73f3d`; `make down` passed; `make build` passed and wrote `cabadrive:local` image `sha256:7a0ca7ec04fddd97a74c03c25ad03e1bfb680a90953a01df336479bba9fec7d4`; `make up` failed with `Error response from daemon: Conflict. The container name "/cabadrive" is already in use by container "0d6fc7e5f782eec79a308c7acb95c2f14bf79a03e8b8c53fe50cb38dfc049c3b"`. `docker inspect` showed that container is running from unrelated worktree `/Users/chap/devel/cabadrive-main-final-validation`, Compose project `cabadrive-main-final-validation`, service `cabadrive`, image `cabadrive:local` container image ID `sha256:32c0b98f11b73980db69c58bb08d0f465051982662699aeb4b8146b246ea9ccf`. `curl -I --max-time 10 http://localhost:5173` returned `HTTP/1.1 200 OK`, and `curl --max-time 10 -sS http://localhost:5173` returned the Cabadrive HTML, but this was served by the pre-existing unrelated container rather than this branch's failed `make up`. Final `make down` passed and removed this worktree's failed Compose network. Because parallel-agent instructions forbid deleting unrelated containers manually, T098 remains open rather than being falsely marked closed.
