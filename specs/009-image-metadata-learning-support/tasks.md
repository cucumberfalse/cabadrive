...
- [x] T098 If runtime-affecting changes exist, run `make down`, `make build`, `make up`, HTTP/browser smoke test against `http://localhost:5173`, and `make down`. Follow-up branch `codex/019-feature-009-docker-smoke-closure` completed the exact local Docker smoke on 2026-05-10 after the user authorized stopping stale blocker container `0d6fc7e5f782` from `/Users/chap/devel/cabadrive-main-final-validation` via `make down`. In `/Users/chap/devel/cabadrive-019-feature-009-docker-smoke-closure` on head `d28855ff4ed18f2b2cd3a44b12431abdeba58ca1`, `make down` passed, `make build` passed and wrote `cabadrive:local` image `sha256:dd197090ceb3ee03b4334c3c087f3c408360e6f7f78fa0166e6f365553d49d1b`, `make up` started container `55e3cb780af183fb338565788021b31269cde0a9d8c251eb70f02b904b909546` with Compose project `cabadrive-019-feature-009-docker-smoke-closure`, working dir `/Users/chap/devel/cabadrive-019-feature-009-docker-smoke-closure`, service `cabadrive`, image ID `sha256:dd197090ceb3ee03b4334c3c087f3c408360e6f7f78fa0166e6f365553d49d1b`, and host port `5173->8080`. `curl -I --max-time 10 http://localhost:5173` returned `HTTP/1.1 200 OK`; `curl -fsS --max-time 10 http://localhost:5173/ | head` returned Cabadrive HTML with `<html lang="ru">` and `<title>Cabadrive</title>`; final `make down` stopped and removed container `cabadrive` and network `cabadrive-019-feature-009-docker-smoke-closure_default`. Earlier blocker evidence from the stale unrelated container is superseded by this successful branch-owned smoke.
...
- Feature 019 T098 docker-smoke follow-up on 2026-05-10 in `/Users/chap/devel/cabadrive-019-feature-009-docker-smoke-closure` on `codex/019-feature-009-docker-smoke-closure`: the earlier failed attempt from head `90a11d943880606586d4bc02aa7774a8d7a73f3d` was superseded after the user authorized stopping stale blocker container `0d6fc7e5f782` from `/Users/chap/devel/cabadrive-main-final-validation` via `make down`. Before cleanup, `docker inspect 0d6fc7e5f782` showed detached worktree head `2af08b0b918fadb14504ae63a7b2850070906992`, Compose project `cabadrive-main-final-validation`, working dir `/Users/chap/devel/cabadrive-main-final-validation`, service `cabadrive`, image ID `sha256:32c0b98f11b73980db69c58bb08d0f465051982662699aeb4b8146b246ea9ccf`, and config `/Users/chap/devel/cabadrive-main-final-validation/docker-compose.yml`; `make down` there stopped and removed only container `cabadrive` and network `cabadrive-main-final-validation_default`. Back in `/Users/chap/devel/cabadrive-019-feature-009-docker-smoke-closure`, `git fetch origin --prune` confirmed local head and upstream both at `d28855ff4ed18f2b2cd3a44b12431abdeba58ca1`; `make down` passed; `make build` passed and wrote `cabadrive:local` image `sha256:dd197090ceb3ee03b4334c3c087f3c408360e6f7f78fa0166e6f365553d49d1b`; `make up` started container `55e3cb780af183fb338565788021b31269cde0a9d8c251eb70f02b904b909546` from this worktree with Compose project `cabadrive-019-feature-009-docker-smoke-closure`, service `cabadrive`, image ID `sha256:dd197090ceb3ee03b4334c3c087f3c408360e6f7f78fa0166e6f365553d49d1b`, and host port `5173->8080`; `curl -I --max-time 10 http://localhost:5173` returned `HTTP/1.1 200 OK`; `curl -fsS --max-time 10 http://localhost:5173/ | head` returned Cabadrive HTML with `<html lang="ru">` and `<title>Cabadrive</title>`; final `make down` stopped and removed this branch's container and network. This closes T098 with branch-owned runtime smoke evidence.
