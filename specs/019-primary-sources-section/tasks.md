...
- PR #106 Article 25 and Article 58 review follow-up on 2026-05-11:
  - Created fresh isolated worktree `/Users/chap/devel/cabadrive-019-primary-sources-content-batch-ley24449-reviewfix3` from remote head `1f032d354541e3c491c3426501c667972bfea583`, without touching the signage/#117/#107/D9 worktrees or the existing `reviewfix2` worktree.
  - Fixed Article 25 `fullTranslationRu` in chunk `ley-24449-transito-seguridad-vial--articulo-25-restricciones-al-dominio-es-obligatorio-para-036`: Spanish `RESTRICCIONES AL DOMINIO` now renders as restrictions on property ownership/right (`ОГРАНИЧЕНИЯ ПРАВА СОБСТВЕННОСТИ`), not registration-number restrictions.
  - Fixed Article 58 `fullTranslationRu` in chunk `ley-24449-transito-seguridad-vial--articulo-58-revisores-de-carga-los-079`: Spanish `REVISORES DE CARGA` now renders as cargo inspectors (`ИНСПЕКТОРЫ ГРУЗА`), and the bank/postal valuables sentence now says duly accredited valuables transport cannot be stopped or delayed, not securities paperwork.
  - Immutable Spanish source text, chunk IDs, source spans, `sourceTextSha256`, and `sourceFingerprint` were preserved for both chunks. The Ley 24.449 search shard uses field references and the QA shard uses generic approved QA notes, so no search/QA text update was required.
  - Verification passed: focused Node check confirmed 131 chunks, preserved Article 25/58 Spanish source spans and fingerprints, corrected Article 25/58 terms, and no stale `регистрационного номера`, `НАГРУЗКА РЕЦЕНЗИРОВ`, or Article 58 `ценных бумаг` wording in those chunks; targeted `rg` found no stale phrases in the Ley 24.449 document/search/QA shards and confirmed the corrected terms.
  - Verification passed: `npm run validate:content`; `PRIMARY_SOURCES_VALIDATION_MODE=coverage npm run validate:content`; `node --test tests/primary-sources-validation.test.mjs tests/primary-sources-generate-coverage.test.mjs` with 41 tests passed; `node scripts/check-feature-memory.mjs --worktree`; `git diff --check`. Full preflight was not rerun for this text-only follow-up because the requested primary-source/content gates passed.
