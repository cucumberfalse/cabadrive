...
- PR #106 Article 40 bis and Article 43 review follow-up on 2026-05-11:
  ...
  - Verification passed: `pnpm run validate:content`; `PRIMARY_SOURCES_VALIDATION_MODE=coverage pnpm run validate:content`; `node --test tests/primary-sources-validation.test.mjs tests/primary-sources-generate-coverage.test.mjs` with 41 tests passed; `node scripts/check-feature-memory.mjs --worktree`; `git diff --check`; `pnpm run preflight`.
- PR #106 Article 44(f) two-way-road review follow-up on 2026-05-11:
  - Synced `/Users/chap/devel/cabadrive-pr106-impl` to remote head `9172a04edf2e59613a3a4df657a967261258f4d2` before editing.
  - Fixed Article 44(f) `fullTranslationRu` and aligned `simpleRu` in chunk `ley-24449-transito-seguridad-vial--articulo-44-vias-semaforizadas-en-las-vias-reguladas-062`: Spanish `En vías de doble mano` now renders as roads with two-way traffic (`дороги с двусторонним движением`), not two-lane roads, so the left-turn prohibition is not narrowed by lane count.
  - Preserved immutable Spanish source text, chunk ID, source span, `sourceTextSha256`, and `sourceFingerprint`. The Ley 24.449 search shard uses field references and the QA shard uses generic approved QA notes, so no search/QA text update was required.
  - Verification passed: focused Node QA confirmed 131 chunks, preserved Article 44 Spanish source span/fingerprint, corrected two-way-road wording in `fullTranslationRu` and `simpleRu`, search projection entry, and preservation of prior Article 40 bis/43 fixes; targeted `rg` found no stale Article 44 `двухполосных дорогах` wording in the Ley 24.449 document/search/QA shards.
  - Verification passed: `pnpm run validate:content`; `PRIMARY_SOURCES_VALIDATION_MODE=coverage pnpm run validate:content`; `node --test tests/primary-sources-validation.test.mjs tests/primary-sources-generate-coverage.test.mjs` with 41 tests passed; `node scripts/check-feature-memory.mjs --worktree`; `git diff --check`; `pnpm run preflight`.
