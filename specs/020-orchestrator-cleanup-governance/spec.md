...
- A first-class `Cleanup Agent` role with exclusive destructive local-disk cleanup execution authority, narrow permissions, forbidden actions, validation requirements, and evidence requirements.
...
3. Given durable role guidance, when completion-time cleanup is needed, then Orchestrator coordinates cleanup through an assigned Cleanup Agent and does not directly delete local repository environments or delegate deletion to an equivalent non-Cleanup role.
...
11. Given live governance, templates, and guidance are searched, when cleanup role wording is reviewed, then no live/current rule may permit equivalent non-Cleanup roles to execute destructive deletion; non-cleanup roles may only coordinate cleanup, request Cleanup Agent assignment, or record evidence.
...
9. Given any role other than an assigned Cleanup Agent is asked to perform destructive completion-time local environment cleanup, when deletion is considered, then that role must refuse execution and route the work to Orchestrator for Cleanup Agent assignment.
...
- FR-004: Durable role guidance must add `Cleanup Agent` as a first-class role and must not allow equivalent non-Cleanup roles to execute destructive cleanup deletion.
...
- FR-006: Orchestrator guidance must require cleanup coordination at completion or handoff, while preserving Orchestrator's no-direct-repository-edit and no-direct-destructive-cleanup boundary; Orchestrator and other non-cleanup roles may coordinate, request assignment, or record evidence only.
- FR-007: Cleanup validation must require positive proof of approved root, Cabadrive repository identity, agent-created status, worktree registration or documented helper metadata, inactive status, clean git status, no unpushed work, no open/unresolved PR, no locks/running processes, and no active process-memory references.
...
- FR-013: Implementation must remove the equivalent-role deletion loophole from live governance/template/guidance files, including `CLAUDE.md`, `.github/pull_request_template.md`, `docs_project/project/devops/ai-pr-workflow.md`, `docs_project/project/devops/review-contract.md`, `.specify/memory/constitution.md`, and this feature memory where current normative wording appears.
...
- SC-009: Text search and manual review show no live/current cleanup wording permits equivalent non-Cleanup role deletion, and every destructive cleanup executor path names an assigned `Cleanup Agent` only.
...
- Review requirements: The Review Agent must verify role boundaries, latest-main startup language, cleanup validation strictness, refusal conditions, one-time cleanup evidence, PR #65 coordination mitigation, scoped-file compliance, and absence of live/current equivalent-cleanup-role deletion permission.
