...
4. Given a Codex native review or summary comment targets an older head, when the gate validates a newer head, then the stale evidence is rejected, including when the summary comment timestamp is fresh enough for the no-SHA fallback.
...
2. Given a Codex review or summary comment from `chatgpt-codex-connector` is tied to a previous commit, when the PR head changes, then the old review remains stale and a fresh current-head review is required.
...
- FR-005: Codex no-findings summary acceptance must continue to require a trusted Codex login and the existing `Codex Review:` summary contract. A summary body containing one or more 7-40 hex SHA-like markers may pass only when every marker is a prefix of the current full head SHA, including 7-character, 9-character, 10-character, and full 40-character current-head markers. A summary body containing any 7-40 hex SHA-like marker that is not a current-head prefix must be rejected before timestamp fallback. A summary body with no 7-40 hex SHA-like marker may still use timestamp freshness fallback.
...
- Review requirements: Review Agent must verify strict trust boundaries, current-head rejection, stale summary SHA rejection before timestamp fallback, unknown-login rejection, association/login separation, default-branch checkout preservation, process-memory completeness, role-boundary compliance, and no unrelated product/runtime changes.
- Test/verification requirements: Required focused command is `node --test tests/ai-review-helpers.test.mjs`. The helper tests must cover old-SHA summary rejection when `headCommittedAt` is supplied and the comment timestamp is fresh, 7-character and 9-character current-head prefix acceptance, full current-SHA summary acceptance, non-matching 7-character marker rejection, unknown-login summary rejection, and fresh no-SHA summary acceptance by timestamp fallback. Also run `node --test tests/ai-review-workflow.test.mjs` if workflow/default-branch checkout assertions are touched or as static guard evidence. Run `node --test tests/finalize-pr.test.mjs` if the helper default affects finalization blocker behavior or if finalize-specific tests are added. Always run `node scripts/check-feature-memory.mjs --worktree`, `git diff --check`, and `pnpm run preflight` when feasible for a workflow/tooling change.
