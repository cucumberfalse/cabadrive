...
- FR-005: Codex no-findings summary acceptance must continue to require a trusted Codex login and the existing `Codex Review:` summary contract. A summary body with a current full-head marker or accepted current short-head marker may pass. A summary body with no 7-40 hex SHA-like marker may use timestamp freshness fallback. A summary body containing any 7-40 hex SHA-like marker that does not match the current head full SHA or accepted short head must be rejected instead of falling through to timestamp freshness.
...
- Test/verification requirements: Required focused command is `node --test tests/ai-review-helpers.test.mjs`. The helper tests must cover old-SHA summary rejection when `headCommittedAt` is supplied and the comment timestamp is fresh, current-SHA summary acceptance, unknown-login summary rejection, and fresh no-SHA summary acceptance by timestamp fallback. Also run `node --test tests/ai-review-workflow.test.mjs` if workflow/default-branch checkout assertions are touched or as static guard evidence. Run `node --test tests/finalize-pr.test.mjs` if the helper default affects finalization blocker behavior or if finalize-specific tests are added. Always run `node scripts/check-feature-memory.mjs --worktree`, `git diff --check`, and `pnpm run preflight` when feasible for a workflow/tooling change.
