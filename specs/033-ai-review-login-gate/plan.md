...
4. Tighten Codex summary SHA handling so a summary body containing one or more 7-40 hex SHA-like markers returns true only when every marker is a prefix of the current full head SHA. Current 7-character, 9-character, 10-character, and full 40-character markers must be accepted; any non-current 7-40 marker must return false before timestamp fallback. Timestamp freshness remains valid only for otherwise acceptable no-SHA Codex summaries.
...
| AC-004 | Helper tests prove stale native reviews and stale summary comments remain rejected for changed heads, including an old-SHA summary with `headCommittedAt` supplied and a fresh comment timestamp. Summary-marker tests must also prove 7-character and 9-character current-head prefixes are accepted, while a non-matching 7-character marker is rejected before timestamp fallback. |
