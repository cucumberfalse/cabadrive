...
const ticketTopicAssignments = readJson(
...
test("reviewed manual placement covers all tickets through sealed curated topic routes", () => {
  ...
  assert.ok(
    result.summary.ownerApprovedThematicFallbacks.some(
      ({ questionId }) => questionId === "b-fallback-011",
    ),
  );
  assert.ok(
    result.summary.ownerApprovedThematicFallbacks.some(
      ({ questionId }) => questionId === "b-fallback-126",
    ),
  );
  assert.ok(
    result.summary.ownerApprovedThematicFallbacks.some(
      ({ questionId }) => questionId === "b-fallback-235",
    ),
  );
});
...
test("candidate ranking has no approval authority or committed write fields", () => {
  ...
  assert.doesNotMatch(
    serialized,
    /placementBasis|answerBasisRu|topicRouteId|reviewedBy|reviewedAt/u,
  );
  ...
  assert.doesNotMatch(
    generatorSource,
    /writeJson\(topicRoutesPath|writeJson\(ticketTopicAssignmentsPath/u,
  );
...
test("validator rejects missing, stale, or synthetic reviewed-manifest evidence", () => {
  assert.ok(
    validate(records, undefined, null).errors.some((error) =>
      error.includes("Reviewed manifest is missing"),
    ),
  );
  ...
  assert.ok(
    validate(records, undefined, staleManifest).errors.some((error) =>
      error.includes("reviewed source differs"),
    ),
  );
  ...
  assert.ok(
    validate(records, undefined, staleRouteManifest).errors.some((error) =>
      error.includes("topic-routing source differs"),
    ),
  );
  ...
  assert.ok(
    validate(records, undefined, syntheticManifest).errors.some((error) =>
      error.includes("reserved or synthetic reviewer"),
    ),
  );
});
...
test("validator rejects strict records without direct-answer evidence, synthetic reviewers, and stale exact anchors", () => {
  ...
  assert.ok(
    validate(unsupportedStrict).errors.some((error) =>
      error.includes("strict placement lacks direct-answer evidence"),
    ),
  );
  ...
  assert.ok(
    validate(syntheticReviewer).errors.some((error) =>
      error.includes("reserved or synthetic reviewer identity"),
    ),
  );
  ...
  assert.ok(
    validate(staleAnchor).errors.some((error) =>
      error.includes("exact reviewed anchor text is stale"),
    ),
  );
});
...
test("validator rejects malformed fallbacks and support-page destinations", () => {
  ...
  assert.ok(
    validate(missingAudit).errors.some((error) =>
      error.includes("outside its reviewed curated route"),
    ),
  );
  ...
  assert.ok(
    validate(supportPage).errors.some((error) =>
      error.includes("ineligible destination front-glossary"),
    ),
  );
});
...
test("validator requires complete ticket-specific fallback ledgers", () => {
  ...
  const missingConceptsFallback = missingConcepts.find(
    (record) => record.questionId === "b-fallback-126",
  ).placements[0].fallbackEvidence;
  delete missingConceptsFallback.searchedConcepts;
  assert.ok(
    validate(missingConcepts).errors.some((error) =>
      error.includes("two distinct searched concepts"),
    ),
  );

  const duplicatedConcepts = structuredClone(records);
  const duplicatedFallback = duplicatedConcepts.find(
    (record) => record.questionId === "b-fallback-126",
  ).placements[0].fallbackEvidence;
  duplicatedFallback.searchedConcepts = [
    duplicatedFallback.searchedConcepts[0],
    duplicatedFallback.searchedConcepts[0],
  ];
  assert.ok(
    validate(duplicatedConcepts).errors.some((error) =>
      error.includes("two distinct searched concepts"),
    ),
  );

  const noRejectedCandidate = structuredClone(records);
  const noRejectedFallback = noRejectedCandidate.find(
    (record) => record.questionId === "b-fallback-126",
  ).placements[0].fallbackEvidence;
  noRejectedFallback.candidatesReviewed = noRejectedFallback.candidatesReviewed.filter(
    (candidate) => candidate.outcome !== "rejected",
  );
  assert.ok(
    validate(noRejectedCandidate).errors.some((error) =>
      error.includes("one selected and at least one rejected"),
    ),
  );
  ...
  assert.ok(
    validate(staleCandidate).errors.some((error) => error.includes("stale exact-anchor evidence")),
  );

  const mismatchedSelected = structuredClone(records);
  mismatchedSelected.find(
    (record) => record.questionId === "b-fallback-126",
  ).placements[0].fallbackEvidence.candidatesReviewed[0].pageId = "ch3-speed";
  assert.ok(
    validate(mismatchedSelected).errors.some((error) =>
      error.includes("differs from the committed placement"),
    ),
  );

  const genericConclusion = structuredClone(records);
  genericConclusion.find(
    (record) => record.questionId === "b-fallback-126",
...
test("validator rejects fallback pages or anchors outside the reviewed route", () => {
  ...
  assert.ok(
    validate(wrongPage).errors.some((error) =>
      error.includes("outside its reviewed curated route"),
    ),
  );
  ...
  assert.ok(
    validate(wrongAnchor).errors.some((error) =>
      error.includes("outside its reviewed curated route"),
    ),
  );

  const missingAssignment = structuredClone(ticketTopicAssignments);
  missingAssignment.entries = missingAssignment.entries.filter(
    (entry) => entry.questionId !== "b-fallback-431",
  );
  assert.ok(
    validate(records, undefined, reviewedManifest, topicRoutes, missingAssignment).errors.some(
      (error) =>
        error.includes("missing reviewed topic-route assignment") ||
        error.includes("assignment count is stale"),
    ),
  );
});
...
test("fresh and prior review regression tickets use conservative classifications and closest curated routes", () => {
  const expected = {
    ...
    "b-fallback-037": [
      ...
      "ch1-bicycle",
      "owner-approved-thematic-fallback",
    ],
  ...
  const assignmentById = new Map(
    ticketTopicAssignments.entries.map((entry) => [entry.questionId, entry]),
  );
...
test("known false mappings 003, 011, and 042 cannot return", () => {
  ...
  assert.ok(
    validate(restored).errors.some((error) =>
      error.includes("known false mapping fixture was restored"),
    ),
  );
});
...
test("tickets 042 and 126 preserve their exact approved anchors and comparison ledgers", () => {
  ...
  assert.ok(
    record126.placements[0].fallbackEvidence.candidatesReviewed.some(
      (candidate) =>
  ...
  const placement042 = alternate042.find((record) => record.questionId === "b-fallback-042")
    .placements[0];
  placement042.pageId = "ch1-public-transport-system";
  assert.ok(
    validate(alternate042).errors.some((error) => error.includes("bus-terminal invariant")),
  );

  const alternate126 = structuredClone(records);
  const placement126 = alternate126.find((record) => record.questionId === "b-fallback-126")
    .placements[0];
...
test("F038-RA-004 rejects undisposed overlaps and seals lexical plus semantic review outcomes", () => {
  ...
  for (const questionId of ["b-fallback-001", "b-fallback-065", "b-fallback-086"]) {
    ...
    assert.ok(
      validate(fixture).errors.some((error) =>
        error.includes("exact self-sufficient answer-bearing candidate remains rejected"),
      ),
    );
  }

  const fallback026 = records.find((record) => record.questionId === "b-fallback-026");
  const candidate026 = fallback026.placements[0].fallbackEvidence.candidatesReviewed.find(
    (candidate) => candidate.outcome === "rejected",
  );
  const question026 = questions.find((question) => question.id === "b-fallback-026");
  const translation026 = translations.find(
    (translation) => translation.questionId === "b-fallback-026",
  );
  assert.ok(
    detectRejectedCandidateAnswerOverlap(candidate026, question026, translation026).length > 0,
  );
  ...
  const candidate202 = fallback202.placements[0].fallbackEvidence.candidatesReviewed.find(
    (candidate) => candidate.outcome === "rejected",
  );
  ...
  const undisposedCandidate = undisposed
    .find((record) => record.questionId === "b-fallback-026")
    .placements[0].fallbackEvidence.candidatesReviewed.find(
      (candidate) => candidate.outcome === "rejected",
    );
  delete undisposedCandidate.answerOverlapDisposition;
  assert.ok(
    validate(undisposed).errors.some((error) =>
      error.includes("lacks a reviewed not-self-sufficient disposition"),
    ),
  );
  ...
  assert.ok(
    validate(records, staleEvidence).errors.some((error) =>
      error.includes("contradiction-audit evidence is stale"),
    ),
  );
});
...
test("F038-RA-005 seals exact answer-bearing anchors for tickets 390, 422, and 430", () => {
  ...
  for (const [questionId, wrongAnchor] of [
    [
      "b-fallback-390",
      records.find((entry) => entry.questionId === "b-fallback-349").placements[0].anchor,
    ],
    [
      "b-fallback-422",
      {
    ...
    const fixture = structuredClone(records);
    fixture.find((entry) => entry.questionId === questionId).placements[0].anchor =
      structuredClone(wrongAnchor);
    assert.ok(
      validate(fixture).errors.some((error) =>
...
test("validator rejects stale, reordered, duplicate, incomplete, and governance-heavy runtime projections", () => {
  ...
  assert.ok(
    validate(
      ...
      missing,
    ).errors.some((error) => error.includes("runtime projection is stale")),
  );
  ...
  assert.ok(
    validate(
      ...
      ticketTopicAssignments,
      duplicate,
    ).errors.some((error) => error.includes("sorted and duplicate-free")),
  );
  ...
  assert.ok(
    validate(
      ...
      ticketTopicAssignments,
      extra,
    ).errors.some((error) => error.includes("non-allowlisted fields")),
  );
});
...
test("runtime source appends tickets after existing page flows and keeps canonical joins", () => {
  ...
  assert.match(
    appSource,
    /<ManualTicketAppendix key="intro-road-pandemic" pageId="intro-road-pandemic" \/>/u,
  );
  assert.match(appSource, /<ManualTicketAppendix key=\{section\.id\} pageId=\{section\.id\} \/>/u);
  assert.match(
    appSource,
    /<ManualTicketAppendix key=\{content\.sectionId\} pageId=\{content\.sectionId\} \/>/u,
  );
  assert.match(appSource, /expanded && <div className="manual-ticket-list">/u);
  assert.match(
    runtimeSource,
    /questionIds\.sort\(\(left, right\) => left\.localeCompare\(right\)\)/u,
  );
...
test("source-conflict notes derive from topic guide and reject divergent duplicate notes", () => {
  ...
  duplicatedIdentical[0].tickets.push(
    structuredClone(
      guide.topics
        .flatMap((topic) => topic.tickets)
        .find((ticket) => ticket.questionId === "b-fallback-024"),
    ),
  );
  assert.equal(
...
test("manual source contract passes note-only prop while Materials keeps topicTicket", () => {
  ...
  assert.match(
    appSource,
    /function buildSourceConflictNoteByQuestionId\(topics: TopicGuideTopic\[\]\)/u,
  );
  assert.match(
    appSource,
  ...
  assert.match(
    appSource,
    /throw new Error\(`Divergent sourceConflictNoteRu for ticket \$\{ticket\.questionId\}`\);/u,
  );
  assert.match(
    appSource,
