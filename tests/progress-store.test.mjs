...
const compiled = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022, moduleResolution: ts.ModuleResolutionKind.Bundler, isolatedModules: true }, fileName: "src/progressStoreCore.ts" }).outputText;
const core = await import(`data:text/javascript;base64,${Buffer.from(compiled).toString("base64")}`);
const { ANSWER_LIMIT, PROGRESS_BACKUP_KEY, PROGRESS_KEY, createProgressStore, emptyProgress, mistakesFromProgress, parseImportedProgress, pruneToLimit } = core;

class FakeStorage {
  constructor(entries = {}, failures = []) { this.entries = new Map(Object.entries(entries)); this.failures = [...failures]; this.writes = []; }
...
const answer = (index, correct = index % 3 !== 0) => ({ questionId: `q-${index % 7}`, selectedAnswerId: `a-${index}`, isCorrect: correct, answeredAt: `2026-01-01T00:00:${String(index % 60).padStart(2, "0")}.000Z`, mode: "learning" });
const attempt = { id: "exam-1", finishedAt: "2026-01-01T00:00:00.000Z", score: 85, passed: true, total: 40 };
const v2 = (overrides = {}) => ({ version: 2, answers: [answer(1)], difficultQuestionIds: ["q-1"], examAttempts: [attempt], prunedAnswerStats: [], ...overrides });

test("migrates production-shaped v1, backs up exact raw payload before v2 overwrite and reloads idempotently", () => {
  const raw = JSON.stringify({ answers: [answer(1)], difficultQuestionIds: ["q-1"], examAttempts: [attempt] });
...
test("does not overwrite v1 primary when exact backup write fails", () => {
  ...
  const storage = new FakeStorage({ [PROGRESS_KEY]: raw }, [new Error("blocked"), new Error("blocked again")]);
  const events = [];
  const store = createProgressStore(storage);
...
test("local corruption salvages retained suffix while corrupt aggregate contributes no invented mistakes", () => {
  const raw = JSON.stringify(v2({ answers: [answer(3, false)], prunedAnswerStats: [{ questionId: "q-9", wrong: 1, lastPrunedAnswer: answer(8, false), firstSeenOrder: 0 }, { questionId: "q-9", wrong: 1, lastPrunedAnswer: answer(9, false), firstSeenOrder: 1 }] }));
  const store = createProgressStore(new FakeStorage({ [PROGRESS_KEY]: raw }));
  assert.deepEqual(store.getSnapshot().prunedAnswerStats, []);
  assert.deepEqual(mistakesFromProgress(store.getSnapshot()).map(({ questionId, wrong }) => ({ questionId, wrong })), [{ questionId: "q-3", wrong: 1 }]);
});

test("unknown local versions fail closed instead of being mistaken for legacy v1", () => {
  const storage = new FakeStorage({ [PROGRESS_KEY]: JSON.stringify({ version: 99, answers: [answer(1, false)] }) });
  const store = createProgressStore(storage);
...
test("pruning six thousand answers preserves uncapped mistake selector including pruned-only errors and stable ties", () => {
  ...
  const full = new Map();
  for (const item of history) { const current = full.get(item.questionId) ?? { wrong: 0 }; current.wrong += Number(!item.isCorrect); current.last = item; full.set(item.questionId, current); }
  const expected = [...full.entries()].filter(([, item]) => item.wrong).sort((a, b) => b[1].wrong - a[1].wrong).map(([questionId, item]) => ({ questionId, wrong: item.wrong, last: item.last }));
...
test("repeated pruning and quota retries fold disjoint prefixes exactly once", () => {
  ...
  assert.deepEqual(mistakesFromProgress(twice), mistakesFromProgress(pruneToLimit({ ...emptyProgress(), answers: history }, 4)));
  ...
  assert.equal(mistakesFromProgress(store.getSnapshot()).reduce((sum, item) => sum + item.wrong, 0), history.filter((item) => !item.isCorrect).length);
});
...
test("quota exhaustion and ordinary writes retain a usable memory snapshot with typed recovery", () => {
  ...
  const store = createProgressStore(storage); const events = [];
  store.subscribeRecovery((event) => events.push(event));
  ...
  const writeFail = createProgressStore(new FakeStorage({}, [new Error("security")])); const failed = [];
  writeFail.subscribeRecovery((event) => failed.push(event)); writeFail.dispatch({ type: "recordAnswer", answer: answer(2) });
  assert.equal(writeFail.getSnapshot().answers.length, 1); assert.equal(failed.at(-1).code, "storageWriteFailed");
});

test("import/export is canonical and invalid input is atomic", () => {
  const storage = new FakeStorage(); const store = createProgressStore(storage);
  ...
  assert.equal(imported.dispatch({ type: "importProgress", raw: JSON.stringify({ ...v2(), version: 3 }) }), false);
  assert.equal(imported.exportProgress(), before);
  assert.equal(parseImportedProgress(JSON.stringify(v2({ difficultQuestionIds: ["q-1", "q-1"] }))), undefined);
...
test("all reducer actions are immutable and the source confines storage writes to the boundary", () => {
  const store = createProgressStore(new FakeStorage()); const before = store.getSnapshot();
