...
const primarySources = await import(`data:text/javascript;base64,${Buffer.from(javaScript).toString("base64")}`);
...
test("runtime primary-source loader accepts singular document shard payloads", () => {
  const shard = primarySources.normalizeSourceDocumentShard("singular.json", { default: { document: clone(baseDocument) } });
...
test("runtime primary-source loader accepts plural documents shard payloads and merges their chunks", () => {
  ...
  const shard = primarySources.normalizeSourceDocumentShard("plural.json", { default: { documents: [first, second] } });
  ...
  assert.deepEqual(document.chunks.map((chunk) => chunk.chunkId), ["doc-a--001", "doc-a--002"]);
...
test("runtime primary-source loader rejects malformed singular and plural payloads", () => {
  assert.throws(
    () => primarySources.normalizeSourceDocumentShard("bad-singular.json", { default: { document: { officialDocumentId: "doc-a" } } }),
    /bad-singular\.json is malformed/
  );
  assert.throws(
    () => primarySources.normalizeSourceDocumentShard("bad-plural.json", { default: { documents: [{ chunks: [] }] } }),
    /bad-plural\.json is malformed/
  );
  assert.throws(
    () => primarySources.normalizeSourceDocumentShard("mixed-plural.json", { default: { documents: [clone(baseDocument), { chunks: [] }] } }),
