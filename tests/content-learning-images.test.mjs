...
const manifest = JSON.parse(
  readFileSync("content/learning-images/learning-images.manifest.json", "utf8"),
);
const runtimeManifest = JSON.parse(
  readFileSync("content/learning-images/learning-images.runtime.json", "utf8"),
);
const evidence = JSON.parse(
  readFileSync("content/validation/learning-images.evidence.json", "utf8"),
);
...
function validateWithMockFiles(badManifest) {
  return validateLearningImages({
    ...
    readDimensions: () => ({
      width: badManifest.images[0].width,
      height: badManifest.images[0].height,
    }),
  });
}

test("learning-image validator passes current manifest and reports computed coverage", () => {
  const output = execFileSync("node", ["scripts/content-learning-images.mjs"], {
    encoding: "utf8",
  });
...
test("learning-image runtime manifest excludes governance-only fields", () => {
  ...
  assert.ok(
    manifest.images.some(
      (image) => image.sha256 && image.provenance?.promptSummary && image.safety,
    ),
  );
  assert.ok(
    manifest.coverage.some(
      (record) => record.sourceFingerprint && record.reviewer && record.evidenceStatus,
    ),
  );
});

test("learning-image validator rejects missing, stale, or governance-heavy runtime manifests", () => {
  const missing = validateLearningImages({
    topicGuide,
    vocabulary,
    manifest,
  ...
  assert.ok(
    missing.errors.includes(
      "learning-images.runtime.json: runtime manifest is missing or malformed.",
    ),
  );

  const stale = clone(runtimeManifest);
  stale.images[0].localPath = runtimeManifest.images[1].localPath;
  const staleResult = validateLearningImages({
    topicGuide,
    vocabulary,
    manifest,
    runtimeManifest: stale,
    evidence,
  });
  assert.ok(
    staleResult.errors.includes(
  ...
  const governanceResult = validateLearningImages({
    topicGuide,
    vocabulary,
    manifest,
    runtimeManifest: governanceHeavy,
    evidence,
  });
  assert.ok(
    governanceResult.errors.some((error) =>
      error.includes("governance-only field must not be present"),
    ),
  );
});
...
test("learning-image runtime projection does not throw for malformed full-manifest records", () => {
  ...
  assert.ok(
    result.errors.some((error) => error.includes("images[0]: image record must be an object.")),
  );
  assert.ok(
    result.errors.some((error) =>
      error.includes("coverage[0]: coverage record must be an object."),
    ),
  );
});
...
test("learning-image CLI direct invocation guard handles URL-escaped paths", () => {
  ...
  assert.equal(
    isDirectInvocation(scriptUrl, resolve("other path", "content-learning-images.mjs")),
    false,
  );
...
test("learning-image manifest has no duplicate direct image hashes", () => {
  const directImageIds = new Set(
    manifest.coverage
      .filter((record) => record.status === "direct")
      .flatMap((record) => record.imageIds),
  );
  const hashes = manifest.images
    .filter((image) => directImageIds.has(image.imageId))
...
test("direct SVG generation uses unit-specific visual entropy", () => {
  const firstSvg = svgForImage(
    "topic-learning:parallel-parking:1:watch-side-clearance-before-moving",
    "topic",
  );
  const secondSvg = svgForImage(
    "topic-learning:parallel-parking:2:watch-rear-clearance-before-moving",
    "topic",
  );
...
test("learning-image units are computed from topic guide and vocabulary", () => {
  ...
  assert.equal(
    units.filter((unit) => unit.kind === "topicSummary").length,
    topicGuide.topics.length,
  );
...
test("learning-image validator rejects remote or question-image replacement paths", () => {
  const badManifest = clone(manifest);
  badManifest.images[0].localPath =
    "content/assets/questions/source-bandinopla-testdeconducir-b/b1.jpg";
...
test("learning-image validator rejects traversed localPath outside learning assets without throwing", () => {
  const badManifest = clone(manifest);
  badManifest.images[0].localPath =
    "content/assets/learning/../../assets/questions/source-bandinopla-testdeconducir-b/b1.jpg";
  const result = validateWithMockFiles(badManifest);
  assert.ok(result.errors.some((error) => error.includes("content/assets/learning/generated/v1")));
  assert.ok(result.errors.some((error) => error.includes("canonical question-image assets")));
});
...
test("learning-image validator accepts safe dot-segment localPath after resolution", () => {
  ...
  safeManifest.images[0].localPath = safeManifest.images[0].localPath.replace(
    "generated/v1/",
    "generated/v1/./",
  );
...
test("learning-image validator rejects non-string localPath values without throwing", () => {
  for (const localPath of [
    null,
    { path: manifest.images[0].localPath },
    [manifest.images[0].localPath],
    42,
  ]) {
    const badManifest = clone(manifest);
    ...
    assert.ok(
      result.errors.some((error) =>
        error.includes("localPath must be a repository-relative string"),
      ),
    );
  }
});
...
test("learning-image validator reports object image entries missing metadata without throwing", () => {
  ...
  assert.ok(
    result.errors.some((error) =>
      error.includes("images[0]: localPath must be a repository-relative string"),
    ),
  );
});
...
test("learning-image validator reports mixed valid and invalid image entries without throwing", () => {
  ...
  assert.ok(
    result.errors.some((error) => error.includes("images[1]: image record must be an object.")),
  );
  assert.ok(
    result.errors.some((error) => error.includes("images[3]: image record must be an object.")),
  );
});
...
test("learning-image validator rejects missing coverage records", () => {
  ...
  assert.ok(
    result.errors.some((error) => error.includes("missing learning-image coverage record")),
  );
});
...
test("learning-image validator rejects generic topic-wide shared coverage", () => {
  ...
  topicRecord.sharedConcept = {
    ...
    titleRu: "Общая тема",
    rationaleRu:
      "Намеренно слишком общий bucket всей темы, который не доказывает смысловую близость единиц.",
    relatedUnitIds: badManifest.coverage
      .filter(
        (record) =>
          record.unitId.includes(`:${topicRecord.unitId.split(":")[1]}:`) ||
          record.unitId.endsWith(`:${topicRecord.unitId.split(":")[1]}`),
      )
      .slice(0, 4)
...
test("learning-image validator rejects non-string shared concept keys without throwing", () => {
  for (const conceptKey of [null, { key: "concept-speed" }, ["concept-speed"], 7]) {
    const badManifest = clone(manifest);
    const [first, second] = badManifest.coverage
      .filter((record) => record.unitKind !== "vocabularyTerm")
      .slice(0, 2);
    first.status = "shared";
    first.sharedConcept = {
      conceptKey,
    ...
    assert.ok(
      result.errors.some((error) => error.includes("sharedConcept.conceptKey is required")),
    );
  }
});
...
test("learning-image validator accepts audited shared concept key shape", () => {
  ...
  first.sharedConcept = {
    ...
    rationaleRu:
      "Пара близких учебных единиц использует одну схему наблюдения без привязки ко всей теме.",
    relatedUnitIds: [first.unitId, second.unitId],
  };
  const result = validateLearningImages({
    topicGuide,
...
test("learning-image validator reports mixed valid and invalid coverage entries without throwing", () => {
  ...
  assert.ok(
    result.errors.some((error) =>
      error.includes("coverage[1]: coverage record must be an object."),
    ),
  );
  assert.ok(
    result.errors.some((error) =>
      error.includes("coverage[3]: coverage record must be an object."),
    ),
  );
});
