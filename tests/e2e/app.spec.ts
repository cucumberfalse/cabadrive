...
const learningImages = JSON.parse(
  readFileSync("content/learning-images/learning-images.runtime.json", "utf8"),
);
const processGuide = JSON.parse(readFileSync("content/guide/caba-exam-process.ru.json", "utf8"));
const primarySourceManifest = JSON.parse(
  readFileSync("content/official-documents/manifest.json", "utf8"),
);
const imageOverlays = JSON.parse(
...
const manualLayout = JSON.parse(
  readFileSync("content/manuals/gcba-manual-vehiculo-4-ruedas-2023/layout.ru.json", "utf8"),
);
const manualNavigation = JSON.parse(
  readFileSync("content/manuals/gcba-manual-vehiculo-4-ruedas-2023/navigation.ru.json", "utf8"),
);
const learningImageById = new Map(
  learningImages.images.map((image: { imageId: string }) => [image.imageId, image]),
);
const learningCoverageByUnitId = new Map(
  learningImages.coverage.map((record: { unitId: string }) => [record.unitId, record]),
);
const firstQuestionWrongAnswerIndex = questions[0].answers.findIndex(
  (answer: { id: string }) => answer.id !== questions[0].correctAnswerId,
);
const canonicalQuestionById = new Map(
  questions.map((question: { id: string }) => [question.id, question]),
);
const translationByQuestionId = new Map(
  translations.map((translation: { questionId: string }) => [translation.questionId, translation]),
);
const explanationByQuestionId = new Map(
  explanations.map((explanation: { questionId: string }) => [explanation.questionId, explanation]),
);
...
function normalizePrimarySourceFixtureShard(fileName: string, value: unknown) {
  ...
  const hasMalformedPluralDocument = pluralDocuments.some(
    (document) => !isPrimarySourceFixtureDocument(document),
  );
  const documents = [
...
function loadPrimarySourceDocuments() {
  ...
  for (const fileName of readdirSync("content/primary-sources/documents").filter((name) =>
    name.endsWith(".json"),
  )) {
  ...
  return primarySourceManifest.entries.map((entry: { id: string }) => {
    ...
    return {
      ...document,
      chunks: [...document.chunks].sort(
        (a, b) => a.order - b.order || a.chunkId.localeCompare(b.chunkId),
      ),
    };
  });
}
...
const trafficLawSource = primarySourceDocuments.find(
  (document) => document.officialDocumentId === "ley-24449-transito-seguridad-vial",
)!;
const cabaTrafficSource = primarySourceDocuments.find(
  (document) => document.officialDocumentId === "ley-2148-caba-codigo-transito-transporte",
)!;
const longPrimarySource = primarySourceDocuments.find(
  (document) => document.officialDocumentId === "ley-26994-codigo-civil-comercial",
)!;
...
test("primary source fixture loader accepts singular and plural document shards", () => {
  ...
  expect(
    normalizePrimarySourceFixtureShard("singular.json", { document: fixtureDocument }),
  ).toHaveLength(1);
  expect(
    normalizePrimarySourceFixtureShard("plural.json", {
      documents: [fixtureDocument, secondDocument],
    }).map((document) => document.chunks[0].chunkId),
  ).toEqual(["fixture-doc--001", "fixture-doc--002"]);
  expect(() =>
    normalizePrimarySourceFixtureShard("mixed-plural.json", {
      documents: [fixtureDocument, { chunks: [] }],
    }),
  ).toThrow(/mixed-plural\.json/);
});
...
async function openPrimarySources(page: Page) {
  ...
  await expect(
    page
      .getByLabel("Покрытие корпуса источников")
      .getByText(`${primarySourceManifest.entries.length} документов`, { exact: true }),
  ).toBeVisible();
}

test("О приложении exposes local version and attribution without runtime source requests", async ({
  page,
}) => {
...
type ManualSourceImageCardMetrics = {
  ...
  sectionWidth: number;
  figureWidth: number;
  figureClientWidth: number;
  figureScrollWidth: number;
  imageWidth: number;
  imageHeight: number;
  naturalWidth: number;
...
async function expectFullWidthSourceImageCard(
  page: Page,
  cardId: string,
  options: { desktop: boolean; expectUsefulContent?: boolean },
) {
  ...
  const metrics = await card.evaluate((element): ManualSourceImageCardMetrics => {
    ...
    if (!image || !figure || !grid || !section)
      throw new Error(
        "source image card is missing image, figure, grid, or source-image-cards container",
      );
    ...
    return {
      ...
      minDisplayWidthPx: Number(element.getAttribute("data-min-display-width-px") ?? 0),
      ...
      usefulBounds,
      usefulRenderedWidth: usefulBounds
        ? (usefulBounds.width / image.naturalWidth) * imageRect.width
        : 0,
      usefulRenderedHeight: usefulBounds
        ? (usefulBounds.height / image.naturalHeight) * imageRect.height
        : 0,
      viewportWidth: window.innerWidth,
      documentOverflow: root.scrollWidth - root.clientWidth,
      rootFontSize: Number.parseFloat(getComputedStyle(root).fontSize),
      documentBodyFontSize: Number.parseFloat(
        getComputedStyle(documentBodySample ?? document.body).fontSize,
      ),
  ...
  expect(
    metrics.maxDisplayWidthPx,
    `${cardId} max display width does not exceed source width`,
  ).toBeLessThanOrEqual(metrics.naturalWidth);
  expect(metrics.imageWidth, `${cardId} avoids browser upscaling`).toBeLessThanOrEqual(
    metrics.naturalWidth + 1,
  );
  expect(metrics.imageHeight, `${cardId} preserves visible height`).toBeGreaterThan(1);
  expect(metrics.rootFontSize, `${cardId} root/body readability baseline`).toBeGreaterThanOrEqual(
    16,
  );
  expect(
    metrics.documentBodyFontSize,
    `${cardId} nearby document body text baseline`,
  ).toBeGreaterThanOrEqual(16);
  expect(
    metrics.sourceCardBodyFontSize,
    `${cardId} source-card body text baseline`,
  ).toBeGreaterThanOrEqual(14.5);
  expect(metrics.cardWidth, `${cardId} card spans the source-image grid`).toBeGreaterThanOrEqual(
    metrics.gridWidth * 0.98,
  );
  expect(metrics.imageWidth, `${cardId} uses meaningful card width`).toBeGreaterThanOrEqual(
    Math.min(metrics.cardWidth * 0.7, metrics.naturalWidth),
  );
  ...
  if (options.expectUsefulContent) {
    ...
    expect(
      metrics.usefulRenderedWidth,
      `${cardId} useful content occupies the rendered image`,
    ).toBeGreaterThanOrEqual(metrics.imageWidth * 0.65);
    expect(
      metrics.usefulRenderedHeight,
      `${cardId} useful content occupies the rendered image height`,
    ).toBeGreaterThanOrEqual(metrics.imageHeight * 0.65);
    expect(
      metrics.usefulRenderedWidth,
      `${cardId} useful content occupies meaningful card width when source quality allows`,
    ).toBeGreaterThanOrEqual(Math.min(metrics.cardWidth * 0.65, metrics.usefulBounds?.width ?? 0));
  }
  return metrics;
}

async function expectScrollableReadableSourceImageCard(
  page: Page,
  cardId: string,
  expectedMinDisplayWidthPx: number,
) {
  const metrics = await expectFullWidthSourceImageCard(page, cardId, { desktop: false });

  expect(metrics.minDisplayWidthPx, `${cardId} readable min width metadata`).toBe(
    expectedMinDisplayWidthPx,
  );
  expect(
    metrics.minDisplayWidthPx,
    `${cardId} min width respects no-upscale cap`,
  ).toBeLessThanOrEqual(metrics.maxDisplayWidthPx);
  expect(metrics.figureWidth, `${cardId} figure stays inside the card`).toBeLessThanOrEqual(
    metrics.cardWidth + 1,
  );
  expect(metrics.figureWidth, `${cardId} figure stays inside the viewport`).toBeLessThanOrEqual(
    metrics.viewportWidth + 1,
  );
  expect(
    metrics.figureClientWidth,
    `${cardId} scroll viewport stays inside card`,
  ...
  expect(
    metrics.figureScrollWidth,
    `${cardId} figure exposes visual-only horizontal scroll`,
  ).toBeGreaterThan(metrics.figureClientWidth + expectedScrollDeltaPx);
  expect(
    metrics.figureScrollWidth,
    `${cardId} scroll content reaches the readable minimum`,
  ).toBeGreaterThanOrEqual(expectedMinDisplayWidthPx - 1);
  expect(metrics.imageWidth, `${cardId} image keeps readable mobile width`).toBeGreaterThanOrEqual(
    expectedMinDisplayWidthPx - 1,
  );
  expect(
    metrics.imageWidth,
    `${cardId} image does not upscale beyond source width`,
  ).toBeLessThanOrEqual(metrics.naturalWidth + 1);
  expect(
    metrics.imageHeight,
    `${cardId} image is not the tiny phone strip regression`,
  ).toBeGreaterThan(70);
  expect(
    metrics.documentOverflow,
    `${cardId} does not create document-level horizontal overflow`,
...
async function expectManualImageTermTranslations(
  ...
  const metrics = await translations.evaluate((element) => {
    ...
    return {
      ...
      emptyDefinitionCount: definitions.filter((definition) => !definition.textContent?.trim())
        .length,
    };
  });

  expect(metrics.renderedWidth, "translation glossary is rendered").toBeGreaterThan(1);
  expect(metrics.termCount, "translation glossary has paired Spanish terms").toBeGreaterThanOrEqual(
    options.minPairs ?? expectedPairs.length,
  );
  expect(metrics.definitionCount, "translation glossary has one Russian definition per term").toBe(
    metrics.termCount,
  );
  expect(metrics.allTermsMarkedSpanish, "translation glossary marks Spanish source terms").toBe(
    true,
  );
  ...
  for (const pair of expectedPairs) {
    await expect(
      translations.locator("dt").filter({ hasText: pair.termEs }).first(),
    ).toHaveAttribute("lang", "es");
    await expect(
      translations.locator("dd").filter({ hasText: pair.translationRu }).first(),
    ).toBeVisible();
  }
}
...
async function expectIndependentManualPageLayout(
  page: Page,
  pageNumber: number,
  minBlockCount = 3,
) {
  ...
  for (const box of blockBoxes) {
    expect(
      box.scrollWidth,
      `manual block ${box.id} overflows horizontally on page ${pageNumber}`,
    ).toBeLessThanOrEqual(box.clientWidth + 2);
    expect(
      box.scrollHeight,
      `manual block ${box.id} overflows vertically on page ${pageNumber}`,
    ).toBeLessThanOrEqual(box.clientHeight + 2);
  }
  for (let index = 0; index < blockBoxes.length; index += 1) {
    for (let nextIndex = index + 1; nextIndex < blockBoxes.length; nextIndex += 1) {
      expect(
        boxesOverlap(blockBoxes[index], blockBoxes[nextIndex]),
        `manual blocks ${blockBoxes[index].id} and ${blockBoxes[nextIndex].id} overlap on page ${pageNumber}`,
      ).toBe(false);
    }
  }

  const visualBoxes = (
    await renderedManualBoxes(page.getByTestId("manual-preserved-visual-region"))
  ).filter((box) => box.width > 0 && box.height > 0);
  expect(visualBoxes.length).toBeGreaterThan(0);
  for (const blockBox of blockBoxes) {
    for (const visualBox of visualBoxes) {
      expect(
        boxesOverlap(blockBox, visualBox),
        `manual block ${blockBox.id} overlaps preserved visual region ${visualBox.id} on page ${pageNumber}`,
      ).toBe(false);
    }
  }
}
...
async function expectAppendixIVSourceMasks(
  ...
  const result = await page.getByTestId("manual-page-canvas").evaluate((canvas, samplePoints) => {
    ...
    return (samplePoints as Array<{ role: string; x: number; y: number; label: string }>).map(
      (sample) => {
        const point = {
  ...
  for (const sample of result) {
    ...
    expect(sample.coveringMask?.geometry, `${sample.label} uses source geometry`).toMatch(
      /^source_page_/,
    );
    expect(sample.coveringMask?.background, `${sample.label} mask uses page-colored fill`).toBe(
      "rgb(255, 253, 248)",
    );
    expect(
      sample.coveringMask?.opacity,
      `${sample.label} mask is opaque enough`,
...
async function expectReadableManualMobileBlocks(page: Page, pageNumber: number) {
  ...
  const instructionalBoxes = blockBoxes.filter(
    (box) => box.type && !["pageNumber", "label", "footnote"].includes(box.type),
  );
  expect(
    instructionalBoxes.length,
    `page ${pageNumber} has primary Russian instructional blocks`,
  ).toBeGreaterThan(0);
  for (const box of instructionalBoxes) {
    expect(
      box.fontSize,
      `manual block ${box.id} on page ${pageNumber} is too small for mobile reading`,
    ).toBeGreaterThanOrEqual(8);
    expect(
      box.height,
      `manual block ${box.id} on page ${pageNumber} has no practical text box height`,
    ).toBeGreaterThanOrEqual(box.fontSize * 0.95);
  }
}
...
async function assertSourceTocGeometry(
  page: Page,
  options: { checkHorizontalOverflow?: boolean } = {},
) {
  const problems = await page
    .locator(".source-toc-list")
    .evaluate((list, checkHorizontalOverflow) => {
      const tolerance = 1;
      const issues: string[] = [];
      ...
      if (
        ...
        issues.push(
          `document horizontal overflow: ${document.documentElement.scrollWidth} > ${viewportWidth}`,
        );
      }
      ...
      const visibleButtons = Array.from(list.querySelectorAll("button"))
        ...
        .filter(
          ({ rect }) =>
            rect.width > 0 &&
            rect.height > 0 &&
            rect.bottom > listRect.top + tolerance &&
            rect.top < listRect.bottom - tolerance,
        )
...
test("default Learn exposes all questions and uses session-stable controlled shuffle", async ({
  browser,
}) => {
  ...
  expect(canonicalOrder).toEqual(
    questions.slice(0, 3).map((question: { id: string }) => question.id),
  );
  ...
  await expect(
    reshuffledPage.getByTestId("question-card").locator(".question-flow-nav").getByText("1 / 460"),
  ).toBeVisible();
...
test("learning flow renders category B image and records a mistake", async ({ page }) => {
  ...
  await expect(
    card.locator(`[aria-label="${difficultyAria[questions[0].difficulty]}"]`),
  ).toBeVisible();
  ...
  await expect(page.locator(".support-block.explanation")).toContainText(
    (explanationByQuestionId.get(questions[0].id) as { textRu: string }).textRu,
  );
  ...
  await page
    ...
    .locator(".question-flow-nav")
    .getByRole("button", { name: "Предыдущий" })
    .click();
  ...
  await expect(
    page.locator(".side-list").locator(`[aria-label="${difficultyAria[questions[0].difficulty]}"]`),
  ).toBeVisible();
  await expect(
    page
      .getByTestId("question-card")
      .locator(`[aria-label="${difficultyAria[questions[0].difficulty]}"]`),
  ).toBeVisible();
  await expect(page.getByText("Что означает этот жест?")).toHaveCount(0);
  await expect(page.locator(".support-block.explanation")).toHaveCount(0);
  ...
  const mistakeToggle = page
    .getByTestId("question-card")
    .getByRole("button", { name: /¿Qué indica esta seña/ });
  ...
  await expect(
    page.getByTestId("question-card").getByTestId("image-explanation-overlay"),
  ).toBeVisible();
...
test("overlay data loads with full current coverage and question-specific reused-image entries", () => {
  ...
  expect(
    new Set(imageOverlays.overlays.map((overlay: { questionId: string }) => overlay.questionId))
      .size,
  ).toBe(imageBackedQuestions.length);
  expect(
    imageOverlays.overlays.filter((overlay: { localPath: string }) =>
      overlay.localPath.endsWith("/b2.jpg"),
    ),
  ).toMatchObject([
...
test("image explanation overlay bounds follow the rendered bitmap when image height is constrained", async ({
  page,
}) => {
  ...
  const geometry = await card.evaluate((element) => {
    ...
    const dimRegions = [
      ...element.querySelectorAll("[data-overlay-role='background_irrelevant_dim']"),
    ];
    if (
...
test("cached image path navigation keeps the frame keyed to the current source dimensions", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.goto("/");
  ...
  await page
    ...
    .click();
  await expect(page.getByText(questions[1].officialTextEs)).toBeVisible();
  await expect(page.locator(".question-image-frame img")).toHaveAttribute(
    "src",
    `/${nextImagePath}`,
  );
  await page.evaluate(
...
test("learning timer pauses, resumes, and does not count down invisible tickets", async ({
  page,
}) => {
...
test("learning timeout is unresolved only until the learner answers after the limit", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-01-01T00:00:00Z") });
  await page.goto("/");
...
test("answered learning ticket restored by search does not restart as unresolved timer", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-01-01T00:00:00Z") });
  await page.goto("/");
...
test("learning search with no matches does not fall back to an answerable question", async ({
  page,
}) => {
  await page.goto("/");
  await expect(page.getByTestId("question-card")).toBeVisible();
  await page
    .getByPlaceholder("Поиск по испанскому, русскому, теме")
    .fill("zzzz-no-local-ticket-match");
...
test("exam mode hides translation and explanation during active attempt and stores score", async ({
  page,
}) => {
  await page.goto("/");
...
test("vocabulary and guide are available", async ({ page }) => {
  ...
  const balizasCoverage = learningCoverageByUnitId.get("vocabulary-term:term-balizas") as {
    imageIds: string[];
  };
  ...
  await expect(vocabularyImage.locator("img")).toHaveAttribute(
    "src",
    new RegExp(balizasImage.localPath.replace(/\//g, "\\/")),
  );
...
test("process guide renders B1 Otorgamiento scope, official sources, volatile warnings, and glossary", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: /Процесс/ }).click();
  ...
  await expect(page.getByTestId("process-section-practical-car-exam")).toContainText(
    "autos doble comando",
  );
  await expect(page.getByTestId("process-section-exam-day-psychophysical")).toContainText(
    "psicología, visión, audición",
  );
  await expect(page.getByTestId("process-section-turno-bui-sede")).toContainText("Sedes, turnos");
  await expect(page.getByTestId("process-section-adjacent-paths")).toContainText(
  ...
  await expect(
    page
      .getByTestId("process-section-practical-car-exam")
      .getByRole("link", { name: /GCBA: Examen práctico/ }),
  ).toHaveAttribute("href", /buenosaires\.gob\.ar/);
  await expect(page.getByRole("link", { name: /Manual de Procedimientos GCBA/ })).toHaveAttribute(
    "href",
    /PE-DIS-SECGVC-DGHC-562-25-ANX\.pdf/,
  );
...
test("process guide stays local-first without external requests, remote images, or PDF viewer", async ({
  page,
}) => {
...
test("complete RU manual surface renders Russian layout pages with semantic navigation and local assets only", async ({
  page,
}) => {
  const externalRequests: string[] = [];
  const pdfRequests: string[] = [];
  ...
  page.on("request", (request) => {
    ...
    if (!["localhost", "127.0.0.1"].includes(url.hostname)) externalRequests.push(request.url());
    if (url.pathname.toLowerCase().endsWith(".pdf")) pdfRequests.push(request.url());
    if (/\/api\/|openai|live-ai|backend/i.test(url.pathname + url.hostname))
      backendLikeRequests.push(request.url());
  });
  ...
  await expect(page.getByLabel("Покрытие полного manual")).toContainText(
    `${manualNavigation.entries.length} разделов`,
  );
  await expect(page.getByLabel("Покрытие полного manual")).toContainText(
    "200 локальных изображений",
  );
  await expect(page.locator("iframe, embed, object")).toHaveCount(0);
  await expect(
    page.locator(".manual-reader a[href$='.pdf'], .manual-reader a[href*='.pdf']"),
  ).toHaveCount(0);
  await expect(page.locator(".manual-page-grid, .manual-visual, .manual-translation")).toHaveCount(
    0,
  );
  await expect(page.locator(".manual-russian-page-flow")).toHaveCount(0);
  ...
  await expectAppendixIVSourceMasks(page, 14, [
    { role: "source-heading", x: 0.481, y: 0.39, label: "page 14 Spanish introduction heading" },
  ]);
  await expect(manualImage).toHaveAttribute(
    "src",
    new RegExp(manualManifest.pages[13].visualAsset.localPath.replace(/\//g, "\\/")),
  );
  await expect(manualImage).toHaveJSProperty(
    "naturalWidth",
    manualManifest.pages[13].visualAsset.width,
  );
  await expect(manualImage).toHaveJSProperty(
    "naturalHeight",
    manualManifest.pages[13].visualAsset.height,
  );
  ...
  await expectAppendixIVSourceMasks(page, 75, [
    { role: "source-list", x: 0.507, y: 0.36, label: "page 75 Spanish list text" },
  ]);
  ...
  await expect(page.getByTestId("manual-page-russian-layout")).toContainText(
  ...
  await expect(manualImage).toHaveAttribute(
    "src",
    new RegExp(manualManifest.pages[184].visualAsset.localPath.replace(/\//g, "\\/")),
  );
  ...
  await expect(page.getByTestId("manual-page-detail")).toContainText("94 / 200");
  await expect(page.getByTestId("manual-selected-semantic-label")).toContainText("Стресс");
  await expect(page.getByTestId("manual-selected-semantic-label")).not.toContainText(
    "Сон и усталость",
  );
  await expect(page.getByTestId("manual-nav-ch4-stress")).toHaveClass(/active/);
  ...
  await expect(page.getByTestId("manual-selected-semantic-label")).toContainText(
    "К равноправному обществу",
  );
  ...
  await expect(page.getByText("Найдено: 1 страниц")).toBeVisible();
  const genderViolenceSearchResult = page.locator(
    '[data-search-result-id="section-ch5-gender-violence-prevention"]',
  );
  await expect(genderViolenceSearchResult).toBeVisible();
  await expect(genderViolenceSearchResult).toHaveAttribute(
  ...
  await expect(page.getByText("Найдено: 1 страниц")).toBeVisible();
  const equalSocietyPageNumberResult = page.locator(
    '[data-search-result-id="section-ch5-equal-society"]',
  );
  const genderViolencePageNumberResult = page.locator(
    '[data-search-result-id="section-ch5-gender-violence-prevention"]',
  );
  await expect(equalSocietyPageNumberResult).toBeVisible();
  await expect(genderViolencePageNumberResult).toBeVisible();
  await expect(equalSocietyPageNumberResult).toHaveAttribute(
    "data-result-entry-id",
    "ch5-equal-society",
  );
  await expect(genderViolencePageNumberResult).toHaveAttribute(
    "data-result-entry-id",
    "ch5-gender-violence-prevention",
  );
  ...
  await expect(
    manualSearchPagingControls.getByRole("button", { name: /Предыдущая/ }),
  ).toBeDisabled();
  await expect(
    manualSearchPagingControls.getByRole("button", { name: /Следующая/ }),
  ).toBeDisabled();
  ...
  await expect(manualImage).toHaveAttribute(
    "src",
    new RegExp(manualManifest.pages[199].visualAsset.localPath.replace(/\//g, "\\/")),
  );
  await expect(page.getByTestId("manual-page-russian-layout")).toContainText(
    manualManifest.pages[199].translation.fullTranslationRu,
  );
...
test("complete RU manual mobile navigation rows and pages 114-123 remain readable without overlap", async ({
  page,
}) => {
  ...
  for (let pageNumber = 114; pageNumber <= 123; pageNumber += 1) {
    ...
    if (!rowBox || !markerBox || !titleBox || !subtitleBox)
      throw new Error(`Missing mobile bounding box for manual page ${pageNumber}`);
  ...
  for (let index = 1; index < rowBoxes.length; index += 1) {
    expect(
      rowBoxes[index - 1].bottom,
      `manual pages ${rowBoxes[index - 1].pageNumber}-${rowBoxes[index].pageNumber} overlap`,
    ).toBeLessThanOrEqual(rowBoxes[index].y + 0.5);
  }
  ...
  for (let pageNumber = 114; pageNumber <= 123; pageNumber += 1) {
    ...
    await row.click();
    await expectIndependentManualPageLayout(
      page,
      pageNumber,
      Math.min(3, manualLayout.pages[pageNumber - 1].blocks.length),
    );
...
test("non-manual startup defers the manual corpus chunk until the manual view opens", async ({
  page,
}) => {
  ...
  page.on("request", (request) => {
    const url = new URL(request.url());
    if (/^\/assets\/manual4Ruedas-[^/]+\.js$/u.test(url.pathname))
      manualChunkRequests.push(request.url());
    if (!["localhost", "127.0.0.1"].includes(url.hostname)) externalRequests.push(request.url());
    if (url.pathname.toLowerCase().endsWith(".pdf")) pdfRequests.push(request.url());
  });
...
test("complete RU manual search with no matches keeps the detail pane empty", async ({ page }) => {
  ...
  await expect(page.locator(".manual-page-list .source-empty-state")).toContainText(
    "Ничего не найдено",
  );
...
test("Pandemia vial prototype opens as a one-section native Russian PDF-faithful composition", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  await page.getByRole("button", { name: /Материалы/ }).click();
  const ordinaryMaterialsBodyTextSize = await page
    .locator(".material-unit-copy p")
    .first()
  ...
  await expect(
    prototype.getByTestId("pandemia-stage-scroll").locator('[data-prose-role="responsive"]'),
  ).toHaveCount(0);
  ...
  await expect(
    prototype.getByText(/Дорожное движение - одна из самых сложных систем/),
  ).toBeVisible();
  await expect(
    prototype.getByTestId("pandemia-segment").filter({ hasText: /В мире/ }),
  ).toBeVisible();
  await expect(
    prototype.getByTestId("pandemia-segment").filter({ hasText: /В городе\s+Буэнос-Айрес/ }),
  ).toBeVisible();
  ...
  await expect(
    prototype.getByRole("button", { name: /Мировой контекст|Контекст города/ }),
  ).toHaveCount(0);
  ...
  for (const asset of cropAssets) {
    expect(asset.src).toContain(
      "/content/assets/manuals/gcba-manual-vehiculo-4-ruedas-2023/sections/pandemia-vial/",
    );
    expect(asset.src).not.toContain("/pages/page-015.jpg");
    expect(asset.src).not.toMatch(/-clean\.svg|icon-people-grid-8m-2f\.svg/);
    expect(asset.src, `${asset.id} uses the accepted source-derived crop`).toMatch(
      /-source\.png(?:[?#].*)?$/,
    );
  ...
  expect(
    pageBox?.width ?? 0,
    "desktop uses a content-sized web frame, not a tiny island",
  ).toBeGreaterThan(850);
  const pageAspect = (pageBox?.width ?? 1) / (pageBox?.height ?? 1);
  expect(pageAspect, "prototype is reframed away from the full PDF page aspect").toBeGreaterThan(
    0.68,
  );
  ...
  expect(
    headingBox?.y ?? 9999,
    "first viewport starts on meaningful content, not blank PDF whitespace",
  ).toBeLessThan((pageBox?.y ?? 0) + 80);
  expect(
    headingBox?.x ?? 9999,
    "content is close to the web frame edge, not a centered PDF island",
  ).toBeLessThan((pageBox?.x ?? 0) + 90);
  const contentBounds = await pageCanvas.evaluate((article) => {
    const articleRect = article.getBoundingClientRect();
    const elementRects = [
      ...article.querySelectorAll(
        '[data-testid="pandemia-segment"], [data-testid="pandemia-crop-asset"], [data-testid="pandemia-native-shape"]',
      ),
    ]
  ...
  expect(contentBounds.bottomRatio, "content frame has no huge bottom PDF margin").toBeLessThan(
    0.08,
  );
  if (testInfo.project.name === "mobile") {
    expect(
      pageBox?.width ?? 0,
      "mobile keeps a larger readable fixed-format document",
    ).toBeGreaterThan(stageBox?.width ?? 0);
    const scrollLeft = await prototype
      .getByTestId("pandemia-stage-scroll")
      .evaluate((element) => element.scrollLeft);
    expect(
      scrollLeft,
      "mobile starts at the reframed content edge instead of panning across blank PDF margin",
    ).toBeLessThan(8);
  }
  ...
  for (const circle of [
    ...
    const textBox = await requireBox(
      pageCanvas.locator(`[data-segment-id="${circle.segment}"]`),
      `${circle.segment} text`,
    );
    const assetBox = await requireBox(assetLocator, `${circle.asset} crop`);
    const shapeBox = await requireBox(
      pageCanvas.locator(`[data-shape-id="${circle.shape}"]`),
      `${circle.shape} shape`,
    );

    expect(textBox.y, `${circle.segment} text starts inside circle`).toBeGreaterThanOrEqual(
      shapeBox.y - 1,
    );
    expect(assetBox.x, `${circle.asset} crop starts inside circle`).toBeGreaterThanOrEqual(
      shapeBox.x - 1,
    );
    expect(
      assetBox.x + assetBox.width,
      `${circle.asset} crop ends inside circle`,
    ).toBeLessThanOrEqual(shapeBox.x + shapeBox.width + 1);
    expect(assetBox.y, `${circle.asset} crop starts inside circle`).toBeGreaterThanOrEqual(
      shapeBox.y - 1,
    );
    expect(
      assetBox.y + assetBox.height,
      `${circle.asset} crop stays inside circle`,
    ).toBeLessThanOrEqual(shapeBox.y + shapeBox.height + 1);
    expect(
      textBox.y + textBox.height,
      `${circle.segment} text stays above icon crop`,
    ).toBeLessThanOrEqual(assetBox.y - 4);
    expect(overlaps(textBox, assetBox), `${circle.segment} text does not overlap icon crop`).toBe(
      false,
    );
    const topElementAssetId = await page.evaluate(
      ({ x, y }) =>
        document.elementFromPoint(x, y)?.closest("[data-asset-id]")?.getAttribute("data-asset-id"),
      { x: assetBox.x + assetBox.width / 2, y: assetBox.y + assetBox.height / 2 },
    );
    expect(
      topElementAssetId,
      `${circle.asset} is not covered by text layers or circle backgrounds`,
    ).toBe(circle.asset);
  }

  const peopleGridBox = await requireBox(
    pageCanvas.locator('[data-asset-id="people-grid-icon"]'),
    "people grid icon",
  );
  const peopleGridSemantics = await pageCanvas
    .locator('[data-asset-id="people-grid-icon"]')
    .evaluate((element) => ({
  ...
  const maleVictimsBox = await requireBox(
    pageCanvas.locator('[data-segment-id="male-victims"]'),
    "male victims row",
  );
  const maleVictimsPanelBox = await requireBox(
    pageCanvas.locator('[data-shape-id="male-victims-panel"]'),
    "male victims gray panel",
  );
  const peoplePairBox = await requireBox(
    pageCanvas.locator('[data-asset-id="people-pair-icon"]'),
    "people pair icon",
  );
  const ageRangeBox = await requireBox(
    pageCanvas.locator('[data-segment-id="age-range"]'),
    "age range row",
  );
  const ageRangePanelBox = await requireBox(
    pageCanvas.locator('[data-shape-id="age-range-panel"]'),
    "age range gray panel",
  );
  ...
  expect(
    maleVictimsPanelBox.x - (peopleGridBox.x + peopleGridBox.width),
    "people-grid icon keeps a source-like white gap before the 8 из 10 gray panel",
  ).toBeGreaterThanOrEqual(lowerRowGapMinimum);
  expect(
    ageRangePanelBox.x - (peoplePairBox.x + peoplePairBox.width),
    "people-pair icon keeps a source-like white gap before the 49% gray panel",
  ).toBeGreaterThanOrEqual(lowerRowGapMinimum);
  expect(
    maleVictimsPanelBox.height,
    "8 из 10 gray panel is not vertically oversized",
  ).toBeLessThanOrEqual(lowerRowPanelMaxHeight);
  expect(ageRangePanelBox.height, "49% gray panel is not vertically oversized").toBeLessThanOrEqual(
    lowerRowPanelMaxHeight,
  );
  for (const row of [
    {
      iconBox: peopleGridBox,
      textBox: maleVictimsBox,
      panelBox: maleVictimsPanelBox,
      label: "male victims text",
    },
    {
      iconBox: peoplePairBox,
      textBox: ageRangeBox,
      panelBox: ageRangePanelBox,
      label: "age range text",
    },
  ]) {
    const rowCenterDelta = Math.abs(
      row.iconBox.y + row.iconBox.height / 2 - (row.panelBox.y + row.panelBox.height / 2),
    );
    const rowBottomDelta = Math.abs(
      row.iconBox.y + row.iconBox.height - (row.panelBox.y + row.panelBox.height),
    );
    const panelToIconHeightRatio = row.panelBox.height / row.iconBox.height;
    const lowerPanelEmptySpaceRatio =
      Math.max(0, row.panelBox.height - row.textBox.height) / row.panelBox.height;
    const lowerTextBottomPadding =
      row.panelBox.y + row.panelBox.height - (row.textBox.y + row.textBox.height);
    expect(
      rowCenterDelta,
      `${row.label} gray panel stays vertically centered with its pictogram row`,
    ).toBeLessThanOrEqual(3);
    expect(
      rowBottomDelta,
      `${row.label} gray panel bottom aligns with its pictogram row`,
    ).toBeLessThanOrEqual(3);
    expect(
      panelToIconHeightRatio,
      `${row.label} gray panel height matches the pictogram row`,
    ...
    expect(
      row.textBox.x,
      `${row.label} starts inside the aligned gray panel`,
    ).toBeGreaterThanOrEqual(row.panelBox.x - 1);
    expect(
      row.textBox.y - row.panelBox.y,
      `${row.label} has source-like top padding inside the gray panel`,
    ).toBeGreaterThanOrEqual(lowerTextPaddingMinimum);
    expect(
      row.textBox.x + row.textBox.width,
      `${row.label} ends inside the aligned gray panel`,
    ).toBeLessThanOrEqual(row.panelBox.x + row.panelBox.width + 1);
    expect(
      lowerTextBottomPadding,
      `${row.label} has source-like bottom padding inside the gray panel`,
    ).toBeGreaterThanOrEqual(lowerTextPaddingMinimum);
    expect(
      lowerTextBottomPadding,
      `${row.label} has no large lower empty area`,
    ).toBeLessThanOrEqual(row.panelBox.height * 0.24);
    expect(
      lowerPanelEmptySpaceRatio,
      `${row.label} gray panel remains compact around the text`,
    ).toBeLessThanOrEqual(0.32);
  }
  const airplaneStripBox = await requireBox(
    pageCanvas.locator('[data-shape-id="airplane-strip-panel"]'),
    "airplane blue strip",
  );
  const airplaneCapBox = await requireBox(
    pageCanvas.locator('[data-shape-id="airplane-strip-cap"]'),
    "airplane localized blue cap",
  );
  const airplanePanelBox = await requireBox(
    pageCanvas.locator('[data-shape-id="airplane-card-panel"]'),
    "airplane gray card",
  );
  const airplaneIconBox = await requireBox(
    pageCanvas.locator('[data-asset-id="airplane-icon"]'),
    "airplane icon",
  );
  const airplaneCardTextBox = await requireBox(
    pageCanvas.locator('[data-segment-id="airplane-card"]'),
    "airplane card text",
  );
  const stadiumStripBox = await requireBox(
    pageCanvas.locator('[data-shape-id="stadium-strip-panel"]'),
    "stadium blue strip",
  );
  const stadiumCapBox = await requireBox(
    pageCanvas.locator('[data-shape-id="stadium-strip-cap"]'),
    "stadium localized blue cap",
  );
  const stadiumPanelBox = await requireBox(
    pageCanvas.locator('[data-shape-id="stadium-card-panel"]'),
    "stadium gray card",
  );
  const stadiumIconBox = await requireBox(
    pageCanvas.locator('[data-asset-id="stadium-icon"]'),
    "stadium icon",
  );
  const stadiumCardTextBox = await requireBox(
    pageCanvas.locator('[data-segment-id="stadium-card"]'),
    "stadium card text",
  );
  const blueStripCornerRadii = await pageCanvas
    .locator('[data-shape-id$="-strip-panel"]')
  ...
  expect(
    Math.abs(airplanePanelBox.height - stadiumPanelBox.height),
    "airplane and stadium gray panels use matching source-like height",
  ).toBeLessThanOrEqual(globalCardHeightAlignmentTolerance);
  expect(
    Math.abs(
      airplanePanelBox.y + airplanePanelBox.height - (stadiumPanelBox.y + stadiumPanelBox.height),
    ),
  ...
  for (const radius of blueStripCornerRadii) {
    ...
    expect(radius.bottomLeft, `${radius.id} keeps a flat rectangular bottom-left corner`).toBe(
      "0px",
    );
    expect(radius.bottomRight, `${radius.id} keeps a flat rectangular bottom-right corner`).toBe(
      "0px",
    );
  }
  for (const card of [
    {
      panelBox: airplanePanelBox,
      textBox: airplaneCardTextBox,
      stripBox: airplaneStripBox,
      capBox: airplaneCapBox,
      iconBox: airplaneIconBox,
      label: "airplane",
    },
    {
      panelBox: stadiumPanelBox,
      textBox: stadiumCardTextBox,
      stripBox: stadiumStripBox,
      capBox: stadiumCapBox,
      iconBox: stadiumIconBox,
      label: "stadium",
    },
  ]) {
    const topPadding = card.textBox.y - card.panelBox.y;
    const bottomPadding =
      card.panelBox.y + card.panelBox.height - (card.textBox.y + card.textBox.height);
    const emptySpaceRatio =
      Math.max(0, card.panelBox.height - card.textBox.height) / card.panelBox.height;
    const panelCenterOffset = Math.abs(
      card.textBox.y + card.textBox.height / 2 - (card.panelBox.y + card.panelBox.height / 2),
    );
    expect(
      card.panelBox.height,
      `${card.label} gray card is not vertically oversized`,
    ).toBeLessThanOrEqual(110);
    expect(
      topPadding,
      `${card.label} card text keeps source-like top padding`,
    ).toBeGreaterThanOrEqual(globalCardTextPaddingMinimum);
    expect(
      bottomPadding,
      `${card.label} card text keeps source-like bottom padding`,
    ).toBeGreaterThanOrEqual(globalCardTextPaddingMinimum);
    expect(bottomPadding, `${card.label} card has no large lower empty area`).toBeLessThanOrEqual(
      card.panelBox.height * 0.26,
    );
    expect(
      emptySpaceRatio,
      `${card.label} gray card density stays close to the source`,
    ).toBeLessThanOrEqual(0.38);
    expect(
      panelCenterOffset,
      `${card.label} text remains vertically balanced in the gray card`,
    ).toBeLessThanOrEqual(card.panelBox.height * 0.12);
    expect(
      card.stripBox.y + card.stripBox.height,
      `${card.label} blue strip touches the gray card`,
    ).toBeGreaterThanOrEqual(card.panelBox.y - 1);
    expect(
      card.stripBox.y + card.stripBox.height,
      `${card.label} blue strip does not drift past the gray card seam`,
    ).toBeLessThanOrEqual(card.panelBox.y + 2);
    expect(
      card.capBox.width,
      `${card.label} cap is localized, not a full-width rounded strip`,
    ).toBeLessThanOrEqual(card.stripBox.width * 0.55);
    expect(card.capBox.x, `${card.label} cap starts inside the flat strip width`).toBeGreaterThan(
      card.stripBox.x,
    );
    expect(
      card.capBox.x + card.capBox.width,
      `${card.label} cap ends inside the flat strip width`,
    ).toBeLessThan(card.stripBox.x + card.stripBox.width);
    expect(
      Math.abs(card.capBox.x + card.capBox.width / 2 - (card.iconBox.x + card.iconBox.width / 2)),
      `${card.label} cap is centered under the source icon`,
    ).toBeLessThanOrEqual(card.iconBox.width * 0.25);
    expect(card.capBox.y, `${card.label} cap rises above the flat strip`).toBeLessThan(
      card.stripBox.y,
    );
    expect(
      card.capBox.y + card.capBox.height,
      `${card.label} localized cap merges into the flat strip without a white seam`,
    ).toBeGreaterThanOrEqual(card.stripBox.y + card.stripBox.height - 1);
    expect(
      card.capBox.y + card.capBox.height,
      `${card.label} localized cap remains aligned with the strip bottom instead of becoming a separate dome`,
    ).toBeLessThanOrEqual(card.stripBox.y + card.stripBox.height + 1);
    expect(
      card.iconBox.y + card.iconBox.height,
      `${card.label} icon visually links into the localized cap`,
    ).toBeGreaterThanOrEqual(card.capBox.y + 1);
    expect(
      card.iconBox.y,
      `${card.label} icon remains visibly above the localized strip cap`,
    ).toBeLessThan(card.capBox.y);
  }
  for (const shapeId of ["motorcyclists-circle", "pedestrians-circle", "car-occupants-circle"]) {
    const shapeBox = await requireBox(
      pageCanvas.locator(`[data-shape-id="${shapeId}"]`),
      `${shapeId} shape`,
    );
    expect(
      shapeBox.y + shapeBox.height,
      `${shapeId} leaves vertical gap before people-grid icon`,
    ).toBeLessThanOrEqual(peopleGridBox.y - 8);
    expect(
      shapeBox.y + shapeBox.height,
      `${shapeId} leaves vertical gap before male-victims row`,
    ).toBeLessThanOrEqual(maleVictimsBox.y - 8);
    expect(overlaps(shapeBox, peopleGridBox), `${shapeId} does not overlap people-grid icon`).toBe(
      false,
    );
    expect(overlaps(shapeBox, maleVictimsBox), `${shapeId} does not overlap male-victims row`).toBe(
      false,
    );
  }
  ...
  for (const size of readableSizes) {
    ...
    expect(
      size.fontSize,
      `${size.id} remains comparable to ordinary study-material text`,
    ).toBeGreaterThanOrEqual(minimum);
    expect(size.scrollWidth, `${size.id} does not overflow horizontally`).toBeLessThanOrEqual(
      size.clientWidth + 2,
    );
    expect(size.scrollHeight, `${size.id} does not overflow vertically`).toBeLessThanOrEqual(
      size.clientHeight + 4,
    );
  ...
  for (const paragraph of readableSizes.filter(
    (size) => size.role === "intro" || size.role === "body",
  )) {
    expect(
      paragraph.fontSize,
      `${paragraph.id} matches ordinary Materials body text`,
    ).toBeGreaterThanOrEqual(ordinaryMaterialsBodyTextSize - 0.5);
    expect(
      paragraph.fontSize,
      `${paragraph.id} does not become oversized relative to Materials body text`,
    ).toBeLessThanOrEqual(ordinaryMaterialsBodyTextSize + 2);
    expect(paragraph.whiteSpace, `${paragraph.id} uses adaptive paragraph wrapping`).toBe("normal");
    expect(
      paragraph.textContent,
      `${paragraph.id} has no forced PDF-style line breaks`,
    ).not.toMatch(/\n/);
  }

  const typographyMetrics = await prototype
    .getByTestId("pandemia-segment")
    .evaluateAll((elements) =>
      elements.map((element) => {
        const style = window.getComputedStyle(element);
  ...
  const headingNoWrapMetrics = await page
    .getByRole("heading", { name: "Дорожная пандемия" })
    .evaluate((element) => {
      const style = window.getComputedStyle(element);
      const rect = element.getBoundingClientRect();
      return {
  ...
  expect(
    headingNoWrapMetrics.maxWidth,
    "heading does not keep the rejected forced 16ch/narrow max-width",
  ).not.toMatch(/ch$/);
  if (testInfo.project.name !== "mobile") {
    expect(
      headingNoWrapMetrics.height,
      "desktop heading stays on one line at normal in-app width",
    ).toBeLessThanOrEqual(headingNoWrapMetrics.lineHeight * 1.35);
  }
  expect(
    headingNoWrapMetrics.scrollWidth,
    "heading is not clipped after removing forced wrap",
  ).toBeLessThanOrEqual(headingNoWrapMetrics.clientWidth + 2);
  for (const metrics of typographyMetrics) {
    expect(metrics.fontFamily, `${metrics.id} uses the readable modern UI Pandemia stack`).toMatch(
      /system-ui|-apple-system|BlinkMacSystemFont|Segoe UI|Roboto|Noto Sans|Helvetica Neue|Arial/i,
    );
    expect(
      metrics.fontFamily,
      `${metrics.id} does not use the rejected SF rounded stack`,
    ).not.toMatch(/^"?SFNSRounded|^"?SF Compact Rounded|^"?SF Pro Rounded|Arial Rounded/i);
    expect(
      metrics.fontFamily,
      `${metrics.id} does not use Avenir as the primary stack`,
    ).not.toMatch(/^"?Avenir/i);
    expect(
      metrics.fontFamily,
      `${metrics.id} does not use app Inter as the Pandemia primary stack`,
    ).not.toMatch(/Inter/i);
    expect(
      metrics.letterSpacing,
      `${metrics.id} keeps non-negative readable letter spacing`,
    ).not.toMatch(/^-/);
  }
  ...
  expect(
    ...
    "heading uses a readable strong UI weight",
  ).toBeGreaterThanOrEqual(680);
  expect(
    headingTypography?.fontWeight,
    "heading avoids an over-heavy decorative weight",
  ).toBeLessThanOrEqual(760);
  expect(
    headingTypography?.lineHeightRatio,
    "heading keeps compact readable line height",
  ...
  for (const metrics of [introTypography, bodyTypography]) {
    expect(
      metrics?.fontWeight,
      `${metrics?.id} uses a normal readable UI body weight`,
    ...
    expect(
      metrics?.lineHeightRatio,
      `${metrics?.id} uses comfortable paragraph rhythm`,
    ).toBeGreaterThanOrEqual(1.55);
    expect(
      metrics?.lineHeightRatio,
      `${metrics?.id} uses looser paragraph rhythm`,
    ).toBeLessThanOrEqual(1.75);
  }
  expect(
    infographicTypography.length,
    "infographic text roles are included in computed typography evidence",
  ).toBeGreaterThanOrEqual(8);
  for (const metrics of infographicTypography) {
    expect(
      metrics.fontWeight,
      `${metrics.id} infographic label/statistic uses readable strong UI weight`,
    ).toBeGreaterThanOrEqual(680);
    expect(
      metrics.fontWeight,
      `${metrics.id} infographic label/statistic avoids an ultra-heavy decorative weight`,
    ).toBeLessThanOrEqual(760);
    expect(
      metrics.lineHeight,
      `${metrics.id} records a concrete infographic line-height`,
    ).toBeGreaterThan(0);
  }
  const contextTypography = typographyMetrics.filter((metrics) => metrics.role === "context-label");
  expect(contextTypography.map((metrics) => metrics.id).sort()).toEqual([
    "city-label",
    "global-label",
  ]);
  ...
  for (const metrics of contextTypography) {
    expect(
      metrics.firstLineFontWeight,
      `${metrics.id} has no partial context-label first-line weight override`,
    ).toBe(metrics.fontWeight);
    expect(
      metrics.firstLineFontFamily,
      `${metrics.id} has no partial context-label first-line font override`,
    ).toBe(metrics.fontFamily);
  }
  ...
  expect(selectedProseText, "intro/body prose can be selected as real DOM text").toContain(
    "Дорожное движение - одна из самых сложных систем",
  );
  expect(selectedProseText, "bottom conclusion can be selected as real DOM text").toContain(
  ...
  async function expectResponsiveProseFitsViewport(label: string) {
    const problems = await prototype.evaluate((root) => {
      ...
      if (document.documentElement.scrollWidth > viewportWidth + tolerance) {
        issues.push(
          `document requires horizontal scroll: ${document.documentElement.scrollWidth} > ${viewportWidth}`,
        );
      }
      ...
      for (const element of Array.from(root.querySelectorAll('[data-prose-role="responsive"]'))) {
        ...
        if (stage?.contains(element))
          issues.push(`${segmentId} prose is inside the horizontal-scroll stage`);
        if (style.position === "absolute" || style.position === "fixed")
          issues.push(`${segmentId} prose is pinned instead of responsive flow`);
        if (rect.left < -tolerance || rect.right > viewportWidth + tolerance) {
          issues.push(
            `${segmentId} prose overflows viewport horizontally: ${rect.left}..${rect.right} of ${viewportWidth}`,
          );
        }
        if (element.scrollWidth > element.clientWidth + tolerance) {
          issues.push(
            `${segmentId} prose content is clipped horizontally: ${element.scrollWidth} > ${element.clientWidth}`,
          );
        }
...
test("Introduction index routes open as separate native Russian document pages", async ({
  page,
}, testInfo) => {
  async function requireBox(locator: Locator, label: string) {
    const box = await locator.boundingBox();
  ...
  for (const route of introRoutes) {
    ...
    await expect(reader.getByTestId("manual-guide-nav")).toHaveAttribute(
      "data-active-group-id",
      "introduction",
    );
    await expect(reader.getByTestId("manual-guide-nav")).toHaveAttribute(
      "data-active-child-id",
      route.id,
    );
    ...
    await expect(reader.getByTestId("manual-guide-nav")).toContainText(
    ...
    expect(manualGuideNavText.indexOf("Предисловие")).toBeLessThan(
      manualGuideNavText.indexOf("Введение"),
    );
    expect(manualGuideNavText.indexOf("Введение")).toBeLessThan(
      manualGuideNavText.indexOf("Глава 1. К устойчивой мобильности"),
    );
    await expect(
      reader.getByTestId("manual-guide-pending-section-front-presentation"),
    ).toHaveAttribute("data-status", "implemented");
    await expect(reader.getByTestId("manual-guide-pending-section-front-glossary")).toHaveAttribute(
      "data-status",
      "implemented",
    );
    ...
    await expect(
      activeRouteItem.getByRole("button", { name: route.title, exact: true }),
    ).toBeVisible();
    ...
    await expect(
      reader.locator('[data-guide-entry-id="introduction"] .manual-guide-children button.active'),
    ).toHaveCount(1);
    await expect(
      reader.locator(
        '[data-guide-entry-id="introduction"] .manual-guide-children button[aria-current="page"]',
      ),
    ).toHaveCount(1);
    ...
    await expect(
      reader.locator('[data-testid="manual-source-mask"], [data-testid="pandemia-source-mask"]'),
    ).toHaveCount(0);
    ...
    const issues = await reader.evaluate((root) => {
      ...
      for (const element of Array.from(
        root.querySelectorAll(
          '[data-testid="intro-article-block"], [data-testid="pandemia-segment"][data-prose-role="responsive"]',
        ),
      )) {
        const rect = element.getBoundingClientRect();
        const style = window.getComputedStyle(element);
        ...
        if (style.whiteSpace === "pre" || style.whiteSpace === "pre-line") {
          ...
          if (kind === "paragraph" || kind === "lead" || role === "intro" || role === "body")
            problems.push(`${id} forces PDF-style line breaks`);
        }
      }
      return problems;
    });
    ...
    if (route.id === "intro-road-safety-plan") {
      ...
      await expect(photo).toHaveAttribute(
        "src",
        /sections\/intro-road-safety-plan\/child-seat-photo-source\.jpg/,
      );
    ...
    if (route.id === "intro-incident") {
      ...
      for (const assetId of ["risk-ambiental", "risk-vehicular", "risk-humano"]) {
        ...
        expect(
          box?.width ?? 0,
          `${assetId} remains bounded source component art, not a full page raster`,
        ).toBeLessThan(maxRenderedComponentSize);
        expect(
          box?.height ?? 0,
          `${assetId} remains bounded source component art, not a full page raster`,
        ).toBeLessThan(maxRenderedComponentSize);
      ...
      for (const riskId of ["ambiental", "vehicular", "humano"]) {
        ...
        expect(
          Math.abs(lobeBox.width - lobeBox.height),
          `${riskId} lobe is circular`,
        ).toBeLessThanOrEqual(1);
        expect(
          lobeBox.width,
          `${riskId} circular lobe rejects the previous too-small/compressed risk row`,
        ).toBeGreaterThanOrEqual(110);
        expect(
          lobeBox.x,
          `${riskId} lobe is integrated into the left edge, not a square crop box`,
        ).toBeLessThan(cardBox.x + 4);
        expect(
          lobeBox.x + lobeBox.width,
          `${riskId} lobe overlaps the long rounded panel`,
        ).toBeGreaterThan(cardBox.x + 28);
        expect(
          panelGeometry.cardBackground,
          `${riskId} card container itself is transparent so no full-height rectangle protrudes`,
        ).toBe("rgba(0, 0, 0, 0)");
        expect(
          panelGeometry.panelContent,
          `${riskId} right panel is drawn by the tested pseudo-element`,
        ).not.toBe("none");
        expect(
          panelGeometry.panel.height,
          `${riskId} right rectangle rejects the previous too-short compressed panel`,
        ).toBeGreaterThanOrEqual(92);
        expect(
          panelGeometry.panel.height,
          `${riskId} right rectangle stays lower than the circular lobe`,
        ).toBeLessThan(panelGeometry.lobe.height - 2);
        expect(
          panelGeometry.panel.y,
          `${riskId} rectangle top sits inside the circle, hiding square seam corners`,
        ).toBeGreaterThan(panelGeometry.lobe.y + 2);
        expect(
        ...
        expect(
          panelGeometry.panel.x,
          `${riskId} rectangle enters the circle instead of starting after it`,
        ).toBeGreaterThan(panelGeometry.lobe.x + panelGeometry.lobe.width * 0.5);
        expect(
          panelGeometry.panel.x,
          `${riskId} circle overlaps the rectangle seam enough to mask square corners`,
        ).toBeLessThan(panelGeometry.lobe.x + panelGeometry.lobe.width - 12);
        expect(
          panelGeometry.panelBorderRadius,
          `${riskId} right panel keeps only a soft right-side corner`,
        ).toMatch(/5px|4px/);
        const riskTextTop = Math.min(riskTitleBox.y, riskTextBox.y);
        const riskTextBottom = Math.max(
          riskTitleBox.y + riskTitleBox.height,
          riskTextBox.y + riskTextBox.height,
        );
        expect(
          riskTextTop - panelGeometry.panel.y,
          `${riskId} text keeps source-like top padding inside the panel`,
        ...
        expect(
          isInside(symbolBox, lobeBox, 2),
          `${riskId} pictogram is fully inside the circular lobe with padding`,
        ).toBe(true);
        expect(lobeStyles.borderRadius, `${riskId} lobe uses circular geometry`).toMatch(
          /999px|50%/,
        );
        expect(lobeStyles.overflow, `${riskId} lobe does not hard-crop the source icon`).not.toBe(
          "hidden",
        );
        expect(
          symbolStyles.objectFit,
          `${riskId} source pictogram is contained, not force-cropped`,
        ).toBe("contain");
        expect(
          symbolStyles.mixBlendMode,
          `${riskId} source crop blends away any square bitmap background`,
        ).toBe("multiply");
        expect(
          naturalSymbolBounds.naturalWidth,
          `${riskId} source pictogram PNG rejects the old tight natural crop width`,
        ).toBeGreaterThanOrEqual(256);
        expect(
          naturalSymbolBounds.naturalHeight,
          `${riskId} source pictogram PNG rejects the old tight natural crop height`,
        ).toBeGreaterThanOrEqual(256);
        expect(
          naturalSymbolBounds.naturalWidth / symbolBox.width,
          `${riskId} source pictogram is never browser-upscaled`,
        ).toBeGreaterThanOrEqual(2);
        expect(
          naturalSymbolBounds.naturalHeight / symbolBox.height,
          `${riskId} source pictogram is never browser-upscaled`,
        ).toBeGreaterThanOrEqual(2);
        for (const [edge, margin] of Object.entries({
          ...
          expect(
            margin,
            `${riskId} source pictogram has transparent ${edge} padding and is not clipped by the PNG bounds`,
          ).toBeGreaterThanOrEqual(10);
        }
        const renderedAlphaBox = {
          x:
            symbolBox.x +
            (naturalSymbolBounds.marginLeft / naturalSymbolBounds.naturalWidth) * symbolBox.width,
          y:
            symbolBox.y +
            (naturalSymbolBounds.marginTop / naturalSymbolBounds.naturalHeight) * symbolBox.height,
          width:
            (naturalSymbolBounds.contentWidth / naturalSymbolBounds.naturalWidth) * symbolBox.width,
          height:
            (naturalSymbolBounds.contentHeight / naturalSymbolBounds.naturalHeight) *
            symbolBox.height,
        };
        const lobeCenter = { x: lobeBox.x + lobeBox.width / 2, y: lobeBox.y + lobeBox.height / 2 };
        const iconCenter = {
          x: renderedAlphaBox.x + renderedAlphaBox.width / 2,
          y: renderedAlphaBox.y + renderedAlphaBox.height / 2,
        };
        expect(
          Math.abs(iconCenter.x - lobeCenter.x),
          `${riskId} visible pictogram is centered in the circular lobe`,
        ).toBeLessThanOrEqual(lobeBox.width * 0.08);
        expect(
          Math.abs(iconCenter.y - lobeCenter.y),
          `${riskId} visible pictogram is vertically centered in the circular lobe`,
        ).toBeLessThanOrEqual(lobeBox.height * 0.08);
        expect(
        ...
        expect(
          isInside(renderedAlphaBox, lobeBox, 4),
          `${riskId} visual alpha bounds stay inside the circular lobe`,
        ).toBe(true);
        if (riskId === "humano") {
          expect(
            lobeStyles.backgroundColor,
            "human risk lobe preserves the yellow source row",
          ).toBe("rgb(245, 229, 31)");
          expect(
            panelGeometry.panelBackground,
            "human risk rectangle preserves the yellow source row",
          ).toBe("rgb(245, 229, 31)");
        } else {
          expect(
            lobeStyles.backgroundColor,
            `${riskId} risk lobe preserves the gray source row`,
          ).toBe("rgb(231, 232, 230)");
          expect(
            panelGeometry.panelBackground,
            `${riskId} risk rectangle preserves the gray source row`,
          ).toBe("rgb(231, 232, 230)");
        }
      }
      ...
      for (let index = 1; index < riskGaps.length; index += 1) {
        ...
        expect(
          cardGap,
          `${previous.id} and ${current.id} risk rows keep source-like vertical whitespace`,
        ).toBeGreaterThanOrEqual(10);
        expect(
          lobeGap,
          `${previous.id} and ${current.id} circular lobes do not touch or visually merge`,
        ).toBeGreaterThanOrEqual(10);
      }
      ...
      const recommendationTabBox = await requireBox(
        recommendation.locator("strong"),
        "recommendation blue tab",
      );
      const recommendationTextBox = await requireBox(
        recommendation.locator("p"),
        "recommendation text",
      );
      ...
      expect(
        recommendationStyles.overflow,
        "recommendation callout keeps the raised tab unclipped",
      ).toBe("visible");
      expect(
        recommendationStyles.borderColor,
        "recommendation border stays aligned with the blue tab",
      ).toBe("rgb(39, 135, 166)");
      expect(recommendationStyles.tabBackground, "recommendation tab keeps the source blue").toBe(
        "rgb(39, 135, 166)",
      );
      expect(
        recommendationTabBox.x,
        "recommendation tab starts inside the border without an icon gap",
      ).toBeGreaterThanOrEqual(recommendationBox.x + 12);
      expect(
        recommendationTabBox.x + recommendationTabBox.width,
        "recommendation tab fits within the callout border",
      ).toBeLessThan(recommendationBox.x + recommendationBox.width - 12);
      expect(
        recommendationTabBox.y,
        "recommendation tab remains raised above the top border",
      ).toBeLessThan(recommendationBox.y);
      expect(
      ...
      expect(recommendationTextBox.y, "recommendation text clears the raised tab").toBeGreaterThan(
        recommendationTabBox.y + recommendationTabBox.height + 8,
      );
      expect(
        oldIconHit,
        "the old decorative clipboard/notebook artwork is not rendered or clipped at the callout edge",
      ).toBeNull();
    }
    ...
    if (route.id === "intro-road-safety-plan") {
      ...
      await expect(
        reader.getByTestId("intro-consequence-diagram").getByTestId("intro-source-artwork"),
      ).toHaveCount(1);
      const consequenceBackground = reader.locator(
        '[data-artwork-id="consequence-diagram-background"]',
      );
      await expect(consequenceBackground).toBeVisible();
      await expect(consequenceBackground).toHaveAttribute(
        "src",
        /diagram-consequences-clean-source\.png/,
      );
      ...
      await expect(consequenceBackground).toHaveAttribute(
        "data-cleanup-status",
        /Spanish\/source text cleanup is limited to the original text-bearing regions/,
      );
      await expect(consequenceBackground).toHaveAttribute(
        "data-cleanup-status",
      ...
      await expect(consequenceBackground).toHaveAttribute(
        "data-fidelity-role",
        /complete original consequences gauge high-DPI PDF source crop/,
      );
      ...
      const sourceCropAudit = await consequenceBackground.evaluate(async (element) => {
        ...
        const maskCleanliness = masks.map((zone) => {
          let darkPixels = 0;
          let lightPixels = 0;
          const zoneData = cleanedContext.getImageData(
            zone.x,
            zone.y,
            zone.width,
            zone.height,
        ...
        const connectorZones = [
          { id: "institutions-vertical-connector", x: 230, y: 98, width: 10, height: 58 },
        ].map(zoneStats);
        const fatalBlackZones = [
          { id: "fatal-victims-black-label", x: 405, y: 90, width: 115, height: 12 },
        ].map(zoneStats);
          ...
          for (let y = zone.y; y < zone.y + zone.height; y += 1) {
            for (let x = zone.x; x < zone.x + zone.width; x += 1) {
              ...
              const delta =
                ...
                3;
              const luma =
                0.2126 * cleanedData[index] +
                0.7152 * cleanedData[index + 1] +
                0.0722 * cleanedData[index + 2];
              totalDelta += delta;
        ...
        const seamStats = (zone: {
          ...
          const maxAdjacentDelta = (values: number[]) =>
            values
              .slice(1)
              .reduce((max, value, index) => Math.max(max, Math.abs(value - values[index])), 0);
          return {
            id: zone.id,
      ...
      expect(
        sourceCropAudit.comparedPixels,
        "page 18 source-crop comparison covers the full non-text diagram artwork",
      ).toBeGreaterThan(100_000);
      expect(
        sourceCropAudit.naturalWidth,
        "page 18 runtime image rejects the old low-resolution 620px natural crop",
      ).toBe(3720);
      expect(
        sourceCropAudit.naturalHeight,
        "page 18 runtime image rejects the old low-resolution 260px natural crop",
      ).toBe(1560);
      expect(
        sourceCropAudit.averageDelta,
        "page 18 non-text artwork remains source-derived after high-DPI cleanup, not a redraw",
      ).toBeLessThan(12);
      expect(
        sourceCropAudit.changedRatio,
        "page 18 arcs, pointer, sectors, label boxes, connectors, and icons are retained",
      ).toBeLessThan(0.13);
      for (const zone of sourceCropAudit.maskCleanliness) {
        if (zone.tone === "dark") {
          expect(
            zone.lightRatio,
            `${zone.id} black label has no residual white Spanish text`,
          ).toBeLessThan(0.01);
        } else if (zone.id.startsWith("center-")) {
          expect(
            zone.darkRatio,
            `${zone.id} cleaned center keeps no visible Spanish while preserving the dark ring edge`,
          ).toBeLessThan(0.02);
        } else {
          expect(
            zone.darkRatio,
            `${zone.id} cleaned page 18 background has no residual dark Spanish/text artifacts`,
          ).toBeLessThan(0.01);
        }
      }
      for (const zone of sourceCropAudit.labelCornerZones) {
        expect(
          zone.whiteRatio,
          `${zone.id} has no white rectangular mask remnant at the category label corner`,
        ).toBeLessThan(0.02);
        expect(
          zone.averageLuma,
          `${zone.id} remains beige source label material, not white cover-up`,
        ).toBeLessThan(210);
      }
      for (const zone of sourceCropAudit.connectorZones) {
        expect(
          zone.nonWhiteRatio,
          `${zone.id} is preserved and not cut by a white cleanup mask`,
        ).toBeGreaterThan(0.2);
      }
      for (const zone of sourceCropAudit.fatalBlackZones) {
        expect(
          zone.whiteRatio,
          `${zone.id} has no white text-mask marks on the black label`,
        ).toBeLessThan(0.005);
        expect(
          zone.averageLuma,
          `${zone.id} remains black after local source-text cleanup`,
        ).toBeLessThan(30);
      }
      for (const zone of sourceCropAudit.rectanglePatchZones) {
        expect(
          zone.whiteRatio,
          `${zone.id} rejects the stale white rectangular cleanup patch`,
        ).toBeLessThan(0.35);
        expect(
          zone.changedRatio,
          `${zone.id} rejects block-level/color-matched rectangular cover-up outside glyph strokes`,
        ).toBeLessThan(0.3);
      }
      for (const zone of sourceCropAudit.restoredSourceZones) {
        expect(
          zone.changedRatio,
          `${zone.id} is restored from source pixels instead of a non-source horizontal rectangle`,
        ).toBeLessThan(0.04);
        expect(
          zone.averageDelta,
          `${zone.id} matches the source crop background after cleanup`,
        ).toBeLessThan(4);
        expect(
          zone.averageLuma,
          `${zone.id} has no visible dark/beige bar artifact`,
        ).toBeGreaterThan(235);
        expect(
          zone.nonWhiteRatio,
          `${zone.id} rejects leftover horizontal bar pixels`,
        ).toBeLessThan(0.2);
      }
      for (const zone of sourceCropAudit.centerSeamZones) {
        expect(
          zone.maxHorizontalEdge,
          `${zone.id} has no horizontal hard-edged center patch seam`,
        ).toBeLessThan(8);
        expect(
          zone.maxVerticalEdge,
          `${zone.id} has no vertical hard-edged center patch seam`,
        ).toBeLessThan(8);
      }
      const consequenceBox = await requireBox(
        reader.getByTestId("intro-consequence-diagram"),
        "page 18 consequence diagram",
      );
      const consequenceBackgroundBox = await requireBox(
        consequenceBackground,
        "page 18 cleaned source background",
      );
      ...
      const consequenceCenterBox = await requireBox(
        reader.locator(".intro-consequence-center"),
        "page 18 center incident label",
      );
      if (testInfo.project.name === "mobile") {
        expect(
          consequenceBackgroundBox.width,
          "mobile keeps the complete page 18 gauge visible within the article",
        ).toBeLessThanOrEqual(consequenceBox.width + 1);
        expect(
          consequenceBackgroundBox.width,
          "mobile still renders the complete page 18 gauge, not a clipped fragment",
        ).toBeGreaterThan(280);
      } else {
        expect(
          consequenceBackgroundBox.width,
          "desktop page 18 source crop renders as the full gauge composition",
        ).toBeGreaterThan(680);
        expect(
          consequenceBackgroundBox.height,
          "desktop page 18 source crop preserves gauge proportions",
        ).toBeGreaterThan(280);
      }
      expect(
      ...
      expect(
        isInside(
          consequenceCenterBox,
          consequenceBackgroundBox,
          testInfo.project.name === "mobile" ? 16 : 40,
        ),
        "center Russian label sits inside the original gauge ring area",
      ).toBe(true);
      const consequenceComposition = await reader
        .getByTestId("intro-consequence-diagram")
        .evaluate((diagram) => {
          const labels = Array.from(diagram.querySelectorAll(".intro-consequence-card")).map(
            (element) => {
              const rect = element.getBoundingClientRect();
              ...
              return {
                ...
                heading: heading
                  ...
                  : null,
                text: textRect
                  ? { x: textRect.x, y: textRect.y, width: textRect.width, height: textRect.height }
                  : null,
                headingStyle: headingStyle
                  ? {
                      ...
                      beforeBackgroundColor: window.getComputedStyle(headingElement!, "::before")
                        .backgroundColor,
                      afterContent: window.getComputedStyle(headingElement!, "::after").content,
                      afterBackgroundColor: window.getComputedStyle(headingElement!, "::after")
                        .backgroundColor,
          ...
          const background = diagram
            .querySelector('[data-artwork-id="consequence-diagram-background"]')!
      ...
      expect(
        consequenceComposition.centerStyle.backgroundColor,
        "center incident text has no visible background rectangle",
      ).toBe("rgba(0, 0, 0, 0)");
      expect(
        consequenceComposition.centerStyle.backgroundImage,
        "center incident text has no background image",
      ).toBe("none");
      expect(
        consequenceComposition.centerStyle.borderTopWidth,
        "center incident text has no DOM border",
      ).toBe("0px");
      expect(
        consequenceComposition.centerStyle.boxShadow,
        "center incident text has no rectangular shadow",
      ).toBe("none");
      expect(
        consequenceComposition.centerStyle.beforeContent,
        "center incident text has no ::before backing",
      ).toBe("none");
      expect(
        consequenceComposition.centerStyle.afterContent,
        "center incident text has no ::after backing",
      ).toBe("none");
      expect(consequenceComposition.labels).toHaveLength(4);
      const sourceLabelBoxes: Record<
        string,
      ...
      const familyLabel = consequenceComposition.labels.find(
        (label) => label.id === "family-economy",
      );
      expect(
        familyLabel?.headingStyle?.fontSize,
        "family/economy label exposes the shared page 18 category label font size",
      ).toBeTruthy();
      for (const label of consequenceComposition.labels) {
        expect(
          label.overflow,
          `${label.id} Russian label is not clipped by its own container`,
        ).toBe("visible");
        expect(
          isInside(label.rect, consequenceComposition.background, -2),
          `${label.id} label stays within the source gauge crop frame`,
        ).toBe(true);
        expect(label.heading, `${label.id} has a rendered label heading`).not.toBeNull();
        expect(
          label.text,
          `${label.id} has measurable text ink for optical centering`,
        ).not.toBeNull();
        expect(
          label.headingStyle?.alignItems,
          `${label.id} label text uses vertical centering`,
        ).toBe("center");
        expect(
          label.headingStyle?.backgroundColor,
          `${label.id} label text does not add a mismatched DOM backing plate`,
        ).toBe("rgba(0, 0, 0, 0)");
        expect(
          label.headingStyle?.backgroundImage,
          `${label.id} label text has no DOM background image`,
        ).toBe("none");
        expect(
          label.headingStyle?.borderTopWidth,
          `${label.id} label text has no DOM border plate`,
        ).toBe("0px");
        expect(
        ...
        expect(
          label.headingStyle?.boxShadow,
          `${label.id} label text has no rectangular backing shadow`,
        ).toBe("none");
        expect(
          label.headingStyle?.beforeContent,
          `${label.id} label text has no ::before backing`,
        ).toBe("none");
        expect(
          label.headingStyle?.afterContent,
          `${label.id} label text has no ::after backing`,
        ).toBe("none");
        expect(
          label.headingStyle?.fontSize,
          `${label.id} category typography matches the other category labels`,
        ).toBe(familyLabel?.headingStyle?.fontSize);
        expect(
          label.headingStyle?.fontWeight,
          `${label.id} category typography uses the shared weight`,
        ).toBe(familyLabel?.headingStyle?.fontWeight);
        expect(
          label.headingStyle?.lineHeight,
          `${label.id} category typography uses the shared line-height`,
        ).toBe(familyLabel?.headingStyle?.lineHeight);
        expect(
          label.headingStyle?.textTransform,
          `${label.id} category label keeps the shared uppercase treatment`,
        ).toBe("uppercase");
        ...
        const sourceCenterY =
          consequenceComposition.background.y +
          ((sourceBox.y + sourceBox.height / 2) / 260) * consequenceComposition.background.height;
        const sourceBackingWidth =
          (sourceBox.width / 620) * consequenceComposition.background.width;
        ...
        const expectedBackingWidth = Math.max(
          sourceBackingWidth,
          label.text!.width + paddingLeft + paddingRight,
        );
        const headingCenterY = label.heading!.y + label.heading!.height / 2;
        const textCenterY = label.text!.y + label.text!.height / 2;
        expect(
          Math.abs(headingCenterY - sourceCenterY),
          `${label.id} Russian label text is vertically centered in the source box`,
        ).toBeLessThanOrEqual(testInfo.project.name === "mobile" ? 5 : 6);
        expect(
          Math.abs(textCenterY - sourceCenterY),
          `${label.id} visible text ink is vertically centered, not top- or bottom-pinned`,
        ).toBeLessThanOrEqual(testInfo.project.name === "mobile" ? 7 : 7);
        expect(
          label.heading!.width,
        ...
        const overlapX = Math.max(
          ...
          Math.min(
            ...
            consequenceComposition.center.x + consequenceComposition.center.width,
          ) - Math.max(label.rect.x, consequenceComposition.center.x),
        );
        const overlapY = Math.max(
          0,
          Math.min(
            label.rect.y + label.rect.height,
            consequenceComposition.center.y + consequenceComposition.center.height,
          ) - Math.max(label.rect.y, consequenceComposition.center.y),
        );
        expect(
          overlapX * overlapY,
          `${label.id} label does not collide with the center ring text`,
        ).toBe(0);
      }
      for (const assetId of [
        "consequence-family-economy",
        "consequence-health",
      ...
      for (const axisId of [
        ...
        const circleBox = await requireBox(
          axisCard.locator(".intro-axis-circle"),
          `${axisId} gray circle`,
        );
        ...
        expect(
          ...
          `${axisId} title stays above the gray circle as in the source`,
        ).toBeLessThanOrEqual(circleBox.y + 4);
        expect(bodyBox.y, `${axisId} body text stays below the gray circle`).toBeGreaterThan(
          circleBox.y + circleBox.height - 2,
        );
        expect(
          Math.abs(circleBox.width - circleBox.height),
          `${axisId} gray field remains circular`,
        ).toBeLessThanOrEqual(1);
        expect(
          isInside(iconBox, circleBox, 6),
          `${axisId} pictogram is complete inside the gray circle with padding`,
        ).toBe(true);
        expect(
          naturalIconBounds.naturalWidth,
          `${axisId} source asset has high-DPI padded natural width`,
        ).toBeGreaterThanOrEqual(192);
        expect(
          naturalIconBounds.naturalHeight,
          `${axisId} source asset has high-DPI padded natural height`,
        ).toBeGreaterThanOrEqual(192);
        expect(
          naturalIconBounds.naturalWidth / iconBox.width,
          `${axisId} source pictogram is never browser-upscaled`,
        ).toBeGreaterThanOrEqual(2);
        expect(
          naturalIconBounds.naturalHeight / iconBox.height,
          `${axisId} source pictogram is never browser-upscaled`,
        ).toBeGreaterThanOrEqual(2);
        expect(
          naturalIconBounds.naturalWidth,
          `${axisId} padded PNG is wider than the previous tight JPG crop`,
        ).toBeGreaterThan(naturalIconBounds.previousTightWidth);
        expect(
          naturalIconBounds.naturalHeight,
          `${axisId} padded PNG is taller than the previous tight JPG crop`,
        ).toBeGreaterThan(naturalIconBounds.previousTightHeight);
        for (const [edge, margin] of Object.entries({
          ...
          expect(
            margin,
            `${axisId} source pictogram alpha bounds have ${edge} padding and are not tight-cropped`,
          ).toBeGreaterThanOrEqual(14);
        }
        expect(circleStyles.borderRadius, `${axisId} axis field uses circular geometry`).toMatch(
          /999px|50%/,
        );
        expect(
          circleStyles.overflow,
          `${axisId} circle does not clip the original pictogram`,
        ).not.toBe("hidden");
        expect(
          circleStyles.backgroundColor,
          `${axisId} circle preserves the source gray field`,
        ).toBe("rgb(236, 236, 234)");
        expect(
          iconStyles.objectFit,
          `${axisId} source pictogram is contained, not force-cropped`,
        ).toBe("contain");
        expect(
          iconStyles.mixBlendMode,
          `${axisId} source crop blends away square crop backgrounds`,
        ).toBe("multiply");
      }
      if (testInfo.project.name !== "mobile") {
        ...
        const diameters = Object.entries(axisCircleBoxes).map(([id, box]) => ({
          id,
          diameter: box.width,
          height: box.height,
        }));
        for (const { id, diameter, height } of diameters) {
          expect(
            Math.abs(diameter - height),
            `${id} axis circle keeps equal width/height`,
          ).toBeLessThanOrEqual(1);
          expect(
            Math.abs(diameter - axisCircleBoxes["safe-infrastructure"].width),
            `${id} axis circle diameter matches the grid`,
          ).toBeLessThanOrEqual(1);
        }
        expect(
          Math.abs(topLeft.y - topRight.y),
          "page 19 top-row circle centers align on desktop",
        ).toBeLessThanOrEqual(1);
        expect(
          Math.abs(bottomLeft.y - bottomRight.y),
          "page 19 bottom-row circle centers align on desktop",
        ).toBeLessThanOrEqual(1);
        expect(
          Math.abs(topLeft.x - bottomLeft.x),
          "page 19 left-column circle centers align on desktop",
        ).toBeLessThanOrEqual(1);
        expect(
          Math.abs(topRight.x - bottomRight.x),
          "page 19 right-column circle centers align on desktop",
        ).toBeLessThanOrEqual(1);
        expect(
          Math.abs(bottomLeft.y - topLeft.y - (bottomRight.y - topRight.y)),
          "page 19 row gaps stay consistent",
        ).toBeLessThanOrEqual(1);
      }
    }
    ...
    if (
      ...
      ["intro-incident", "intro-road-safety-plan"].includes(route.id)
    ) {
      await page.setViewportSize({ width: 760, height: 900 });
      await reader.screenshot({
...
test("Manual guide Chapter 3 highways renders retained Spanish terms with Russian support", async ({
  page,
}) => {
  for (const viewport of [
    ...
    await expect(
      section.getByRole("heading", {
        name: "Движение по автомагистралям и другим скоростным дорогам",
      }),
    ).toBeVisible();
    ...
    const issues = await section.evaluate((root) => {
      ...
      if (!selection || !firstBlock) {
        ...
        selection.removeAllRanges();
        if (!selectedText.includes("Autopistas"))
          problems.push("retained Spanish/Russian text is not selectable");
      }
      for (const element of Array.from(
        root.querySelectorAll('[data-testid="manual-guide-section-block"]'),
      )) {
        const rect = element.getBoundingClientRect();
        const style = window.getComputedStyle(element);
...
test("Manual guide exposes implemented Chapter 1, Chapter 2, Chapter 3, Chapter 4, Chapter 5, and Appendix III section pages", async ({
  page,
}, testInfo) => {
  ...
  const appendix3 = nav.locator('[data-guide-entry-id="appendix-3-cargo"]');
  ...
  if (!(await appendix3.evaluate((element) => (element as HTMLDetailsElement).open))) {
    await appendix3.locator("summary").click();
  }
  ...
  await expect(appendix3.getByText("Приложение III. Перевозка грузов и товаров")).toBeVisible();
  ...
  await expect(appendix3).toHaveAttribute("data-status", "active");
  ...
  await expect(appendix3.locator("summary small")).toHaveCount(0);
  await expect(nav).not.toContainText("готово");
  ...
  await expect(
    chapter5.getByText("Профилактика и помощь в ситуациях гендерного насилия"),
  ).toBeVisible();
  await expect(chapter5.getByText("Предупредительное и эффективное вождение")).toBeVisible();
  await expect(appendix3.getByText("Профиль перевозчика грузов")).toBeVisible();
  await expect(appendix3.getByText("Социальная ответственность")).toBeVisible();
  await expect(appendix3.getByText("Факторы, участвующие в вождении")).toBeVisible();
  await expect(appendix3.getByText("Безопасное вождение")).toBeVisible();
  await expect(appendix3.getByText("Элементы безопасности")).toBeVisible();
  await expect(appendix3.getByText("Автомагистрали")).toBeVisible();
  ...
  const publicTransport = reader.getByTestId(
    "manual-guide-pending-section-ch1-public-transport-system",
  );
  ...
  const requiredDocuments = reader.getByTestId(
    "manual-guide-pending-section-ch2-required-documents",
  );
  const incidentObligations = reader.getByTestId(
    "manual-guide-pending-section-ch2-incident-obligations",
  );
  ...
  const adverseConditions = reader.getByTestId(
    "manual-guide-pending-section-ch3-adverse-conditions",
  );
  ...
  const genderViolencePrevention = reader.getByTestId(
    "manual-guide-pending-section-ch5-gender-violence-prevention",
  );
  const anticipatoryEfficientDriving = reader.getByTestId(
    "manual-guide-pending-section-ch5-anticipatory-efficient-driving",
  );
  const app3CargoDriverProfile = reader.getByTestId(
    "manual-guide-pending-section-app3-cargo-driver-profile",
  );
  const app3SocialResponsibility = reader.getByTestId(
    "manual-guide-pending-section-app3-social-responsibility",
  );
  const app3DrivingFactors = reader.getByTestId(
    "manual-guide-pending-section-app3-driving-factors",
  );
  const app3SafeDriving = reader.getByTestId("manual-guide-pending-section-app3-safe-driving");
  const app3SafetyElements = reader.getByTestId(
    "manual-guide-pending-section-app3-safety-elements",
  );
  const app3Highways = reader.getByTestId("manual-guide-pending-section-app3-highways");
  await expect(cities).toBeVisible();
  await expect(sustainable).toBeVisible();
  await expect(bicycle).toBeVisible();
  await expect(app3CargoDriverProfile).toBeVisible();
  await expect(app3SocialResponsibility).toBeVisible();
  await expect(app3DrivingFactors).toBeVisible();
  await expect(app3SafeDriving).toBeVisible();
  await expect(app3SafetyElements).toBeVisible();
  await expect(app3Highways).toBeVisible();
  for (const sectionButton of [
    ...
    app3CargoDriverProfile,
    app3SocialResponsibility,
    app3DrivingFactors,
    app3SafeDriving,
    app3SafetyElements,
    app3Highways,
    ...
    await expect(sectionButton).not.toContainText("готово");
    await expect(sectionButton).not.toHaveAttribute("aria-label", /готово/u);
  ...
  await expect(cities).toHaveAttribute("aria-label", "Города для людей");
  await expect(app3DrivingFactors).toHaveAttribute("aria-label", "Факторы, участвующие в вождении");
  ...
  await expect(sustainable).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch1-sustainable-mobility",
  );
  await expect(sustainable).toHaveAttribute("data-source-pages", "23");
  await expect(pedestrian).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch1-pedestrian-priority",
  );
  ...
  await expect(publicTransport).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch1-public-transport-system",
  );
  ...
  await expect(legal).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch2-legal-responsibility",
  );
  await expect(legal).toHaveAttribute("data-source-pages", "44-45");
  await expect(requiredDocuments).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch2-required-documents",
  );
  await expect(requiredDocuments).toHaveAttribute("data-source-pages", "46-50");
  await expect(incidentObligations).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch2-incident-obligations",
  );
  ...
  await expect(priorityOfRules).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch3-priority-of-rules",
  );
  ...
  await expect(adverseConditions).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch3-adverse-conditions",
  );
  await expect(adverseConditions).toHaveAttribute("data-source-pages", "79-82");
  await expect(stoppingParking).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch3-stopping-parking",
  );
  await expect(stoppingParking).toHaveAttribute("data-source-pages", "83-88");
  await expect(alcoholDrugs).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch4-alcohol-drugs",
  );
  await expect(alcoholDrugs).toHaveAttribute("data-source-pages", "90-93");
  await expect(sleepFatigue).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch4-sleep-fatigue",
  );
  ...
  await expect(attitudeTypes).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch5-attitude-types",
  );
  await expect(attitudeTypes).toHaveAttribute("data-source-pages", "99");
  await expect(equalSociety).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch5-equal-society",
  );
  await expect(equalSociety).toHaveAttribute("data-source-pages", "99-100");
  await expect(genderViolencePrevention).toHaveAttribute(
    "data-route-hash",
    "#manual-section-ch5-gender-violence-prevention",
  );
  await expect(genderViolencePrevention).toHaveAttribute("data-source-pages", "100-101");
  await expect(anticipatoryEfficientDriving).toHaveAttribute(
    "data-route-hash",
  ...
  await expect(app3CargoDriverProfile).toHaveAttribute(
    "data-route-hash",
    "#manual-section-app3-cargo-driver-profile",
  );
  await expect(app3CargoDriverProfile).toHaveAttribute("data-source-pages", "153-154");
  await expect(app3SocialResponsibility).toHaveAttribute(
    "data-route-hash",
    "#manual-section-app3-social-responsibility",
  );
  await expect(app3SocialResponsibility).toHaveAttribute("data-source-pages", "155-159");
  await expect(app3DrivingFactors).toHaveAttribute(
    "data-route-hash",
    "#manual-section-app3-driving-factors",
  );
  await expect(app3DrivingFactors).toHaveAttribute("data-source-pages", "160-161");
  await expect(app3DrivingFactors).toHaveAttribute(
    "data-content-module-path",
    "src/data/manual-sections/app3-driving-factors.ts",
  );
  await expect(app3SafeDriving).toHaveAttribute(
    "data-route-hash",
    "#manual-section-app3-safe-driving",
  );
  await expect(app3SafeDriving).toHaveAttribute("data-source-pages", "162-168");
  await expect(app3SafetyElements).toHaveAttribute(
    "data-route-hash",
    "#manual-section-app3-safety-elements",
  );
  await expect(app3SafetyElements).toHaveAttribute("data-source-pages", "169-181");
  await expect(app3Highways).toHaveAttribute("data-route-hash", "#manual-section-app3-highways");
  await expect(app3Highways).toHaveAttribute("data-source-pages", "182-183");
  ...
  await app3DrivingFactors.click();
  await expect(page).toHaveURL(/#manual-section-app3-driving-factors$/);
  await expect(nav).toHaveAttribute("data-active-group-id", "appendix-3-cargo");
  await expect(nav).toHaveAttribute("data-active-child-id", "app3-driving-factors");
  await expect(app3DrivingFactors).toHaveAttribute("aria-current", "page");
  await expect(appendix3.locator(".manual-guide-children button.active")).toHaveCount(1);
  await expect(app3SafeDriving).not.toHaveClass(/active/);
  const app3Section = content.getByTestId("manual-guide-section");
  await expect(app3Section).toBeVisible();
  await expect(app3Section).toHaveAttribute("data-manual-section-id", "app3-driving-factors");
  await expect(
    app3Section.getByRole("heading", { name: "Факторы, участвующие в вождении" }),
  ).toBeVisible();
  await expect(app3Section).toContainText("Профессиональный водитель");
  await expect(app3Section).toContainText("Поза тела и здоровье");
  await expect(nav).not.toContainText("готово");

  await page.goto("/#manual-section-app3-highways");
  await expect(nav).toHaveAttribute("data-active-group-id", "appendix-3-cargo");
  await expect(nav).toHaveAttribute("data-active-child-id", "app3-highways");
  await expect(app3Highways).toHaveAttribute("aria-current", "page");
  const app3HighwaysSection = content.getByTestId("manual-guide-section");
  await expect(app3HighwaysSection).toHaveAttribute("data-manual-section-id", "app3-highways");
  await expect(app3HighwaysSection.getByRole("heading", { name: "Автомагистрали" })).toBeVisible();
  await expect(app3HighwaysSection).toContainText("Профессионализация перевозки грузов");
  await expect(nav).not.toContainText("готово");
  ...
  await expect(
    section.locator('[data-testid="manual-page-canvas"], [data-testid="manual-source-mask"]'),
  ).toHaveCount(0);
  ...
  const issues = await section.evaluate((root) => {
    ...
    for (const element of Array.from(
      root.querySelectorAll(
        '[data-testid="manual-guide-section-block"], .manual-principle-terms strong',
      ),
    )) {
      const rect = element.getBoundingClientRect();
      const style = window.getComputedStyle(element);
  ...
  await expect(page).toHaveURL(/#manual-section-ch1-cities-for-people$/);
  await expect(page.getByTestId("manual-guide-nav")).toHaveAttribute(
    "data-active-group-id",
    "chapter-1-sustainable-mobility",
  );
  await expect(page.getByTestId("manual-guide-section")).toHaveAttribute(
  ...
  await expect(sustainableSection).toHaveAttribute(
    "data-manual-section-id",
    "ch1-sustainable-mobility",
  );
  await expect(
    sustainableSection.getByRole("heading", { name: "Что такое устойчивая мобильность?" }),
  ).toBeVisible();
  ...
  await expect(
    sustainableSection.locator('[data-block-kind="vulnerability-ranking"]'),
  ).toBeVisible();
  const spaceComparisonImage = sustainableSection.locator(
    'img[src*="space-comparison-50-people-source.jpg"]',
  );
  await expect(spaceComparisonImage).toBeVisible();
  await expect(spaceComparisonImage).toHaveAttribute("data-visible-spanish", "true");
  await expect(spaceComparisonImage).toHaveAttribute(
    "data-source-image-exception",
    "source-image-original-visible-text",
  );
  await expect(spaceComparisonImage).toHaveAttribute(
    "data-visible-spanish-scope",
    "source-image-only",
  );
  ...
  await expect(
    sustainableSection.locator(
      '[data-testid="manual-page-canvas"], [data-testid="manual-source-mask"]',
    ),
  ).toHaveCount(0);
  ...
  const sustainableIssues = await sustainableSection.evaluate((root) => {
    ...
    for (const scroller of Array.from(root.querySelectorAll(".manual-source-row-scroll"))) {
      const sourceAsIsImage = scroller.querySelector(
        'img[data-source-image-exception="source-image-original-visible-text"][data-source-as-is="true"]',
      );
      ...
      if (scroller.scrollWidth > scroller.clientWidth + tolerance) {
        ...
        problems.push(
          `${parentBlock?.getAttribute("data-block-id") ?? "source row"} requires horizontal scroll`,
        );
      }
    }
    ...
    for (const element of Array.from(
      root.querySelectorAll(
        '[data-testid="manual-guide-section-block"], .manual-space-labels span, .manual-vulnerability-labels span, .manual-mobile-pair-label',
      ),
    )) {
      const rect = element.getBoundingClientRect();
      const style = window.getComputedStyle(element);
    ...
    for (const label of Array.from(
      root.querySelectorAll(".manual-vulnerability-labels span, .manual-mobile-pair-label"),
    )) {
      ...
      const pairGroups = [
        {
          ...
          selector: ".manual-vulnerability-mobile-pair",
          expected: 6,
        },
      ];
      for (const group of pairGroups) {
        const visiblePairs = Array.from(root.querySelectorAll(group.selector)).filter(
          (pair) => window.getComputedStyle(pair).display !== "none",
        );
        if (visiblePairs.length !== group.expected) {
          problems.push(
            `${group.name} has ${visiblePairs.length} visible mobile pairs, expected ${group.expected}`,
          );
        }
        for (const pair of visiblePairs) {
          ...
          const id =
            pair.getAttribute("data-mobile-pair-id") ?? pair.textContent?.trim() ?? group.name;
          if (!label || !icon) {
          ...
          if (
            labelRect.left < pairRect.left - tolerance ||
            labelRect.right > pairRect.right + tolerance ||
            iconRect.left < pairRect.left - tolerance ||
            iconRect.right > pairRect.right + tolerance
          ) {
  ...
  await expect(pedestrianSection).toHaveAttribute(
    "data-manual-section-id",
    "ch1-pedestrian-priority",
  );
  await expect(
    pedestrianSection.getByRole("heading", { name: "Пешеходный приоритет" }),
  ).toBeVisible();
  ...
  await expect(pedestrianSection).toContainText(
  ...
  await expect(
    pedestrianSection.locator('[data-block-kind="pedestrian-photo-comparison"]'),
  ).toBeVisible();
  await expect(pedestrianSection.locator('[data-block-kind="impact-diagram"]')).toBeVisible();
  await expect(
    pedestrianSection.locator('[data-block-kind="pedestrian-infrastructure"]'),
  ).toHaveCount(5);
  await expect(
    pedestrianSection.locator('[data-card-id="area-infrastructure"] .manual-infrastructure-visual'),
  ).toHaveCount(0);
  ...
  const pedestrianOriginalSourceImages = pedestrianSection.locator(
    'img[data-source-image-exception="source-image-original-visible-text"]',
  );
  ...
  const pedestrianRestrictionSigns = pedestrianSection.locator(
    'img[data-official-sign-exception="official-traffic-sign-source-as-is"]',
  );
  await expect(pedestrianRestrictionSigns).toHaveCount(1);
  await expect(pedestrianRestrictionSigns).toHaveAttribute(
    "src",
    /restriction-signs-source-as-is\.png/,
  );
  await expect(pedestrianRestrictionSigns).toHaveAttribute("data-visible-spanish", "true");
  await expect(pedestrianRestrictionSigns).toHaveAttribute(
    "data-visible-spanish-scope",
    "official-sign-image-only",
  );
  await expect(pedestrianRestrictionSigns).toHaveAttribute("data-source-as-is", "true");
  await expect(
    pedestrianSection.locator(
      ".manual-restriction-signs, .manual-no-parking-sign, .manual-authorized-sign, .manual-control-sign",
    ),
  ).toHaveCount(0);
  await expect(
    pedestrianSection
      .locator(".manual-source-image-term-translations")
      .filter({ hasText: "Prioridad peatonal" })
      .first(),
  ).toBeVisible();
  const pedestrianTextOutsideTranslations = await pedestrianSection.evaluate((section) => {
    const clone = section.cloneNode(true) as HTMLElement;
    clone
      .querySelectorAll(".manual-source-image-term-translations")
  ...
  const pedestrianIssues = await pedestrianSection.evaluate((root) => {
    ...
    for (const image of Array.from(root.querySelectorAll("img"))) {
      ...
      if (officialSignException === "official-traffic-sign-source-as-is") {
        if (visibleSpanish !== "true")
          problems.push(`${src} official sign exception must record visible-Spanish=true`);
        if (image.getAttribute("data-visible-spanish-scope") !== "official-sign-image-only")
          problems.push(`${src} has wrong sign exception scope`);
        if (image.getAttribute("data-source-as-is") !== "true")
          problems.push(`${src} must record source-as-is`);
        const rect = image.getBoundingClientRect();
        if (rect.width > image.naturalWidth + tolerance)
          problems.push(`${src} must not be upscaled beyond source crop width`);
      }
      if (sourceImageException === "source-image-original-visible-text") {
        if (visibleSpanish !== "true")
          problems.push(`${src} source image exception must record visible-Spanish=true`);
        if (image.getAttribute("data-visible-spanish-scope") !== "source-image-only")
          problems.push(`${src} has wrong source-image exception scope`);
        if (image.getAttribute("data-source-as-is") !== "true")
          problems.push(`${src} must record source-as-is`);
        const rect = image.getBoundingClientRect();
        if (rect.width > image.naturalWidth + tolerance)
      ...
      if (/pages\/page-02[4-9]\.jpg/u.test(src))
        problems.push(`${src} renders a full source page raster`);
    }
  ...
  await expect(
    bicycleSection.getByRole("heading", { name: "Велосипед", exact: true }),
  ).toBeVisible();
  ...
  await expect(bicycleSection).toContainText(
    "Официальная таблица знаков оставлена без изменений; пояснение ниже не является частью изображения.",
  );
  ...
  await expect(bicycleSection.locator('[data-block-kind="pedestrian-infrastructure"]')).toHaveCount(
    2,
  );
  ...
  await expect(
    bicycleSection.locator('img[data-source-image-exception="source-image-original-visible-text"]'),
  ).toHaveCount(2);
  const bicycleSignSheet = bicycleSection.locator(
    'img[data-official-sign-exception="official-traffic-sign-source-as-is"]',
  );
  ...
  await expect(bicycleSignSheet).toHaveAttribute(
    "data-visible-spanish-scope",
    "official-sign-image-only",
  );
  await expect(
    bicycleSection.locator(".manual-bicycle-sign-grid, .manual-bicycle-sign-marker"),
  ).toHaveCount(0);
  ...
  const bicycleIssues = await bicycleSection.evaluate((root) => {
    ...
    for (const image of Array.from(root.querySelectorAll("img"))) {
      ...
      if (/pages\/page-03[0-8]\.jpg/u.test(src))
        problems.push(`${src} renders a full source page raster`);
    }
    for (const marker of Array.from(
      root.querySelectorAll(
        ".manual-bicycle-helmet-labels span, .manual-bicycle-signal-labels article",
      ),
    )) {
      ...
      if (
        markerRect.width > viewportWidth + tolerance ||
        markerRect.left < -tolerance ||
        markerRect.right > viewportWidth + tolerance
      ) {
  ...
  expect(bicycleSelectedText).toContain(
    "Официальная таблица знаков оставлена без изменений; пояснение ниже не является частью изображения.",
  );
  ...
  await expect(publicTransportSection).toHaveAttribute(
    "data-manual-section-id",
    "ch1-public-transport-system",
  );
  await expect(
    publicTransportSection.getByRole("heading", {
      name: "Система общественного транспорта",
      exact: true,
    }),
  ).toBeVisible();
  ...
  await expect(
    publicTransportSection.locator('[data-block-kind="public-transport-comparison"]'),
  ).toBeVisible();
  await expect(
    publicTransportSection.locator('[data-block-kind="public-transport-infrastructure"]'),
  ).toBeVisible();
  await expect(publicTransportSection.locator('img[data-visible-spanish="false"]')).toHaveCount(4);
  const publicTransportSourceImages = publicTransportSection.locator(
    'img[data-source-image-exception="source-image-original-visible-text"]',
  );
  await expect(publicTransportSourceImages).toHaveCount(2);
  await expect(publicTransportSourceImages.nth(0)).toHaveAttribute(
    "src",
  ...
  await expect(
    publicTransportSection.locator(
      '[data-testid="manual-page-canvas"], [data-testid="manual-source-mask"]',
    ),
  ).toHaveCount(0);
  ...
  const publicTransportIssues = await publicTransportSection.evaluate((root) => {
    ...
    for (const image of Array.from(root.querySelectorAll("img"))) {
      ...
      if (/pages\/page-0(?:39|40)\.jpg/u.test(src))
        problems.push(`${src} renders a full source page raster`);
    }
    return problems;
  });
  ...
  if (testInfo.project.name === "chromium") {
    await expect(
      publicTransportSection.locator(".manual-public-transport-comparison-layout"),
    ).toHaveCSS("grid-template-columns", /px .*px/);
    await expect(publicTransportSection.locator(".manual-public-transport-card").first()).toHaveCSS(
    ...
    for (const width of [761, 768, 785]) {
      ...
      await expect(
        publicTransportSection.locator(".manual-public-transport-comparison-layout"),
      ).toHaveCSS("grid-template-columns", /^[0-9.]+px$/);
      await expect(
        publicTransportSection.locator(".manual-public-transport-card").first(),
      ).toHaveCSS("grid-template-columns", /^[0-9.]+px$/);
      const narrowIssues = await publicTransportSection.evaluate((root) => {
        ...
        for (const element of Array.from(
          root.querySelectorAll(
            ".manual-public-transport-comparison, .manual-public-transport-comparison-layout, .manual-public-transport-facts article, .manual-public-transport-card-grid, .manual-public-transport-card, .manual-public-transport-copy",
          ),
        )) {
          const rect = element.getBoundingClientRect();
          const id =
            element.getAttribute("data-block-id") ?? element.textContent?.trim() ?? "unknown";
          if (
            rect.width > 0 &&
            (rect.left < -tolerance || rect.right > viewportWidth + tolerance)
          ) {
  ...
  await expect(
    sharedTripSection.getByRole("heading", { name: "Совместная поездка", exact: true }),
  ).toBeVisible();
  ...
  await expect(sharedTripSection).toContainText(
    'эффект "на четыре автомобиля меньше" относится к поездке с другими водителями',
  );
  ...
  const sharedTripSourceImage = sharedTripSection.locator(
    'img[data-source-image-exception="source-image-original-visible-text"]',
  );
  ...
  await expect(sharedTripSourceImage).toHaveAttribute(
    "data-visible-spanish-scope",
    "source-image-only",
  );
  ...
  const sharedTripIssues = await sharedTripSection.evaluate((root) => {
    ...
    for (const element of Array.from(
      ...
      const id =
        element.getAttribute("data-block-id") ??
        element.getAttribute("data-benefit-id") ??
        element.textContent?.trim() ??
        "unknown";
      if (rect.width > 0 && (rect.left < -tolerance || rect.right > viewportWidth + tolerance)) {
        problems.push(`${id} overflows viewport horizontally`);
    ...
    for (const image of Array.from(root.querySelectorAll("img"))) {
      ...
      if (/pages\/page-0(?:41|42)\.jpg/u.test(src))
        problems.push(`${src} renders a full source page raster`);
    }
    return problems;
  });
  ...
  if (testInfo.project.name === "chromium") {
    await expect(sharedTripSection.locator(".manual-shared-trip-benefits-layout")).toHaveCSS(
      "grid-template-columns",
      /px .*px/,
    );
    await expect(sharedTripSection.locator(".manual-shared-trip-closing")).toHaveCSS(
      "grid-template-columns",
      /px .*px/,
    );
    for (const width of [761, 768, 785]) {
      ...
      const narrowIssues = await sharedTripSection.evaluate((root) => {
        ...
        for (const element of Array.from(
          root.querySelectorAll(
            ".manual-shared-trip-benefits, .manual-shared-trip-benefits-layout, .manual-shared-trip-benefit-grid, .manual-shared-trip-benefit-grid article, .manual-shared-trip-closing, .manual-shared-trip-quote",
          ),
        )) {
          const rect = element.getBoundingClientRect();
          const id =
  ...
  expect(sharedTripSelectedText).toContain(
    'эффект "на четыре автомобиля меньше" относится к поездке с другими водителями',
  );
  ...
  await expect(
    chapter2Section.getByRole("heading", { name: "Обязательные документы" }),
  ).toBeVisible();
  ...
  await expect(chapter2Section).not.toContainText(
    "После первого прохождения срок становится ежегодным",
  );
  await expect(chapter2Section.locator('[data-source-as-is="true"]')).toHaveCount(6);
  await expect(
    chapter2Section.locator(
      '[data-source-image-exception="source-document-example-original-visible-text"]',
    ),
  ).toHaveCount(6);
  await expect(
    chapter2Section.locator('[data-visible-spanish-scope="source-document-example-image-only"]'),
  ).toHaveCount(6);
  ...
  await expect(scoringSection).not.toContainText(
    "Соблюдать правила дорожного движения означает спасать жизни",
  );
...
test("Manual guide front glossary renders structured term translations responsively", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    section.getByText("Vía rápida (скоростная дорога):", { exact: false }),
  ).toBeVisible();
  await expect(
    section.getByText(
  ...
  await expect(aceraItem.locator(".manual-glossary-definition")).toContainText(
    "сектор общественной дороги",
  );

  const longTermItem = section.locator(
    '[data-testid="manual-glossary-item"][data-term-es="Ciclorodado con pedaleo asistido eléctricamente"]',
  );
...
test("Introduction guide exits on hash Back and keeps route buttons native", async ({ page }) => {
  ...
  await expect(reader.getByTestId("manual-guide-route-item-intro-road-pandemic")).toHaveAttribute(
    "role",
    "listitem",
  );
  await expect(reader.locator('button[role="listitem"]')).toHaveCount(0);
  ...
  await expect(page.getByTestId("intro-route-intro-incident")).toHaveAttribute(
    "aria-current",
    "page",
  );
...
test("route navigation resets stale window scroll on desktop top-level pages", async ({ page }) => {
  ...
  const scrollBeforeClick = await dispatchClickWithoutAutoScroll(
    page.locator(".tabs").getByRole("button", { name: "Материалы", exact: true }),
  );
...
test("manual route navigation resets stale window scroll on desktop", async ({ page }) => {
  ...
  const scrollBeforeClick = await dispatchClickWithoutAutoScroll(
    page.getByTestId("manual-guide-pending-section-ch3-stopping-parking"),
  );
  expect(scrollBeforeClick).toBeGreaterThan(100);
...
test("mobile route navigation resets stale window scroll", async ({ page }) => {
  ...
  await scrollWindowTo(page, 900);
  const scrollBeforeClick = await dispatchClickWithoutAutoScroll(
    page.getByRole("button", { name: /Источники/ }),
  );
  expect(scrollBeforeClick).toBeGreaterThan(100);
...
test("intro-plan-seguridad-vial work-axis grid fits at 320px", async ({ page }, testInfo) => {
  ...
  const columnCount = await axisGrid.evaluate(
    (element) =>
  ...
  const problems = await workAxes.evaluate((root) => {
    ...
    const withinViewport = (rect: DOMRect, label: string) => {
      if (rect.left < -tolerance || rect.right > viewportWidth + tolerance) {
        issues.push(
          `${label} overflows viewport horizontally: ${rect.left}..${rect.right} of ${viewportWidth}`,
        );
      }
    };
    if (document.documentElement.scrollWidth > viewportWidth + tolerance) {
    ...
    for (let index = 0; index < cards.length; index += 1) {
      ...
      if (
        Math.abs(circleRect.x + circleRect.width / 2 - (cardRect.x + cardRect.width / 2)) >
        tolerance
      ) {
      ...
      for (const [label, element] of [
        ...
        if (element.scrollWidth > element.clientWidth + tolerance) {
          issues.push(
            `${label} text overflows its box: ${element.scrollWidth} > ${element.clientWidth}`,
          );
        }
      }
    }
...
test("primary source reader opens, preserves app flows, and switches Russian/Spanish modes", async ({
  page,
}) => {
  await openPrimarySources(page);
  ...
  await expect(page.getByTestId("source-detail-pane")).not.toContainText(
    "Проверка точного текста ожидается.",
  );
  await expect(page.getByTestId("source-mode-simple")).toHaveAttribute("aria-pressed", "true");
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(trafficLawSource.chunks[0].simpleRu),
  );
  ...
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(trafficLawSource.chunks[0].fullTranslationRu),
  );
  ...
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(trafficLawSource.chunks[0].originalSpanish),
  );

  await expect(
    page.getByRole("button", {
      name: /Español simplificado|Spanish simplificado|Simple ES|Упрощенный испанский/i,
    }),
  ).toHaveCount(0);
  await expect(page.locator(".source-reader")).not.toContainText(
    /simplified Spanish|español simplificado|spanish simple/i,
  );
...
test("primary source search, filters, long-document TOC, and keyboard focus work locally", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1240, height: 900 });
  await openPrimarySources(page);
  ...
  await expect(
    page.getByRole("button", { name: new RegExp(trafficLawSource.shortTitleRu) }),
  ).toBeVisible();

  await page
    .getByLabel("Фильтр источников по практической категории")
    .selectOption(trafficLawSource.category);
  await expect(
    page.getByRole("button", { name: new RegExp(trafficLawSource.shortTitleRu) }),
  ).toBeVisible();
  await page
    .getByLabel("Фильтр источников по юрисдикции или типу")
    .selectOption(`jurisdiction:${trafficLawSource.jurisdiction}`);
  await expect(
    page.getByRole("button", { name: new RegExp(trafficLawSource.shortTitleRu) }),
  ).toBeVisible();
  ...
  await openSourceDocument(page, longPrimarySource);
  await expect(
    page.getByRole("navigation", { name: "Оглавление фрагментов источника" }),
  ).toBeVisible();
  await expect(page.getByText(`${longPrimarySource.chunks.length} фрагментов`)).toBeVisible();
  await page
    .getByRole("button", {
      name: new RegExp(longPrimarySource.chunks[50].officialLabel!.slice(0, 30)),
    })
    .click();
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(longPrimarySource.chunks[50].simpleRu),
  );
  await expect(page.getByTestId("source-chunk-reader")).not.toContainText(
    textSample(longPrimarySource.chunks[0].simpleRu),
  );
  ...
  await page.keyboard.press("Tab");
  await expect(
    page.getByRole("button", { name: new RegExp(primarySourceDocuments[0].shortTitleRu) }),
  ).toBeFocused();
});
...
test("primary source fragment navigation has stable desktop row geometry", async ({ page }) => {
  ...
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(trafficLawSource.chunks[3].simpleRu),
  );
  await assertSourceTocGeometry(page);
});
...
test("primary source fragment navigation has stable mobile row geometry", async ({ page }) => {
  ...
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(trafficLawSource.chunks[2].simpleRu),
  );
  await assertSourceTocGeometry(page, { checkHorizontalOverflow: true });
});

test("primary source reader adapts between compact and expanded widths without runtime network or PDF dependencies", async ({
  page,
}) => {
  const externalRequests: string[] = [];
  const pdfRequests: string[] = [];
  ...
  await expect(page.getByTestId("source-chunk-reader")).toContainText(
    textSample(cabaTrafficSource.chunks[0].simpleRu),
  );
  ...
  await expect(page.locator("iframe, embed, object")).toHaveCount(0);
  await expect(
    page.locator(".source-reader a[href$='.pdf'], .source-reader a[href*='.pdf']"),
  ).toHaveCount(0);
  expect(externalRequests).toEqual([]);
  expect(pdfRequests).toEqual([]);
  expect(backendLikeRequests).toEqual([]);
...
test("materials view renders topic guide status, list, details, canonical ticket data, and local images", async ({
  page,
}) => {
  ...
  const institutionDistanceTrapNote = firstTopic.trapNotes.find(
    (note: { textRu: string }) =>
      note.textRu.includes("5 metros") && note.textRu.includes("10 metros"),
  ) as { textRu: string } | undefined;
  if (!institutionDistanceTrapNote)
    throw new Error("Expected institution distance trap note in topic guide fixture.");
  ...
  const correctAnswer = canonicalQuestion.answers.find(
    (answer) => answer.id === canonicalQuestion.correctAnswerId,
  )!;
  const correctExplanation = firstTicket.answerExplanations.find(
    (item: { answerId: string }) => item.answerId === correctAnswer.id,
  )!;
  const incorrectExplanation = firstTicket.answerExplanations.find(
    (item: { answerId: string }) => item.answerId !== correctAnswer.id,
  )!;
  ...
  await expect(
    page.getByText("Текущие билеты: неофициальная B-практика, не полная официальная база GCBA"),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: new RegExp(firstTopic.titleRu) })).toBeVisible();
  await expect(
    page
      .getByRole("button", { name: new RegExp(firstTopic.titleRu) })
      .locator(`[aria-label="${difficultyAria[firstTopic.difficulty]}"]`),
  ).toBeVisible();
  await expect(page.getByRole("heading", { name: firstTopic.titleRu })).toBeVisible();
  await expect(
    page
      .locator(".materials-topic-heading")
      .locator(`[aria-label="${difficultyAria[firstTopic.difficulty]}"]`),
  ).toBeVisible();
  ...
  await expect(summaryImage.locator("img")).toHaveAttribute(
    "src",
    /content\/assets\/learning\/generated\/v1\/topic-/,
  );
  ...
  const institutionDistanceTrap = page
    .locator(".trap-note")
    .filter({ hasText: institutionDistanceTrapNote.textRu })
    .first();
  ...
  const firstTerm = page
    .locator(".materials-term")
    .filter({ hasText: firstTopic.spanishTerms[0].translationRu });
  ...
  await expect(
    firstTerm.getByText(firstTopic.spanishTerms[0].termEs, { exact: true }),
  ).toBeVisible();
  await expect(
    firstTerm.locator("[lang='es']").filter({ hasText: firstTopic.spanishTerms[0].termEs }),
  ).toBeVisible();
  ...
  await expect(
    ticketBlock.locator(`[aria-label="${difficultyAria[canonicalQuestion.difficulty]}"]`),
  ).toBeVisible();
  ...
  for (const answer of canonicalQuestion.answers) {
    ...
    await expect(
      ticketAnswers.getByText(translation.answerTranslations[answer.id], { exact: true }),
    ).toBeVisible();
  }
  await expect(ticketAnswers.getByText("Правильный ответ", { exact: true })).toBeVisible();
  await expect(
    ticketBlock
      .locator(".material-answer p")
      .filter({ hasText: correctExplanation.explanationRu })
      .first(),
  ).toBeVisible();
  await expect(
    ticketBlock
      .locator(".material-answer p")
      .filter({ hasText: incorrectExplanation.explanationRu })
      .first(),
  ).toBeVisible();
  await expect(ticketBlock.locator("img")).toHaveAttribute(
    "src",
    new RegExp(canonicalQuestion.image.localPath),
  );
});

test("materials view renders a dual-topic ticket as a full block in both assigned topics", async ({
  page,
}) => {
  const firstTopic = topicGuide.topics.find(
    (topic: { slug: string }) => topic.slug === "parking-clearances-and-corners",
  );
  const secondTopic = topicGuide.topics.find(
    (topic: { slug: string }) => topic.slug === "right-of-way-special-situations",
  );
  ...
  const firstTopicTicket = firstTopic.tickets.find(
    (ticket: { questionId: string }) => ticket.questionId === dualQuestionId,
  );
  const secondTopicTicket = secondTopic.tickets.find(
    (ticket: { questionId: string }) => ticket.questionId === dualQuestionId,
  );
  ...
  async function expectFullDualTopicTicket(
    ticketBlock: Locator,
    guideTicket: {
    ...
    for (const explanation of guideTicket.answerExplanations) {
      await expect(
        ticketBlock
          .locator(".material-answer p")
          .filter({ hasText: explanation.explanationRu })
          .first(),
      ).toBeVisible();
    }
  }
...
test("materials view stays local-first without external requests or PDF viewer", async ({
  page,
}) => {
  const externalRequests: string[] = [];
  const pdfRequests: string[] = [];
  ...
  page.on("request", (request) => {
    ...
    if (!["localhost", "127.0.0.1"].includes(url.hostname)) externalRequests.push(request.url());
    if (url.pathname.toLowerCase().endsWith(".pdf")) pdfRequests.push(request.url());
    if (/\/api\/|openai|live-ai|backend|analytics/i.test(url.pathname + url.hostname))
      backendLikeRequests.push(request.url());
  });
...
test("learning-image manifest coverage matches rendered local material/vocabulary assets", async ({
  page,
}) => {
  const topicUnitCount = topicGuide.topics.reduce(
    (
      total: number,
      topic: {
      ...
      total +
      1 +
      topic.learningMaterialRu.length +
      (topic.practicalReasoningRu?.length ?? 0) +
      topic.trapNotes.length +
      topic.spanishTerms.length,
    0,
  );
  const expectedCoverage =
    topicUnitCount +
    learningImages.coverage.filter(
      (record: { unitKind: string }) => record.unitKind === "vocabularyTerm",
    ).length;
  expect(learningImages.coverage).toHaveLength(expectedCoverage);
  expect(learningImages.images).toHaveLength(expectedCoverage);
  expect(
    learningImages.coverage.every((record: { status: string }) => record.status === "direct"),
  ).toBeTruthy();
  expect(
    learningImages.images.every((image: { localPath: string }) =>
      image.localPath.startsWith("content/assets/learning/generated/v1/"),
    ),
  ).toBeTruthy();
  ...
  const imageSrcs = await page
    .getByTestId("learning-image")
    .locator("img")
    .evaluateAll((images) =>
      images.map((image) => (image as HTMLImageElement).getAttribute("src")),
    );
  expect(
    imageSrcs.every((src) => src?.startsWith("/content/assets/learning/generated/v1/")),
  ).toBeTruthy();
});

test("modernized learning, vocabulary, and materials layouts avoid horizontal overflow on mobile", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 900 });
  await page.goto("/");
...
test("Manual guide opens Chapter 4 stress and distractions from direct routes", async ({
  page,
}, testInfo) => {
  ...
  await section.screenshot({
    path: testInfo.outputPath(`ch4-stress-direct-${testInfo.project.name}.png`),
  });
  ...
  await section.screenshot({
    path: testInfo.outputPath(`ch4-distractions-direct-${testInfo.project.name}.png`),
  });
});

test("Manual guide Chapter 4 alcohol overlay labels remain readable on phone width", async ({
  page,
}, testInfo) => {
  await page.setViewportSize({ width: 320, height: 900 });
  ...
  await expect(figure.locator('[data-overlay-label-id="acompanantes-label"]')).toContainText(
    "Пасс. мото",
  );
  ...
  const overlayProblems = await figure.evaluate((root) => {
    ...
    const problems: string[] = [];
    for (const label of Array.from(
      root.querySelectorAll<HTMLElement>(".manual-source-image-overlay-label"),
    )) {
      const id =
        label.getAttribute("data-overlay-label-id") ?? label.textContent?.trim() ?? "unknown";
  ...
  await section.screenshot({
    path: testInfo.outputPath(`ch4-alcohol-overlay-labels-phone-${testInfo.project.name}.png`),
  });
});

test("Manual guide full-width source image cards stay readable and avoid upscaling", async ({
  page,
}, testInfo) => {
  ...
  const scenarios: Array<{
    ...
    },
    {
      ...
      readableScrollCards: [
        { id: "app4-traffic-lights-page-198-source-card", minDisplayWidthPx: 757 },
      ],
    },
    {
    ...
    for (const scenario of scenarios) {
      ...
      if (!viewport.desktop) {
        for (const readableScrollCard of scenario.readableScrollCards ?? []) {
          await expectScrollableReadableSourceImageCard(
            page,
            readableScrollCard.id,
            readableScrollCard.minDisplayWidthPx,
          );
        }
      }
      const overflow = await page.evaluate(
      ...
      expect(
        overflow,
        `${scenario.sectionId} has no document-level horizontal overflow at ${viewport.name}`,
      ).toBeLessThanOrEqual(2);
      await section.screenshot({
        path: testInfo.outputPath(
          `manual-source-full-width-${scenario.sectionId}-${viewport.name}-${testInfo.project.name}.png`,
        ),
      });
    }
  }
  ...
  const bodyPostureSection = await openManualSection(
    "/#manual-section-app3-driving-factors",
    "app3-driving-factors",
  );
  const bodyPostureCard = bodyPostureSection.locator(
    '[data-card-id="app3-body-posture-source-card"]',
  );
  ...
  await bodyPostureSection.screenshot({
    path: testInfo.outputPath(
      `manual-source-translations-app3-driving-factors-${testInfo.project.name}.png`,
    ),
  });

  const hospitalSection = await openManualSection(
    "/#manual-section-app2-highways-hospitals",
  ...
  const documentsSection = await openManualSection(
    "/#manual-section-ch2-required-documents",
  ...
  await expectManualImageTermTranslations(
    ...
    [
      {
        termEs: "Documento Nacional de Identidad",
        translationRu: "Национальный документ личности",
      },
  ...
  await expectManualImageTermTranslations(
    ...
    [
      {
        termEs: "Verificación Técnica Vehicular",
        translationRu: "Техническая проверка транспортного средства",
      },
  ...
  const app2SafetySection = await openManualSection(
    "/#manual-section-app2-safety-elements",
  ...
  const app3SafetySection = await openManualSection(
    "/#manual-section-app3-safety-elements",
  ...
  const attentionSection = await openManualSection(
    "/#manual-section-ch4-distractions",
    "ch4-distractions",
  );
  await expectManualImageTermTranslations(
  ...
  const drivingCultureSection = await openManualSection(
    "/#manual-section-ch5-anticipatory-efficient-driving",
  ...
  await expectManualImageTermTranslations(
    ...
    [
      {
        termEs: /expresión de la cultura ciudadana/,
        translationRu: /выражением городской культуры/,
      },
    ],
    { minPairs: 1 },
  );
  ...
  await expectManualImageTermTranslations(
    ...
    [
      {
        termEs: "Circulación exclusiva para bicicletas",
        translationRu: "Исключительное движение для велосипедов",
      },
      {
        termEs: "Vereda de convivencia con peatones",
        translationRu: "Тротуар совместного движения с пешеходами",
      },
      {
        termEs: "Prohibido circular en bicicleta",
        translationRu: "Движение на велосипеде запрещено",
      },
  ...
  await expectManualImageTermTranslations(
    ...
    [
      ...
      {
        termEs: "Distancia de vehículos estacionados",
        translationRu: "Дистанция от припаркованных автомобилей",
      },
    ],
    { minPairs: 2 },
  );
    ...
    [
      {
        ...
        translationRu: /держась за другие транспортные средства/,
      },
    ],
    { minPairs: 1 },
  );

  const pedestrianPrioritySection = await openManualSection(
    "/#manual-section-ch1-pedestrian-priority",
    "ch1-pedestrian-priority",
  );
  await expectManualImageTermTranslations(
  ...
  const publicTransportSection = await openManualSection(
    "/#manual-section-ch1-public-transport-system",
    "ch1-public-transport-system",
  );
  await expectManualImageTermTranslations(
  ...
  for (const scenario of app4TranslationScenarios) {
    ...
    await expectManualImageTermTranslations(
      section.locator(`[data-card-id="${scenario.cardId}"]`),
      scenario.pairs,
      { minPairs: scenario.minPairs },
    );
    if (scenario.sectionId === "app4-signs-warning") {
      await section.screenshot({
        path: testInfo.outputPath(
          `manual-source-translations-app4-signs-warning-${testInfo.project.name}.png`,
        ),
      });
    }
  }
  ...
  await expect(noAvanzarCard.locator(".manual-source-image-term-translations")).toContainText(
    "NO AVANZAR",
  );
  await expect(noAvanzarCard.locator(".manual-source-image-term-translations")).toContainText(
    "Проезд запрещен",
  ...
  await expect(anexoPanel01.locator("img")).toHaveAttribute(
    "src",
    /anexo-regulatory-panel-01-source-as-is\.jpg/,
  );
  await expect(anexoPanel01.locator("img")).toHaveAttribute(
    "data-official-sign-exception",
  ...
  await expect(anexoPanel01.locator(".manual-source-image-term-translations")).toContainText(
    "NO AVANZAR",
  );
  await expect(anexoPanel01.locator(".manual-source-image-term-translations")).toContainText(
    "Проезд запрещен",
  ...
  await expect(anexoPanel02.locator(".manual-source-image-term-translations")).toContainText(
  ...
  await expect(anexoPanel03.locator(".manual-source-image-term-translations")).toContainText(
    "CIRCULACIÓN EXCLUSIVA (PEATONES)",
  );
  await expect(anexoPanel03.locator(".manual-source-image-term-translations")).toContainText(
    "Движение только пешеходов",
  );
  const anexoPanel04 = page.locator('[data-card-id="app4-regulatory-anexo-panel-04-source-card"]');
  await expect(anexoPanel04.locator(".manual-source-image-term-translations")).toContainText(
  ...
  for (const panelCardId of anexoPanelCardIds) {
    await expectRenderedManualImage(
      page.locator(`[data-card-id="${panelCardId}"] img`),
      panelCardId,
    );
  }
  ...
  for (const panel of anexoPanelSizing) {
    ...
    expect(panel.renderedWidth, `${panel.id} does not upscale`).toBeLessThanOrEqual(
      panel.expectedWidth + 1,
    );
  }
  ...
  await expect(blindSpotCard.locator("img")).toHaveAttribute(
    "data-source-image-exception",
    "source-image-original-visible-text",
  );
  await expect(blindSpotCard.locator("img")).toHaveAttribute(
  ...
  await expect(blindSpotCard.locator(".manual-source-image-term-translations")).toContainText(
    "PUNTO CIEGO AUTOS",
  );
  await expect(blindSpotCard.locator(".manual-source-image-term-translations")).toContainText(
    "Слепая зона автомобилей",
  );
  await expect(blindSpotCard.locator(".manual-source-image-term-translations")).toContainText(
  ...
  await expect(tireCard.locator("img")).toHaveAttribute(
    "src",
    /tire-manufacturing-tread-life-source-as-is\.jpg/,
  );
  ...
  await expect(tireCard.locator(".manual-source-image-term-translations")).toContainText(
  ...
  expect(tireSizing.renderedWidth).toBeGreaterThanOrEqual(
    Math.min(tireSizing.cardWidth * 0.7, 760),
  );
  ...
  expect(blindSpotSizing.renderedWidth).toBeGreaterThanOrEqual(
    Math.min(blindSpotSizing.cardWidth * 0.7, 546),
  );
  await page.goto("/#manual-section-app4-signs-regulatory");
  await expectRenderedManualImage(
    page.locator('[data-card-id="app4-regulatory-no-avanzar-source-card"] img'),
    "app4-regulatory-no-avanzar-source-card",
  );
  await expectRenderedManualImage(
    page.locator('[data-card-id="app4-regulatory-page-185-source-card"] img'),
    "app4-regulatory-page-185-source-card",
  );
  ...
  expect(
    noAvanzarSizing.focusedRendered.width,
    "focused NO AVANZAR does not upscale",
  ).toBeLessThanOrEqual(201);
  expect(
    noAvanzarSizing.focusedRendered.width,
    "focused NO AVANZAR uses its natural readable width",
  ).toBeGreaterThanOrEqual(199);
  expect(
    noAvanzarSizing.focusedRedBounds,
    "focused NO AVANZAR red sign bbox is measurable",
  ).not.toBeNull();
  expect(
    noAvanzarSizing.overviewFirstSignRedBounds,
    "overview NO AVANZAR red sign bbox is measurable",
  ).not.toBeNull();
  expect(
