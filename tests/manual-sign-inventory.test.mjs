...
function regulatoryDetachedLabelAttachment(entry) {
  ...
  return (
    ...
    (entry.baselineCropNaturalHeight >= 110 ||
      /placa|zona-de-caudales|ciclovia|exclusivo|discapacitados|ciclistas|peatones|barreras|ferroviarias|cajon|descienda|convivencia|interrupcion|desvio|obra|parada|evento|frentistas/u.test(
        searchable,
      ))
  );
}
...
test("inventory retains all baseline rows and final 037 summary counts", () => {
  ...
  assert.deepEqual(
    countBy(inventory.entries, "sourcePage"),
    Object.fromEntries(
      Object.entries(expectedEntriesBySourcePage).map(([page, count]) => [String(page), count]),
    ),
  );
...
test("source-limited rows disclose output-pixel 3x separately from effective/native quality", () => {
  ...
  for (const entry of inventory.entries.filter(signLike)) {
    ...
    const page185RegulatoryParkingAttachmentRow =
      page185RegulatoryParkingRow && /acarreo|zona-de-caudales|ciclovia/u.test(entry.id);
    ...
    if (regulatoryDetachedLabelAttachment(finalRow)) {
      assert.equal(
        finalRow.cropAuditBasis?.regulatoryDetachedLabelRightEdgeGuardPass,
        true,
        entry.id,
      );
      assert.equal(
        finalRow.cropAuditBasis?.regulatoryDetachedLabelSourceLabelTrimPass,
        true,
        entry.id,
      );
      assert.equal(
      ...
      if (finalRow.cropAuditBasis?.edgeContact?.right) {
        ...
        assert.ok(withinWidthGuard || withinPixelGuard, entry.id);
      }
    }
...
test("review-blocked crop regressions have explicit passing audit basis and corrected bounds", () => {
  ...
  for (const expectation of expectations) {
    ...
    if (expectation.maxSourceWidth)
      assert.ok(sourceRegion.width <= expectation.maxSourceWidth, expectation.id);
    if (expectation.maxSourceHeight)
      assert.ok(sourceRegion.height <= expectation.maxSourceHeight, expectation.id);
...
test("review-blocked warning contamination regressions have isolated final crops", () => {
  ...
  for (const expectation of expectations) {
    ...
    assert.ok(
      finalRow.finalSourceRegionAtBaseScale.width <= expectation.maxSourceWidth,
      expectation.id,
    );
    assert.ok(
      finalRow.finalSourceRegionAtBaseScale.height <= expectation.maxSourceHeight,
      expectation.id,
    );
  }
});
...
test("warning crops cannot pass reviewed audit with right-edge neighboring fragments", () => {
  ...
  for (const entry of warningEntries) {
    ...
    if (finalRow.cropAuditBasis?.edgeContact?.left) {
      assert.ok(
        finalRow.cropAuditBasis.relativeSourceWidthRatio <=
          finalRow.cropAuditBasis.warningHorizontalEdgeMaximumRelativeWidthRatio,
        entry.id,
      );
    }
  }
...
test("regulatory Zona de Caudales crops trim neighboring right-edge labels and signs", () => {
  ...
  for (const expectation of expectations) {
    ...
    assert.equal(finalRow.cropAuditBasis?.passes, true, expectation.id);
    assert.equal(
      finalRow.cropAuditBasis?.regulatoryCaudalesRightEdgeGuardPass,
      true,
      expectation.id,
    );
    assert.equal(
      finalRow.cropAuditBasis?.regulatoryCaudalesSourceLabelTrimPass,
      true,
      expectation.id,
    );
    assert.equal(finalRow.cropAuditBasis?.neighborContaminationGuardPass, true, expectation.id);
...
test("page-185 regulatory parking crops use the generalized right-edge and source-label guard", () => {
  ...
  assert.deepEqual(
    regulatoryParkingRows.map((entry) => entry.id),
    page185ParkingExpectations,
  );
  ...
  for (const id of page185ParkingExpectations) {
    ...
    if (attachmentExpectations.has(id)) {
      assert.ok(
        finalRow.finalSourceRegionAtBaseScale.height <=
          Math.ceil(finalRow.baselineCropNaturalHeight * 0.72),
        id,
      );
    }
  }
...
test("regulatory attachment crops trim detached source captions and right-edge neighbors", () => {
  ...
  const actualAttachmentIds = inventory.entries
    .filter(
      (entry) =>
        signLike(entry) && regulatoryDetachedLabelAttachment(finalRowById(finalRowsById, entry.id)),
    )
    .map((entry) => entry.id);
  ...
  for (const id of expectedAttachmentIds) {
    ...
    assert.equal(
      typeof finalRow.cropAuditBasis?.regulatoryDetachedLabelRightEdgeMeaningfulPixelRatio,
      "number",
      id,
    );
  ...
  const noRightEdgeExpected = ["app4regulatory-p186-025-ceda-el-paso-a-ciclistas-y-peatones"];
  ...
  const cleanRightEdgeByPixelGuard = finalRowById(
    finalRowsById,
    "app4regulatory-p186-017-circulacion-exclusiva-convivencia",
  );
  ...
  assert.ok(
    cleanRightEdgeByPixelGuard.cropAuditBasis.relativeSourceWidthRatio >
      cleanRightEdgeByPixelGuard.cropAuditBasis
        .regulatoryDetachedLabelRightEdgeMaximumRelativeWidthRatio,
  );
  assert.equal(
    cleanRightEdgeByPixelGuard.cropAuditBasis.regulatoryDetachedLabelRightEdgePixelGuardPass,
    true,
  );
  assert.ok(
    cleanRightEdgeByPixelGuard.cropAuditBasis
      .regulatoryDetachedLabelRightEdgeMeaningfulPixelRatio <=
      cleanRightEdgeByPixelGuard.cropAuditBasis
...
test("crop generator cannot stamp reviewed-final-correct without audit pass", () => {
  ...
  assert.match(
    ...
    /automatedCropAudit\.passes\s*\?\s*"reviewed-final-correct"\s*:\s*"pending-crop-audit"/u,
  );
...
test("known screenshot problem rows have corrected source bounds and no old CSS clip runtime path", () => {
  const inventory = loadJson(inventoryPath);
  const finalRowsById = loadFinalRowsById();
  const animalCart = inventory.entries.find(
    (entry) => entry.id === "app4regulatory-p185-011-no-circular-carro-de-traccion-animal",
  );
  const noParkingPlate = inventory.entries.find(
    (entry) =>
      entry.id === "app4regulatory-p185-021-no-estacionar-acarreo-de-infractores-placa-horar",
  );
  ...
  assert.equal(
    noParkingPlateFinal.finalTailTrimMode,
    "preserve-colorless-lower-attachment-trim-detached-source-label",
  );
  assert.ok(
    animalCartFinal.finalSourceRegionAtBaseScale.width < animalCartFinal.baselineCropNaturalWidth,
    animalCart.id,
  );
  assert.ok(
    noParkingPlateFinal.finalSourceRegionAtBaseScale.height <=
      Math.ceil(noParkingPlateFinal.baselineCropNaturalHeight * 0.72),
    noParkingPlate.id,
  );
  assert.equal(
    noParkingPlateFinal.cropAuditBasis?.regulatoryParkingRightEdgeGuardPass,
    true,
    noParkingPlate.id,
  );
...
test("runtime renderer uses direct image assets and no learner-facing sheet clip component", () => {
  ...
  assert.doesNotMatch(
    cssSource,
    /transform:\s*translate\(calc\(-1 \* var\(--manual-sign-crop-x\)/u,
  );
});
...
test("inventory and renderer avoid forbidden replacement or runtime-source patterns", () => {
  ...
  assert.doesNotMatch(
    inventoryText,
    /"cropAuditBasis"|"noUpscaleProof"|"baselineCropNaturalWidth"|"finalOutputSha256"/u,
  );
