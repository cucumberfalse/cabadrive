...
test("content validation command passes", () => {
  ...
  assert.match(
    output,
    new RegExp(
      `Learning images validated: ${learningSummary.coverageUnitCount} units, ${learningSummary.imageCount} local images`,
    ),
  );
  assert.match(
    output,
...
test("parking vocabulary source links match canonical ticket wording", () => {
  ...
  const canonicalText = (question) =>
    normalize(
      [question.officialTextEs, ...question.answers.map((answer) => answer.officialTextEs)].join(
        " ",
      ),
    );
  ...
  for (const term of vocabulary.filter((item) => parkingTermIds.has(item.id))) {
    for (const questionId of term.sourceQuestionIds) {
      ...
      assert.ok(
        canonicalText(question).includes(normalize(term.termEs)),
        `${term.id} termEs does not appear in ${questionId}`,
      );
    }
    for (const example of term.examples) {
      ...
      assert.ok(
        canonicalText(question).includes(normalize(example.textEs)),
        `${term.id} example text does not appear in ${example.questionId}`,
      );
    }
  }
});
...
test("Russian explanations cover every current question with structured rationales", () => {
  ...
  for (const explanation of explanations) {
    assert.ok(
      explanation.textRu.length >= 120,
      `${explanation.questionId} explanation is too terse`,
    );
    ...
    assert.ok(
      explanation.correctAnswerExplanationRu.length >= 40,
      `${explanation.questionId} correct-answer rationale is too terse`,
    );
    assert.ok(
      Object.keys(explanation.wrongAnswerExplanations).length >= 1,
      `${explanation.questionId} missing wrong-answer rationales`,
    );
  }
});
