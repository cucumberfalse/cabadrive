...
- Orchestrator may use `pnpm run pr:finalize -- --pr <number> --expected-head <sha> --feature specs/<feature-id>` to verify gates and squash-merge through GitHub. Pending required checks remain blockers unless Orchestrator explicitly passes `--auto-merge-pending`, which may only enable GitHub protected auto-merge and must not bypass branch protection.
