...
- May rely on final Architect and Analyst validation for the effective content head only. The effective content head is the PR head containing implementation, workflow docs/templates, feature memory, review fixes, and other behaviorally meaningful content. Role/process evidence must record `Effective content head: <40-hex-sha>`. A later final-validation evidence-only commit is allowed only to record role-owned validation evidence or process memory.
...
- If Architect and Analyst validated an effective content head and a later commit only records final-validation evidence in role-owned process memory, Orchestrator may continue only after role/process evidence records `Effective content head: <40-hex-sha>` and a read-only current-PR-head guard explicitly references that effective content head, proves the later commit is evidence-only, and confirms all merge-readiness gates still apply to the current head.
