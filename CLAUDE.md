...
- Completion-time cleanup of agent-created local environments is coordinated by Orchestrator and executed only by an assigned Cleanup Agent or explicitly equivalent cleanup role. Implementation Agent does not delete sibling worktrees or local environments unless the assignment is specifically cleanup-scoped.
