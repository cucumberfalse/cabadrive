...
- Final Architect and Analyst validation may apply to the effective content head: the PR head containing implementation, workflow docs/templates, feature memory, review fixes, and other behaviorally meaningful content. Role/process evidence must record `Effective content head: <40-hex-sha>`. A later commit may skip recursive role validation only when it is a final-validation evidence-only commit that records role-owned validation evidence or process memory. Orchestrator must run a read-only current-PR-head guard before completion or authorized merge mechanics; the guard must explicitly reference the effective content head by full SHA or unambiguous short prefix, and any post-validation non-evidence change makes prior validation stale and must be routed back through role-appropriate follow-up or final validation.
