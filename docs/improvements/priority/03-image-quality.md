...
3. **Фотографии билетов** — сторонний источник (`content/assets/questions/source-bandinopla-testdeconducir-b/*.jpg`): 275 уникальных локальных JPG, на которые приходится 276 ссылок из контента; типичная ширина **574 px** (максимум в выборке — 962 px). Контейнер `.question-image-frame` отображает их до **620 CSS px** (`src/styles.css:3558`: `width: min(100%, 620px, ...)`), т. е. при DPR=2 требуется 1240 px — фото растягивается более чем вдвое сверх натурального размера. Исходников лучшего качества нет — единственный путь = AI-апскейл или замена источника.
...
| (c) Фото билетов (сторонние) | `assets/questions/` (275 уникальных JPG, 276 ссылок, 9.6 МБ) | 574 px, JPEG-артефакты | AI-апскейл ×4 + даунскейл до ×2–2.3 (с ручным QA) |
...
- [ ] Батч-апскейл 275 уникальных JPG → мастера; ручной QA текстовых кадров; проверка всех 276 ссылок; журнал upscale-статусов в манифесте
